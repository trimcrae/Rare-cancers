...
def render():
    ...
    for p in records:
        ...
        rows.append(f'''<article class="literature-record" data-year="{escape(str(p['year']), quote=True)}" data-scope="{p['discovery_scope']}" data-confirmed="{str(p['screening_status'] == 'EMC_report_identified').lower()}" data-access="{p['access']}" data-types="{escape(types.lower(), quote=True)}">
...
<div class="record-links">{' '.join(links)}</div>{note}</article>''')
    counts = data['counts']
    focused = sum(p['discovery_scope'] != 'full_text_or_index' or p['screening_status'] == 'EMC_report_identified' for p in records)
...
<p>This is a discovery catalogue, not a claim that every listed record contains original EMC results. The default view includes title/abstract matches and reports reviewed to confirm EMC content, including subgroups within broader studies. Title and abstract matches are automated screening aids. Broader matches may mention EMC only in background or references. Papers about other tumour types that resemble EMC also need exclusion after review.</p>
