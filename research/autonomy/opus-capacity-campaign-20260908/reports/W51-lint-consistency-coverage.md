...
| A **stale cross-reference** (dead anchor, dead file link) | **NO** (no link, anchor or heading resolution anywhere in the module) | **NOT CAUGHT** | E2 (⚠ **NEGATIVE-PROBE SPECIMEN** — both targets were deliberately invalid; neither the anchor nor the file ever existed): a link whose text was `the dependency graph` and whose target was the same-document anchor `#4--the-dependency-graph-THAT-NO-LONGER-EXISTS`, plus a link whose text was `the SI` and whose target was the path `…/nr4a3-degrader-paper-SI-DELETED.md` → `0 ERROR`, **exit 0**. ⛔ The text and the target are written apart here on purpose. The injection was made into a scratch copy and never into the tree; reproduced as a live Markdown link, this evidence line is read by `systems_check.py` `check_links` as a broken pointer **of this report** ([K2] for the anchor, [K1] for the path), which is what the 6186189a run reported. Splitting the specimen preserves both strings verbatim and removes the false claim that this report points at them. |
