...
biomarker, and this file assembles them into one transparent accounting. It is the ONE
HOME of every number in that synthesis (CLAUDE.md rule 1).
  ⛔ *Superseded, retained (2026-09-08, residual batch R8): "nobody has pooled them".* That
  was a claim about the whole field made from a non-systematic selection. The 2020 disease
  review (PMC7563993) already brings mechanism, morphology, targets and clinical partner
  response together, and the 2025 review connects morphology, partner and therapy again.
  What is bounded and true is narrower: this file gives ONE transparent numerical accounting
  of the reports THIS SYNTHESIS EXAMINED, with denominators, source sentences and limits
  attached. No novelty claim about the literature at large is made or supported here.
...
⛔ HONEST CURRENT SCOPE OF THAT GUARD CLAIM (recorded 2026-09-08, residual batch R9). The last
retained run of the five fusion-partner test modules ended 107 failed / 75 passed with
PYTEST_EXIT=1, and its failure prefix is deleted-key errors raised BEFORE several mutations reach
`--check` at all. So the sentence above describes the guard's DESIGN and what it does when it runs
to completion; it is NOT a measurement that every named mutation is currently exercised against a
live `--check`. That retained failing run is preserved as historical evidence and is NOT
reconstructed, re-run or declared stale here, and guard maintenance remains OUT OF SCOPE for this
batch. Do not read this docstring as a claim that the suite is green.
"""
...
def zero_death_patients_to_reconcile(taf: dict, comparator: dict) -> dict:
    """INACTIVE / SUPERSEDED 2026-09-08 — NOT CALLED, AND NOTHING IN THE ARTIFACT USES IT.

    ⛔ READ THIS FIRST. This function is DEAD CODE retained as history. Its last caller was removed
    on 2026-09-08 when the two-cohort pooled death contrast it was derived from was withdrawn as
    source-unverified (cohorts[huang-2023-outcome].withdrawn_2026_09_08). NO THRESHOLD IS COMPUTED
    ANYWHERE IN THE LIVE BUILD PATH, no threshold appears in the artifact, and no new threshold has
    been calculated to replace it. The paragraphs below are the docstring as it stood while the
    function was live; they are DATED HISTORY (2026-08-27 / 2026-09-08 first pass) and are NOT a
    description of any current calculation. In particular, the sentence "the paper prints whatever
    this returns" was true of a build path that no longer exists.
...
CITATIONS = {
    ...
    "davis2017": {
        ...
        "verification_note": (
            ...
            "sentence that fixes the sunitinib denominators at 2 TAF15 / 8 non-TAF15. \u26a0 THAT "
            "DISCUSSION SENTENCE RUNS classical -> SD/PR AND CANNOT BE READ BACKWARDS. \u2b50 BUT "
            "THIS PAPER DOES CLASSIFY THE EIGHT, ELSEWHERE, AND THIS FIELD ONCE DENIED IT "
            "(corrected 2026-09-08, residual batch R2). Its Introduction (retained copy "
            "PMC5400622.txt line 14) states that the eight patients with clinical benefit -- six "
            "partial responses AND the two stable diseases -- harboured EWSR1-NR4A3. \u26d4 "
            "*Superseded, retained: 'IT FIXES THE DENOMINATORS, NOT THE COMPARATOR ARM'S PARTNER "
            "... so it cannot be read backwards to type the eight as EWSR1::NR4A3.'* The "
            "backwards-reading objection is correct about the Discussion sentence and WRONG as a "
            "statement about this source, which types all eight directly. The current record is: "
            "EIGHT EWSR1::NR4A3 CASES AS REPORTED BY A SECONDARY SOURCE. \u26d4 That is a reported "
            "classification of another group's cohort, NOT primary patient-level verification -- "
            "the 2014 primary is retained here as an abstract only -- and it does not transfer to "
            "the pazopanib trial, whose partner distribution no retained source gives."
        ),
    },
    "agaram2014": {
        "short": "Agaram 2014",
    ...
    "huang2023": {
        ...
        "population": (
            ...
            "Kaohsiung Chang Gung Memorial Hospital. ⛔ CORRECTED 2026-09-08 (residual batch R1): "
            "the clause 'follow-up available in 53' is REMOVED from this current field. That "
            "figure was transcribed from the withdrawn Table 1 reading, is quarantined at "
            "cohorts[huang-2023-outcome].withdrawn_2026_09_08.removed_denominators, and is NOT "
            "supported by the retained abstract. The number of patients followed for outcome in "
            "this series is UNKNOWN to this synthesis."
        ),
        "accessed": "2026-08-08",
        "verified": True,
        "verified_scope": (
            "⛔ IDENTITY AND SPECIFIED ABSTRACT CONTENT ONLY (current status, 2026-09-08). "
            "`verified: true` on this record means: the PMID/DOI/journal/first-author identity was "
            "machine-checked (see `identity_confirmed`), and the quantities this synthesis still "
            "uses from this paper -- the 58-case series size, the 46/9/2/1 partner counts, the "
            "78% size association with P = .025, the five named univariate factors, and the two "
            "factors that remained prognostically independent -- are verbatim-supported by the "
            "retained Europe PMC core record. IT DOES NOT MEAN the full text or Table 1 was read "
            "against a retained original. No retained original of the full text exists in this "
            "repository; the outcome cells that were once taken from it are withdrawn."
        ),
        ...
        "verification_note": (
            "⛔ CURRENT STATUS, 2026-09-08 (residual batch R1): THIS CITATION IS VERIFIED AT "
            "IDENTITY AND ABSTRACT SCOPE ONLY. This cohort contributes to the PREVALENCE pool and "
            "to abstract-scope descriptive context; IT DOES NOT CONTRIBUTE TO THE OUTCOME POOL, "
            "and this file holds no source-verified per-partner outcome counts from it. "
            "⛔ *Superseded, retained: 'THE PUBLISHED FULL TEXT WAS READ BY A HUMAN ON 2026-08-08 "
            "AND ITS TABLE 1 PER-PARTNER EVENT COUNTS ARE NOW IN THIS FILE. This cohort therefore "
            "contributes to the OUTCOME pool as well as the prevalence pool, and it is the second "
            "cohort ever to publish EMC outcome event counts stratified by NR4A3 partner.'* THAT "
            "CERTIFICATION IS WITHDRAWN. A human narrative of a PDF read is not a retained "
            "original: no retained original of the full text or of Table 1 exists in this "
            "repository, the transcribed cells are quarantined at "
            "cohorts[huang-2023-outcome].withdrawn_2026_09_08, and the 'second cohort ever' "
            "superlative was in any case a claim about the literature at large that no "
            "systematic search supports. ⚠ THIS IS SCOPED ABSENCE, NOT A FINDING THAT THE "
            "PUBLISHED COUNTS ARE WRONG; it reopens only on authentic cell-, endpoint-, "
            "denominator- and follow-up-level evidence. The measurement history below is retained "
            "as dated 2026-08-08 record, not as current verification. ⚠ *Superseded, "
            ...
            "with a browser. ⛔ *Superseded, retained: '-- which is exactly how these counts were "
            "obtained.'* That clause asserted the unretained human read as the counts' provenance "
            "and is withdrawn with it (2026-09-08). Europe PMC's own "
...
COHORTS = [
    ...
    },
    {
        ...
        "overlap_note": (
            ...
            "split -- and is the only PRIMARY-report EWSR1 typing this file holds for any of "
            "those eight. ⭐ CORRECTED 2026-09-08 (R2): it is NOT the only typing. Davis 2017 "
            "(secondary) classifies all eight clinical-benefit patients as EWSR1-NR4A3; this "
            "2012 report independently and primarily types two of them. It "
            "covers two of the eight at primary level; the other six are typed by Davis 2017's "
            "secondary account and by nothing primary held here."
        ),
    },
    # ---- endpoint: disease-specific death by partner -----------------------
    {
        ...
        "pool": True,
        "pool_note": (
            "⛔ CURRENT STATUS 2026-09-08 (residual batch R1/R6): THIS IS THE ONE COHORT IN THE "
            "MAIN EVENT ANALYSIS. It is not pooled with any other outcome cohort, so every figure "
            "derived from it is a SINGLE-COHORT Wilson interval, stated as such rather than "
            "dressed as a meta-analysis. ⛔ *Superseded, retained: 'Pooled with "
            "huang-2023-outcome (POLICY-evidence.md s2.2) ... This cohort is now the SMALLER of "
            "two.'* Huang 2023's Table 1 counts are withdrawn as source-unverified and "
            "quarantined; there is no second numerical outcome cohort. ⚠ AND 'ONE COHORT IN THE "
            "MAIN EVENT ANALYSIS' IS NOT 'ONE SOURCE-VERIFIED COHORT': Sjogren 2003's per-partner "
            "recorded tumour-related deaths (0/3 TAF15 against 1/5 EWSR1) are ALSO verified "
            "against a retained primary source and are reported as separate descriptive context "
            "at cohorts[sjogren-2003-prevalence].recorded_outcomes_as_separate_descriptive_"
            "context. The distinction is a selection judgement made by this synthesis, not a "
            "statement about what the sources contain."
        ),
        "follow_up_warning": (
            ...
            "⛔ REWRITTEN 2026-09-08 (residual batch R4). THE DIRECTION AND AMOUNT OF THE "
            "RESULTING BIAS CANNOT BE SIGNED, IN EITHER DIRECTION, ON ANY ROW. In an uncensored "
            "during-follow-up analysis follow-up ENDS AT DEATH, so the TAF15 arm's shorter window "
            "is partly PRODUCED BY its own 3/7 deaths; and a patient who has died is no longer at "
            "risk of a recorded recurrence or metastasis. Both mechanisms are real and neither "
            "identifies a net direction or amount from final counts alone, without event ordering "
            "and comparable risk sets. ⛔ TWO SIGNED READINGS ARE WITHDRAWN OUTRIGHT, NOT SOFTENED "
            "-- retained as superseded: (i) 'the bias runs AGAINST the TAF15 arm accruing events, "
            "so the death excess is observed DESPITE shorter observation'; (ii) 'informative "
            "censoring by the competing event biases the arm with more deaths and shorter "
            "observation DOWNWARD on those two rows if anything. THE SIGN MATTERS FOR HOW THE "
            "NULL ON THOSE ROWS IS READ: an understated TAF15 recurrence and metastasis rate "
            "means the two-cohort null is not evidence that those endpoints do NOT differ by "
            "partner.' ⛔ THERE IS NO TWO-COHORT NULL. The second numerical cohort is withdrawn, "
            "and a non-significant contrast on these counts was never a null in the first place: "
            "at these denominators clinically substantial differences in either direction remain "
            "compatible with the recorded events. ⚠ *Superseded, retained: 'the reversal reported "
            "below'* -- and the node it pointed at, "
            "analyses.B_outcome_by_partner.metastasis_reading, no longer exists; the live cell is "
            "analyses.B_outcome_by_partner.source_verified_recorded_outcomes.distant_recurrence. "
            "The honest statement is that the windows differ, that the difference is not "
            "independent of the endpoints being compared, and that this is a limitation of the "
            "display rather than a correction that can be applied to it."
        ),
    },
    {
    ...
    },
    {
        ...
        "context_note": (
            "A FOURTH SERIES TESTING THE PARTNER AGAINST SURVIVAL, AND IT FAILS TO ESTABLISH AN "
            "ASSOCIATION ON ITS OWN ENDPOINT: 'no statistically "
            "significant correlation was detected between OS and fusion subtypes'. ⛔ *Superseded, "
            "retained (2026-09-08, R4): 'AND IT IS NULL'.* A series that fails to establish an "
            "association has not established a null; with 2 TAF15 patients of 18 it could not. "
            "It reports the "
            ...
            "agaram-2014 supplies every TAF15 disease-specific death in the main event analysis "
            "(3 of 7 TAF15 patients). ⛔ *Superseded, retained (2026-09-08, R1): 'agaram-2014 "
            "supplies 3 of the 7 pooled TAF15 disease-specific deaths.'* There is no seven-death "
            "pool: the second cohort that supplied the other four is withdrawn as "
            "source-unverified. Nobody has "
            "checked the overlap. "
    ...
    },
    {
        ...
        "published_p_values": {
            ...
            "note": (
                ...
                "no conventional threshold and the SIZE covariate does. ⛔ *WITHDRAWN OUTRIGHT "
                "2026-09-08 (residual batch R3), retained as superseded: '-- the same pattern "
                "huang-2023-outcome's multivariable model shows.'* THE TWO ARE NOT THE SAME "
                "PATTERN AND MUST NOT BE CONTRASTED THAT WAY. Whether this analysis was adjusted "
                "at all is NOT recorded in anything held here, so it is not a second multivariable "
                "result; its size p-value concerns DISTANT METASTASIS-FREE SURVIVAL while Huang's "
                "concerns DISEASE-SPECIFIC SURVIVAL, a different endpoint; and comparing two "
                "p-values cannot establish that one variable carries prognosis and the other does "
                "not."
            ),
        },
        "full_text_closed": True,
        "context_note": (
            "⛔ *WITHDRAWN OUTRIGHT 2026-09-08 (residual batch R3), retained as superseded: 'THE "
            "THIRD INDEPENDENT TEST OF THE PARTNER AS A PROGNOSTIC FACTOR, AND IT IS NEGATIVE AT "
            "THE CONVENTIONAL THRESHOLD ... -- the same pattern Huang 2023 found.'* THE "
            "INDEPENDENT-NEGATIVE-TEST STORY IS WITHDRAWN. This series' adjustment structure is "
            "not recorded in anything held here, so it is not established as a test of the "
            "partner independent of anything; a p-value above a threshold is not a negative "
            "result; and its endpoints are not Huang's. WHAT THE ABSTRACT REPORTS: NR4A3-EWS "
            "showed a TREND toward better disease-free survival and distant-metastasis-free "
            "survival versus NR4A3-TAF15, and primary tumour SIZE was significantly related to "
            "DMFS. The three p-values are in `published_p_values` above and are deliberately NOT "
    ...
    },
    {
        ...
        "entry_route_per_patient": {
            ...
            "new_in_this_series": {
                ...
                "fusion_status_at_entry": "NOT PREVIOUSLY REPORTED -- the series went looking: "
                                          "'To search for "
                                          ...
                                          "rather than inherited. ⛔ NARROWED 2026-09-08 (R6): "
                                          "these five were not previously reported, which is what "
                                          "the source says; the source does NOT document that "
                                          "they were unselectedly or 'freely' admitted, and no "
                                          "such recruitment rule is claimed here.",
            },
            "previously_reported_by_the_same_group": {
                "cases": [6, 7, 8, 9],
                "partners": {"EWSR1::NR4A3": 1, "TAF15::NR4A3": 2, "TCF12::NR4A3": 1},
                "fusion_status_at_entry": (
                    "ALREADY PUBLISHED. ⛔ NARROWED 2026-09-08 (residual batch R6) TO WHAT THE "
                    "SOURCE ACTUALLY DOCUMENTS. The retained Materials and Methods (lines "
                    "178-184) describe this work as an extension of the same group's prior "
                    "studies and distinguish previously reported from new material; that is what "
                    "is established. ⛔ *Superseded, retained: 'these four are in because their "
                    "fusion transcript had been reported, so partner assignment is the entry "
                    "ticket and a partner-unassigned tumour could not have been among them.'* "
                    "THAT IS A COUNTERFACTUAL RECRUITMENT RULE AND THE SOURCE DOES NOT DOCUMENT "
                    "IT. Prior publication of these four cases' fusion transcripts is a real and "
                    "sufficient reason for caution about partner ascertainment in this half of "
                    "the cohort; it is not proof of an inclusion criterion, and this synthesis "
                    "labels the partner-enrichment reading an INFERENCE rather than an "
        ...
        "context_note": (
            ...
            "variant-partner against 1/5 among the five not previously reported, and the "
            "partner-unassigned residue of zero on that half of the cohort follows from their "
            "already-known assignment. ⛔ *Superseded, retained (2026-09-08, R6): 'the five freely "
            "admitted'.* The source documents that five cases had not been previously reported; "
            "it does not document unselected ascertainment of them, and this cohort's own "
            "selection of available cytogenetic material applies to all nine. ⚠ THE PARTNER-"
            "ENRICHMENT READING IS AN INFERENCE FROM THE ENTRY ROUTES ABOVE, NOT A DOCUMENTED "
            "RECRUITMENT MECHANISM, AND IS NOT ASSERTED AS ONE. ⚠ NOR IS AN IDENTICAL RULE "
            "ESTABLISHED FOR EVERY INCLUDED STUDY: what is applied uniformly is this synthesis's "
            "own stated criterion, which is a selection judgement of this paper and not a "
            "property of the sources. ⚠ THE "
    ...
    # ---- mechanism / preclinical context, never pooled ---------------------
    {
        ...
        "context_note": (
            ...
            "anchorage-independent growth for TAF15-NR4A3, WITH INDEPENDENT BIOLOGICAL REPLICATES "
            "REPORTED IN THE SOURCE, and with the two chimeric proteins present at DIFFERENT "
            "ABUNDANCE despite similar mRNA. WHAT IT SUPPORTS: a mechanistic hypothesis in the "
            "measured systems. WHAT IT DOES NOT SUPPLY: a clinical treatment interaction -- an "
            "ENDPOINT AND DESIGN limitation, not an evidential one. ⛔ *NARROWED 2026-09-08 "
            "(residual batch R7), retained as superseded: 'so the mechanism is not an independent "
            "replication of the clinical correlation -- it is the same investigators explaining "
            "their own observation.'* SHARED INVESTIGATORS DO NOT BY THEMSELVES NEGATE "
            "EXPERIMENTAL EVIDENCE OR BIOLOGICAL REPLICATES. Four things are kept apart here: "
            "investigator independence (this consortium, Stacchiotti/Maestro, Milan/Aviano, is "
            "shared with both clinical reports, so this is not EXTERNAL replication); "
            "patient/sample overlap (unestablished either way); experimental replication (the "
            "source reports independent biological replicates); and endpoint relevance (an "
            "engineered-construct effect is not a clinical response endpoint). The consortium "
            "overlap bears on the first only."
        ),
    },
    {
        "id": "bangerter-2022-exvivo",
        "endpoint": "preclinical_drug_response",
        "label": "Two patient-derived ex vivo models from two DIFFERENT patients: USZ20-EMC1 (EWSR1::NR4A3) vs USZ22-EMC2 (TAF15::NR4A3)",
        "⛔_not_a_matched_pair": (
            "CORRECTED 2026-09-08 (residual batch R7). ⛔ *Superseded, retained: the label "
            "'Matched patient-derived ex vivo pair'.* THEY ARE NOT MATCHED AND NOT ISOGENIC. The "
            "retained patient information distinguishes a long-treated recurrent EWSR1 tumour "
            "from a newly diagnosed TAF15 tumour, and the models differ in other alterations as "
            "well as in fusion partner. In the source, 'matched' describes each model's "
            "correspondence with ITS OWN native tumour, not matching of patients across partners. "
            "Two patients differing in tumour history and other biology do not provide a "
            "controlled test of partner dependence."
        ),
        ...
        "context_note": (
            ...
            "model per arm and no statistical claim. ⛔ *WITHDRAWN OUTRIGHT 2026-09-08 (residual "
            "batch R7), retained as superseded: 'NEITHER MODEL WAS TESTED AGAINST AN "
            "ANTIANGIOGENIC TKI'.* THE 40-COMPOUND INVENTORY IS REFERRED TO A FIGURE NOT RETAINED "
            "HERE, so that blanket exclusion cannot be checked and is not asserted. What is "
            "checkable is narrower and is what is stated: THE THREE AGENTS VALIDATED IN BOTH "
            "MODELS ARE NAMED -- carfilzomib, doxorubicin and venetoclax -- AND NONE OF THEM IS "
            "AN ANTIANGIOGENIC TKI. So this work cannot confirm or refute the clinical "
            "correlation. ⛔ *WITHDRAWN, retained as superseded: 'the authors' finding is "
            "partner-INDEPENDENCE ... there is no evidence that the partner predicts response to "
            "drug classes in general'* AS A GENERAL INFERENCE. Similar responses in two "
            "unmatched patient-derived models to three named agents bound the observation; they "
            "do not establish partner independence of drug response in general, and two different "
            "patients' models cannot supply that test."
        ),
    },
]
...
def build() -> dict:
    ...
    analysis_tki = {
        ...
        "primary_non_overlapping": {
            "definition": (
                ...
                "held out and the pazopanib trial stands alone. ⛔ THIS CONTRAST IS ITSELF A "
                "CONDITIONAL RECONSTRUCTION (stated at first appearance, 2026-09-08, R2). The "
                "trial's primary report distinguishes 26 treated, 23 modified-ITT and 22 "
                "primary-endpoint evaluable; the retained reviews place three TAF15 patients in "
                "the trial and report that none responded, WITHOUT giving a "
                "partner-by-analysis-population flow. 0/3 versus 4/19 is therefore CONDITIONAL ON "
                "all three TAF15 patients sitting inside the 22 evaluable, and the three other "
                "exclusions between 26 and 22 are unaccounted for in every source held here. It "
                "is not an established partner partition of the evaluable cohort."
            ),
            "contrast": contrast(
        ...
        "secondary_assume_independent": {
            "definition": (
                "⛔ CONDITIONAL DESCRIPTIVE MIXTURE (label applied at every live site, "
                "2026-09-08, R2). Crude denominator-weighted tally of both cohorts "
                "(POLICY-evidence.md s2.2), valid ONLY under all three of: no patient appears in "
                "both reports; the pazopanib evaluability assumption above, which this mixture "
                "INHERITS; and the two comparator arms' shared `non-TAF15` label meaning the same "
                "thing. It is a mixture of different drugs, settings and eligibility, not a "
                "class response estimate."
            ),
            "contrast": contrast(
                "tki_response_both_cohorts",
                union_taf,
                union_oth,
                "⛔ CONDITIONAL DESCRIPTIVE MIXTURE, NOT A CLASS RESPONSE ESTIMATE. "
                "Denominator-weighted crude tally; the pazopanib trial supplies 19 of 27 "
                "comparator patients. It is conditional on THREE assumptions stated together: "
                "(1) no patient appears in both reports; (2) all three pazopanib TAF15 patients "
                "sit inside the 22-patient evaluable set (the pazopanib evaluability assumption, "
                "which the mixture inherits); (3) the two arms' shared `non-TAF15` label means "
                "the same thing. ⛔ *Superseded, retained: 'Both comparator arms are non-TAF15 "
                "arms, which is what makes them poolable with each other.'* A SHARED LABEL IS NOT "
                "A SCIENTIFIC JUSTIFICATION FOR A COMMON RESPONSE ESTIMAND. The drugs, settings, "
                "eligibility and observed comparator rates differ (75.0% against 21.1%), so the "
                "combined figure is a sample-weighted mixture under the selected study weights "
                "and is not a demonstrated drug-class response proportion. The drug-specific rows "
                "are the primary display and are reported first.",
            ),
        ...
        "overlap_sensitivity_bounds": {
            "explanation": (
                "⛔ *WITHDRAWN OUTRIGHT 2026-09-08 (residual batch R2), retained as superseded: "
                "'The truth lies between the two analyses above ... and the CONCLUSION IS THE "
                "SAME AT BOTH ENDS.'* THAT WAS A TRUTH-BOUNDS CLAIM AND IT IS NOT DEFENSIBLE. It "
                "treated overlap as the only unresolved condition. It is not: the pazopanib "
                "partition is itself CONDITIONAL on all three TAF15 patients sitting inside the "
                "22 evaluable, and three of the 26 treated patients are unaccounted for in every "
                "source held here, so neither endpoint of this range is a bound on the truth. "
                "WHAT THE TWO NUMBERS BELOW ARE: the denominators and TAF15 Wilson upper bounds "
                "of the two analyses actually displayed, under their own stated assumptions, "
                "recorded so a reader can see the span of the reported reconstructions. They are "
                "NOT an interval on any underlying quantity and NOT an argument that the "
                "conclusion is assumption-free."
            ),
        ...
        "sensitivity_analyses": {
            "taf15_patient_not_evaluable": {
                ...
                "conclusion": (
                    "The TAF15 upper bound rises; the contrast weakens. No claim here depends on "
                    "which way this resolves. ⛔ SCOPE, STATED 2026-09-08 (R2): THIS ILLUSTRATION "
                    "ADDRESSES ONE EXCLUDED PATIENT -- the single death between the 23 modified "
                    "ITT and the 22 evaluable -- UNDER THE FURTHER ASSUMPTION THAT THE REST OF "
                    "THE POPULATION IS OTHERWISE UNCHANGED. It does NOT enumerate the three "
                    "remaining exclusions between the 26 treated and the 22 evaluable, whose "
                    "partners no retained source gives, and it is therefore not an exhaustive "
                    "bound over the analysis-population reconstruction."
                ),
            }
        },
        "verdict": (
            ...
            "\u26d4 *Superseded, retained: 'the magnitude is not established'.* THAT SENTENCE BLURRED "
            "TWO DIFFERENT THINGS AND IS REPLACED (2026-09-08, R2): the OBSERVED differences of "
            "recorded response proportions exist and are printed above (21.1 and 37.0 percentage "
            "points); the POPULATION magnitudes they estimate are imprecise, the data cannot "
            "exclude a TAF15 response rate equal to the comparator arm's, and a treated-only "
            "contrast cannot establish treatment effect modification at any denominator. \u26d4 BOTH "
            "DENOMINATOR SETS ARE CONDITIONAL RECONSTRUCTIONS: 0/3 versus 4/19 assumes all three "
            "TAF15 patients are inside the 22 evaluable, and 0/5 versus 10/27 additionally "
            "assumes distinct patients across two different drugs and settings. \u26d4 "
    ...
    analysis_outcome = {
        ...
        **_roster("outcome_by_partner"),
        "⚠_roster_scope": (
            "⛔ WHAT THE ROSTER COUNTS ABOVE DO AND DO NOT MEAN (added 2026-09-08, residual batch "
            "R6). `cohorts_identified`, `cohorts_pooled` and `cohorts_excluded` are DERIVED by "
            "_roster() from the COHORTS objects carrying `endpoint: outcome_by_partner`. That is "
            "a SCHEMA scope, not a claim about which sources report per-partner outcomes. ONE "
            "FURTHER SERIES WITH SOURCE-VERIFIED PER-PARTNER OUTCOME COUNTS IS NOT COUNTED HERE: "
            "Sjogren 2003, whose cohort object carries `endpoint: partner_prevalence` because "
            "that is the pool it was assessed for, houses recorded tumour-related deaths of 0/3 "
            "TAF15 against 1/5 EWSR1 at "
            "cohorts[sjogren-2003-prevalence].recorded_outcomes_as_separate_descriptive_context. "
            "It is reported in `separate_descriptive_context_not_pooled` below. A schema choice "
            "must not become a claim about what the sources contain, and this note exists so the "
            "derived count cannot be read as one."
        ),
        "source_verified_recorded_outcomes": {
            "_cohort": "agaram-2014-outcome",
            "_what_this_is": (
                "⛔ CORRECTED 2026-09-08 (residual batch R6). THE ONE SERIES IN THIS SYNTHESIS'S "
                "MAIN EVENT ANALYSIS, verified against a retained primary source (PMC4015728). "
                "⛔ *Superseded, retained: 'The one cohort in this synthesis whose per-partner "
                "outcome event counts are verified against a retained primary source.'* THAT WAS "
                "FALSE AS A STATEMENT ABOUT SOURCE VERIFICATION. Sjogren 2003's per-partner "
                "recorded tumour-related deaths are also verified against a retained primary "
                "source and are reported as separate descriptive context (see "
                "`separate_descriptive_context_not_pooled.sjogren-2003` and "
                "cohorts[sjogren-2003-prevalence]). What distinguishes this cohort is a SELECTION "
                "JUDGEMENT made by this synthesis -- one series carries the displayed event "
                "contrasts, another is reported beside it -- not a difference in what the sources "
                "contain. Its arms are 16 "
        ...
        "separate_descriptive_context_not_pooled": {
            ...
            "huang-2023": (
                ...
                "the partner. ⚠ AND THE TWO STATEMENTS ARE KEPT SEPARATE (2026-09-08, R3): the "
                "abstract DOES report TAF15::NR4A3 among the factors associated with shorter "
                "UNIVARIATE disease-specific survival, and separately reports that it was not "
                "among the two factors remaining independent in the MULTIVARIABLE model. "
                "Collapsing those into 'the partner was not associated with survival' misreports "
                "the source. No coefficient, interval, event count or model specification is "
                "available in any retained source. ⛔ *Superseded, retained: 'the event count "
                "implied by a series this size makes model instability a live concern'.* THE "
                "EVENT COUNT CANNOT BE INFERRED FROM n. Small cohorts and several candidate "
                "predictors warrant a cautious concern about model stability, but model adequacy, "
                "precision and overfitting CANNOT BE ASSESSED without the actual event count and "
                "specification, and none is available. Size may itself lie on a "
        ...
        "what_is_not_established": (
            ...
            "these endpoints says anything about response to treatment. What IS supported is ONE "
            "SERIES IN THE MAIN EVENT ANALYSIS -- its recorded event counts and observation "
            "windows -- PLUS a separately reported, separately source-verified Sjogren outcome "
            "context, and three further series described at their own abstract scope."
        ),
    }
    ...
    analysis_prev = {
        ...
        "ascertainment_stages": {
            "_added": (
                "⛔ ADDED 2026-09-08 (residual batch R5). MOLECULAR CONFIRMATION AND NAMED-PARTNER "
                "IDENTIFICATION ARE DIFFERENT STAGES AND THIS SYNTHESIS ONCE PRINTED THE SECOND "
                "TWICE. ⛔ *Superseded, retained: a 'molecularly characterised' column reading "
                "24 / 57 / 12 / 62, which duplicated the named-partner counts for three of the "
                "four series.* THE TWO SETS OF INTEGERS ARE NOT THE SAME AND NOT ALL NUMBERS HERE "
                "ARE INVARIANT: the molecular-confirmation / successful-test counts are "
                "26 / 58 / 12 / 67 and the named-partner counts are 24 / 57 / 11 / 62. The "
                "named-partner counts are what the prevalence arithmetic uses and THEY ARE "
                "UNCHANGED, so 28/154 = 18.2 % (95 % CI 12.9-25.0) is unchanged. The corrected "
                "column is the confirmation stage, which was wrong."
            ),
            "_definitions": {
                "total_in_series": "Cases the report describes as its series.",
                "molecular_confirmation_or_successful_test": (
                    "Cases in which the reported molecular workup succeeded in establishing an "
                    "NR4A3-rearranged tumour. The exact assay and result definition differs by "
                    "series and is stated per row; these are NOT one harmonised criterion."
                ),
                "named_partner_assigned": (
                    "Cases in which a 5' partner gene was identified and named. THIS IS THE "
                    "PREVALENCE DENOMINATOR."
                ),
                "followed_for_outcome": (
                    "Cases with outcome follow-up reported. ⛔ THIS STAGE IS UNKNOWN FOR THREE OF "
                    "THE FOUR SERIES and is recorded as unknown rather than filled in. Huang's "
                    "53-followed figure is NOT reinstated here: it came from the withdrawn Table 1 "
                    "reading and no retained original supports it."
                ),
            },
            "per_series": {
                "agaram-2014-prevalence": {
                    "total_in_series": 26,
                    "molecular_confirmation_or_successful_test": 26,
                    "named_partner_assigned": 24,
                    "followed_for_outcome": 26,
                    "assay_and_result_definition": (
                        "All 26 received NR4A3/EWSR1 FISH and all 26 had an NR4A3 rearrangement "
                        "(retained full text, line 626). Two of the 26 had no identified 5' "
                        "partner. Follow-up is reported for all 26."
                    ),
                },
                "huang-2023-prevalence": {
                    "total_in_series": 58,
                    "molecular_confirmation_or_successful_test": 58,
                    "named_partner_assigned": 57,
                    "followed_for_outcome": "UNKNOWN",
                    "assay_and_result_definition": (
                        "FISH confirmed 58 EMC (primary abstract, PMID 36948401); one of the 58 "
                        "was NR4A3-rearranged with no identified partner. ⛔ The number followed "
                        "for outcome is UNKNOWN to this synthesis: see "
                        "cohorts[huang-2023-outcome].withdrawn_2026_09_08."
                    ),
                },
                "lenz-2023-prevalence": {
                    "total_in_series": 17,
                    "molecular_confirmation_or_successful_test": 12,
                    "named_partner_assigned": 11,
                    "followed_for_outcome": "UNKNOWN",
                    "assay_and_result_definition": (
                        "17 cases in the series; molecular testing succeeded in 12 (authentic "
                        "primary abstract, PMID 36563884). One of those 12 was NR4A3-positive with "
                        "no other identified alteration. The remaining 5 had no successful "
                        "molecular result and are outside the assigned denominator entirely -- a "
                        "different thing from the 1 unassigned case. ⛔ This is the ONE series "
                        "where the confirmation and named-partner counts genuinely differ from "
                        "each other AND from the series total."
                    ),
                },
                "paioli-2021-prevalence": {
                    "total_in_series": 67,
                    "molecular_confirmation_or_successful_test": 67,
                    "named_partner_assigned": 62,
                    "followed_for_outcome": "UNKNOWN",
                    "assay_and_result_definition": (
                        "67 identified under an inclusion rule requiring a molecularly confirmed "
                        "NR4A3 rearrangement, so confirmation and series total coincide by "
                        "construction. Explicit named-partner counts account for 62; ⛔ THE "
                        "FIVE-CASE RESIDUE IS NOT EXPLAINED BY THE ABSTRACT and is recorded as "
                        "unexplained, not as an unassigned-partner residue of a known kind."
                    ),
                },
            },
            "⛔_what_this_table_does_not_license": (
                "The four stages are not interchangeable and the differences between them are not "
                "a missingness model. No imputation is performed, no missing-at-random assumption "
                "is made, and no newly pooled series is introduced. The prevalence figures below "
                "are computed over NAMED-PARTNER ASSIGNED CASES ONLY, in four selected series."
            ),
        },
        ...
        "outlier_note": (
            ...
            "be -- are plausibly over-represented. ⚠ THAT IS A HYPOTHESIS ABOUT A SELECTION "
            "MECHANISM, LABELLED AS ONE (2026-09-08, R5), NOT AN IDENTIFIED BIAS. Nothing here "
            "signs the net direction of representativeness bias in this set, and an increased "
            "proportion of a partner in a selected sample does not by itself establish bias in "
            "any partner-specific outcome proportion. "
        ...
        "verdict": (
            "TAF15::NR4A3 is carried by {t}/{n} partner-assigned cases across four selected "
            "series -- {p}% (95% CI {lo}-{hi}). ⛔ NARROWED 2026-09-08 (residual batch R5). "
            "*Superseded, retained: 'That is the size of the population any partner-stratified "
            "recommendation would move.'* THIS IS A CONDITIONAL SHARE AMONG ASSIGNED CASES IN "
            "FOUR SELECTED SERIES, NOT AN ESTIMATE FOR ALL EMC and therefore not the size of any "
            "clinical population. Generalising it requires a missing-at-random assumption nothing "
            "here supports; the Wilson interval is conditional on a binomial model over the "
            "pooled counts and accounts for neither study selection, differential molecular "
            "ascertainment, nor between-study variation. What it does show is that every series' "
            "TAF15 arm is 2-10 patients, which is why every contrast on this page is imprecise."
        ).format(
            t=taf_n,
            n=tot,
    ...
    doc = {
        ...
        "method": {
            ...
            "not_used": (
                ...
                "\u26d4 THE REASON IS A JUDGEMENT ABOUT INTERPRETABILITY, STATED PLAINLY AND WITHOUT "
                "A TECHNICAL IMPOSSIBILITY CLAIM (rewritten 2026-09-08, residual batch R4). This "
                "paper ELECTS a descriptive accounting of heterogeneous, very sparse reported "
                "counts. Across the {n} response and outcome strata it displays, the per-stratum "
                "denominator runs from {dlo} to {dhi} and the event count from {elo} to {ehi}; a "
                "between-study variance estimated from that many strata at that scale would be "
                "unstable and a tau-squared from these counts would be hard to interpret, so "
                "quoting a random-effects interval would suggest precision this evidence base "
                "does not have. \u26d4 *WITHDRAWN OUTRIGHT, retained as superseded: 'a between-study "
                "variance is not estimable at that scale, a tau-squared from these counts would "
                "be an artefact', and the categorical assertion it stood beside.* A CATEGORICAL "
                "TECHNICAL ASSERTION FOLLOWED BY A DISCLAIMER IS NOT A COHERENT REPAIR: the "
                "earlier text said the variance was not estimable and then said this was not a "
                "claim of mathematical impossibility. It is neither impossible nor "
                "non-estimable; it is uninformative here, and that -- not a mathematical bar -- "
                "is the stated reason. A repository policy is likewise not by itself a scientific "
                "pooling justification. \u26a0 Superseded, retained: "
        ...
        "resolved_2026_08_08": {
            ...
            "huang2023_full_text": {
                ...
                "now": (
                    "⛔ NOT RESOLVED. CURRENT STATUS 2026-09-08 (residual batch R1): THIS ITEM IS "
                    "BACK IN THE UNRESOLVED SET AND ITS 2026-08-08 RESOLUTION IS WITHDRAWN. ⛔ "
                    "*Superseded, retained: 'RETRIEVED, by a human reading the published PDF on "
                    "2026-08-08 ... Table 1's per-partner event counts are now in "
                    "cohorts[huang-2023-outcome].strata and the outcome pool has gone from one "
                    "cohort to two.'* No retained original of that PDF or of Table 1 exists in "
                    "this repository; a human narrative of a read is not a retained original. "
                    "There is no `strata` node on cohorts[huang-2023-outcome] any more -- the "
                    "transcribed cells are quarantined at `.withdrawn_2026_09_08` and nothing "
                    "reads them -- and the outcome pool is ONE cohort (Agaram 2014), not two. "
                    "WHAT REMAINS TRUE FROM THE 2026-08-08 MEASUREMENT is the access diagnosis "
                    "only: the blocker was never a paywall -- Unpaywall and OpenAlex "
                    "independently designate a free publisher-hosted publishedVersion PDF "
                    "(oa_status bronze, oa_date 2023-03-21), and every automated fetch of it "
                    "returned HTTP 403 behind an anti-bot challenge. A BOT BLOCK AND A PAYWALL "
                    "HAVE DIFFERENT REMEDIES. The document itself remains unheld. ⚠ SCOPED "
                    "ABSENCE, NOT A FINDING THAT THE PUBLISHED COUNTS ARE WRONG."
                ),
                "evidence": (
            ...
            "nct02066285_eligibility_text": {
                ...
                "now": (
                    ...
                    "accrual opened. ⭐ APPLIED 2026-09-08. ⛔ *Superseded, retained: 'NOT ACTED "
                    "ON IN THIS FILE, deliberately.'* That was stale the moment the rationale was "
                    "rewritten: the exclusion criterion IS now part of "
                    "cohorts[sunitinib-2014].overlap_note and is stated in manuscript s3.1 and "
                    "s4.5 as evidence AGAINST re-enrolment. ⚠ AND IT IS THE PROTOCOL'S RULE, NOT "
                    "A PATIENT-LEVEL "
        ...
        "the_two_questions_this_page_answers": {
            "a_prognosis_by_partner": (
                ...
                "(2026-09-08). What the record supports is ONE SERIES IN THE MAIN EVENT ANALYSIS "
                "-- Agaram 2014's recorded per-partner event counts and observation windows -- "
                "PLUS a separately reported and separately source-verified Sjogren 2003 outcome "
                "context (0/3 TAF15 against 1/5 EWSR1 recorded tumour-related deaths), with three "
                "further series described at their own abstract scope. No independent prognostic "
                "effect, no equivalence, no null, no signed follow-up bias and no size-confounding "
                "explanation is established. See analyses.B_outcome_by_partner."
            ),
            ...
            "why_this_block_exists": (
                "Because the two are easy to conflate and the conflation would be an over-claim "
                "in the direction the whole page is trying to avoid. ⛔ *WITHDRAWN OUTRIGHT "
                "2026-09-08 (residual batch R3), retained as superseded: 'knowing that TAF15 "
                "patients die more often, crudely and probably because their tumours are "
                "bigger'.* THAT IS THE REJECTED CAUSAL SIZE EXPLANATION and it is not supported "
                "by anything in this synthesis: no partner coefficient, interval, event count, "
                "model specification or missing-data rule is available in any retained source, "
                "and size may itself be a consequence of fusion-driven biology or lie on the "
                "causal path between partner and outcome. The point that survives is only the "
                "distinction itself: a prognostic contrast among untreated-or-mixed patients and "
                "a response contrast among treated patients are different endpoints in different "
                "populations, and neither moves the other. That remains true with the pooled "
                "magnitude withdrawn."
            ),
        },
        "retrieval_provenance": {
            ...
            "every_count_on_this_page_is_read_from_a_retained_source": (
                "⛔ CORRECTED AND RENAMED 2026-09-08 (residual batch R1). ⛔ *Superseded, "
                "retained, under the old key `the_one_source_not_read_from_the_cache`: 'Huang "
                "2023's Table 1 counts were read by a HUMAN from the published PDF on 2026-08-08 "
                "... a future session that re-fetches will get the 403 again and must not read "
                "that as the counts being unavailable.'* THAT INSTRUCTION IS WITHDRAWN AND ITS "
                "PREMISE IS REVERSED. The Huang Table 1 counts are no longer on this page: they "
                "are withdrawn as source-unverified and quarantined at "
                "cohorts[huang-2023-outcome].withdrawn_2026_09_08. Every count this artifact now "
                "carries is read from a source retained byte-for-byte in this repository. A "
                "future session that re-fetches and meets the 403 SHOULD read it as the full text "
                "being unheld here, which is exactly what it is; the counts reopen only on an "
                "authentic retained original verified at cell, endpoint, denominator, follow-up "
                "and cause-of-death level. The identity of the paper WAS machine-verified and "
                "that stands -- citations.huang2023.identity_confirmed."
            ),
            "cache_slugs_used": [
