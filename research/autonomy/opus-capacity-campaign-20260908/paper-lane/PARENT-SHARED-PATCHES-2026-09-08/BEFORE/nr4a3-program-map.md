...
[THE ORDERED PLAN](#the-ordered-plan-spend-gated--read-top-to-bottom-for-whats-next) **and** the
`status` in [degrader-paper-schedule.json](program/degrader-paper-schedule.json).
⚠ *Superseded, retained: "its milestone `id`s match the stage tags **one-for-one**; that JSON is a machine
MIRROR of **this file**". Neither half holds today. The schedule carries **28** milestones against the plan's
**38** items, so the ids cannot be one-for-one; and THE ORDERED PLAN moved out of this file on 2026-08-05 into
[`systems/graph/plan.json`](../../systems/graph/plan.json), so the schedule mirrors THAT, not this. The
schedule's own `_README` still names this file as its human master, which is the same stale pointer seen from
the other end.* ⛔ **They are two registers that must agree, not one master and one mirror** — where they
disagree, the roadmap owns a gate's verdict and the plan JSON owns an item's marker.
...
[reviewer verdict](degrader/nr4a3-degrader-reviewer-revisions-2026-07-15.md) (verbatim) ·
[ternary-first strategy note](degrader/nr4a3-degrader-strategy-ternary-first.md) (biological/chemotype rationale) ·
[**ternary-selectivity strategy revision 2026-07-24**](degrader/nr4a3-ternary-selectivity-strategy-revision-2026-07-24.md)
(evidence behind the mechanism-first search and the six cost levers) ·
[the manuscript](degrader/nr4a3-degrader-paper.md) itself.
...
including [`paper-framing-options.md`](program/paper-framing-options.md), and renaming it to *"Seven invariants"*
...
| written as | means | home |
|---|---|---|
| **`C1`…`C25`** | a **configuration item on this page** — a frozen definitional choice a number is conditional on | [§3b](#3b--configuration--the-frozen-choices-every-conditional-number-depends-on) |
...
configuration register runs to **`C25`** ⚠ *(superseded, retained: "`C23`")*, so **`C10`–`C16` exist in BOTH and are spelled IDENTICALLY** — there
...
|---|---|---|
| [`THE ORDERED PLAN (spend-gated)`](#the-ordered-plan-spend-gated--read-top-to-bottom-for-whats-next) | **38** checkbox items, each with a gate, a price and a marker — ⚠ **MOVED 2026-08-05 — this row describes content this file no longer holds.** Its home is [`systems/graph/plan.json`](../../systems/graph/plan.json) → [`systems/views/plan.md`](../../systems/views/plan.md); the section here is a stub. ⚠ *Superseded, retained: "30 checkbox items" and the claim that `work_ledger.scan_plan_items` parses them HERE; its `DEFAULT_PLAN_DOC` is `systems/views/plan.md`, and this section now holds ZERO checkbox items.* | `work_ledger.scan_plan_items`, against the view |
| [`Spend summary`](#spend-summary) + [`Dependency spine`](#dependency-spine) | the pinned total and its derivation, the rung table, the authorisation graph — ⚠ **MOVED 2026-08-05 — this row describes content this file no longer holds.** Its home is [`systems/graph/plan.json`](../../systems/graph/plan.json) → [`systems/views/plan.md`](../../systems/views/plan.md); the section here is a stub. ⚠ *Superseded, retained: "the total must appear **here**, in `pricing.md` and in `bid-strategy.md`." `pinned-figures.json` → `derivations[0].must_appear_in` lists `systems/views/plan.md`, `pricing.md`, `bid-strategy.md` — this file is not on it and states no total, correctly.* | `lint_consistency.check_derivations`, against the view |
...
| [`Open decisions`](#open-decisions) | 15 numbered rulings, all closed — ⚠ **MOVED 2026-08-05 — this row describes content this file no longer holds.** Its home is [`systems/graph/plan.json`](../../systems/graph/plan.json) → [`systems/views/plan.md`](../../systems/views/plan.md); the section here is a stub. | **cited by number in 30 files and nothing resolves a decision number — the numbering is FROZEN** |
...
| [`Spending rules`](#spending-rules) | no pre-authorization · cheapest-decisive-first · GO/NO-GO per rung · PROJECTED never enters the pinned total — ⚠ **MOVED 2026-08-05 — this row describes content this file no longer holds.** Its home is [`systems/graph/plan.json`](../../systems/graph/plan.json) → [`systems/views/plan.md`](../../systems/views/plan.md); the section here is a stub. | — |
| [`GPU economics`](#gpu-economics-full-provenance-in-pricingmd) | a pointer to [pricing.md](../compute/pricing.md), plus the six cost levers — ⚠ **MOVED 2026-08-05 — this row describes content this file no longer holds.** Its home is [`systems/graph/plan.json`](../../systems/graph/plan.json) → [`systems/views/plan.md`](../../systems/views/plan.md); the section here is a stub. | `bid-strategy.md` names it |
...
| [`Current front`](#current-front) | ⚠ a duplicate that names its own homes and has **zero** inbound references; superseded by [§10](#10--the-roadmap--one-ordered-list). ⚠ **MOVED 2026-08-05 — this row describes content this file no longer holds.** Its home is [`systems/graph/plan.json`](../../systems/graph/plan.json) → [`systems/views/plan.md`](../../systems/views/plan.md); the section here is a stub. ⚠ *Superseded, retained: "Retained for one statement: the feasibility panel is WITHDRAWN." That statement left with the move, so §12 finding 12's pointer to it now lands on a stub.* | — |
...
| [`paper-framing-options.md`](program/paper-framing-options.md) | **what else this body of work could publish** — 7 framings on six columns | [§13](#13--the-deliverables-framing--an-open-question-with-a-register-and-no-decision), recorded as open and **not decided here** |
| [`path-family-synthesis.md`](program/path-family-synthesis.md) | the ranked synthesis across the five | read as a reader's guide; every ranking it states is graded independently below |
...
★ **ONE FILE PER DELIVERABLE:** [nr4a3-degrader-paper.md](degrader/nr4a3-degrader-paper.md) + its SI **is** both the
...
[emc-treatment-strategy.md](program/emc-treatment-strategy.md) and [IDEAS.md](../IDEAS.md).
...
> ### ↗ IF THIS BOARD'S FAILURES ARE THE PROGRAM'S ANSWER RATHER THAN A DETOUR — READ [`emc-post-degrader-options.md`](program/emc-post-degrader-options.md)
>
...
| **R7** | **The binder is paralogue-selective over NR4A1/NR4A2** — ⚠ **and the two halves are NOT the same requirement**: NR4A1-sparing is the HARD half (a named anti-target genotype a non-selective degrader reconstitutes), NR4A2-sparing is the SOFT half — bounded for developmental loss, unbounded for the adult transient loss a degrader actually delivers ([§2.4](#24--the-selectivity-requirement-is-asymmetric--and-this-page-stated-it-symmetrically)). ⚠ *Superseded, retained: "NR4A2-sparing is unbounded in both directions" — true when written, superseded by the 2026-08-03 MGI reading in §2.4's own table.* Node `B` | ○ open — the existing result is ⏸ parked | 🔒 (`V4`) | `V4` (no result) · `V6` `V7` `V8` `V9` `V10` · `V19` — the generation-matched null, ⚠ **PARTIAL** (its generative arm is unrun) | ⛔ **an unvalidated prediction.** Three separate blocks, only one of which is the instrument — see [§8](#8--the-two-live-routes-to-selectivity--and-where-each-is-actually-blocked) |
| **R8** | **A linker geometry is feasible** at an NR4A3-unique cysteine. Node `LK` | ✓ computed · ✅ **RECONCILED 2026-08-03** ⚠ *(superseded, retained: "⚠ not reconciled to its artifact" — the hold on quoting branch 1b was DISCHARGED that day and every claim re-checked against `nr4a3-linker-covalent-reach.json`)* | — ($0 CPU) | `V17` (fails its own positive control) + `INS-MONOVALENT-REACH` — the reach enumeration. ⚠ **Its own known-answer control PASSES and it still cannot license `R8`:** it inherits V3's inconclusive site question and V17's defective exposure cutoff — neither backticked here, because this clause explains a LIMIT rather than naming another instrument that verifies `R8` — so it *"can refute a route and cannot license one"* | **geometry only.** No thiol pKa, reactivity, adduct or degradation quantity; reach is necessary and never sufficient. And it is conditional on `R5` |
...
| **R11** | **The ternary adds or preserves selectivity.** Node `TS` | ○ future | 🔒 (via `R9`) | `V1` (passes, in scope) · `V16` (null with a bound, **uncalibrated**) · `V5` ⏸ FAILS · `V11` ⏸ no pass | one sequence-encoded candidate at **1 model per arm against a reproducibility bar of 3** (`C18`) ⚠ *Superseded, retained: the one-model-per-arm ceiling. Rung `5b-T` read **16 models per arm against the bar of 3** — models 2026-08-03, the `V1` read over them 2026-08-05 ([`nr4a3-5bt-signature.json`](../modalities/nr4a3-5bt-signature.json): `min_models_per_arm: 16`, `reproducibility_bar: 3`) — and returned **ZERO** sequence-encoded discriminating contacts in zero of them. ⛔ Note the two reads are over DIFFERENT structure sets: the old sequence-only co-folds and the new assembly-route ternaries, so the ceiling did not merely rise, it was replaced.* |
...
| **R16** | **NR4A3 is the right target** (EMC dependence). Node `TG` | ○ future — **DELEGATED** | — | the dTAG degradation test, in the EMC-program paper | not a blocker of this paper. `:2623`: *"This paper's claimed contribution is the target's computational druggability/selectivity, **not EMC efficacy**"* |
...
- ✅ **It changes the brief**, at $0 and with no new claim: **hard constraint — spare NR4A1; soft constraint
  — spare NR4A2 as far as the four handles allow, and carry the residual as a DISCLOSED, UNSIZED exposure
  question.** A narrowing of a requirement inherits no instrument, so nothing on
  [§3](#3--instruments--which-one-answers-each-requirement) has to pass for it to be adopted.
  ⚠ *Superseded, retained: "carry the residual as an exposure question rather than **a chemistry one**."
  The last three words read as a hand-off to a lever this program holds, and the HPA reading two bullets
  down removes it — 47 of 51 tissues co-express NR4A2 and NR4A3, so the selectivity has to be molecular
  and the residual is disclosed rather than discharged. The hard/soft weighting is unaffected.*
...
- ✅ **Both $0 observations have been taken (2026-08-03, row 26).** MGI's four public reports were scanned in full and HPA per-tissue nTPM is now measured for all three paralogues across 51 tissues: NR4A2 and NR4A3 are co-expressed in 47 of them, so **tissue distribution cannot separate target from anti-target and the selectivity has to be molecular**; NR4A2 is unbuffered (both paralogues below the cut) in 0. One home for every figure: [`nr4a2-sparing-bound.json`](../modalities/nr4a2-sparing-bound.json) → `hpa.overlap.counts`. Full argument, both directions, and the
  route it came from: [`target-route-options.md`](program/target-route-options.md) route 1.
...
| **V16** | The causal matched-pair test `S` (RUNG 5a-KS, ligand-side double difference, §2.10e) | ⛔ **none — it has no known-answer calibrator** | **S = −0.1297 ± 0.3264 kcal/mol** — its **preregistered null**, registered in advance as the LIKELY outcome and explicitly **not** a stop. ⛔ **Corrected 2026-09-08 (F02): it is NOT a bound** — an exploratory conditional estimate with a two-seed between-seed dispersion, compatible with zero at the observed precision | ⛔ **`S` may be read as a bound and may NOT be reported as calibrated** (Open decision 13). `S` is non-covalent and therefore **structurally incapable** of testing the categorical mechanism; `S ≈ 0` means the *marginal* wedge is absent, and STOP applies only if the categorical axis has ALSO failed | ✓ complete — **preregistered null, uncalibrated** | `R11` |
...
3. ⛔ **And "every withdrawn claim came from an untested instrument" is REFUTED by the largest retraction in
   ...
   The same is true of the E3-recruiter retraction (a biological-assembly frame defect, `:974`) and the
   Gate-3B withdrawal (cross-replica divergence, `:974`).
...
⛔ **FROZEN IS A PROVENANCE STATEMENT, NOT A CORRECTNESS ONE.** ⚠ *Superseded, retained: "Nineteen of the twenty-three above are frozen and uncontested … Four are not." The register now runs to **C25**, and the counts are DERIVED from its own status column rather than typed: **twenty of the twenty-five** are frozen and uncontested.* Twenty of the twenty-five above are frozen
...
| **R11** the ternary adds or preserves selectivity | `V1` `V5` `V11` `V16` | `C18` `C20` `C21` | **`C18`** — the claim ceiling is literally *"1 model per arm against a bar of 3"*; ⚠ *Superseded 2026-08-05: rung `5b-T` read **16 models per arm against the bar of 3** and returned ZERO discriminating contacts — over the assembly-route ternaries, a DIFFERENT structure set from the sequence-only co-folds this ceiling was written about.* **`C20`**/**`C21`** set what `V11`'s two nulls license |
...
verdict](degrader/nr4a3-degrader-reviewer-revisions-2026-07-15.md)) and govern what any result is
allowed to claim.
...
⚠ **FIVE requirements are not drawable here and that is a property of the graph, not of them.** `R3`
(submission gate), `R13` (fusion-context object), `R14` (AR/MR scope) and `R15` are **claim-ceiling conditions**
that bound every node rather than feeding one, and `R2` sits inside `R1`. ⚠ *Superseded, retained: "**Four**
requirements are not drawable here", which then named only four of the five — the graph draws R1, R4–R12 and
R16 (eleven of sixteen), so `R15` was undrawn AND unnamed, the one state this sentence exists to prevent.* They are in
...
graph BT
  ...
  LK["✓ R8 · LINKER geometry<br/>computed — RECONCILED 2026-08-03"]
  ARCH["✓ R9 · ternary REBUILT 5b-T<br/>$0 — gate returns NO-GO"]
...
- **`LK` ◐ → ✓.** [`nr4a3-linker-covalent-reach.json`](../modalities/nr4a3-linker-covalent-reach.json)
  **landed at commit `dc0befd9c`**, which retires the branch-1b banner's *"the artifact this section cites
  ...
  ([§7 branch 1b](#branch-1b--computed-and-reconciled-to-its-artifact-2026-08-03)). *Superseded, retained: "the
...
| **R7 · The binder is paralogue-selective** | ⚠ **More than this page used to say, and weaker than it sounds.** The paralogue ABFE **has been run and reported at three independent-seed replicates** with exactly the replicate-SD error bars this row used to ask for: ΔΔG(NR4A3−NR4A1) **−4.76 ± 2.03**, ΔΔG(NR4A3−NR4A2) **−4.98 ± 0.68**, both resolved below zero (`:1230–1239`, `:2303`). It is held **provisional and deliberately parked** for a named defect — `V9`, a soft-core-tail λ-overlap failure on *every* leg — *"It is not currently running: the whole ABFE block is **deliberately held** … it is not the next thing worth computing"* (`:1362–1365`). **"Run, reported, consciously parked" ≠ "not started"**, which is what this row said before. The paper's live reading is that selectivity rests on the binder margin **plus the nominated categorical handles**, and it explicitly refuses to write the ternary off (`:2600–2601`; SI `:141–144`) | **Three things, and they are not the same thing.** (1) **The instrument:** `V4`, the CREBBP/BRD4 selectivity known-answer test. *(highest leverage in the program · 🔒 **not authorized** · would **not** discharge this row — it is a **binary** control.)* (2) ⛔ **The missing physical term:** `R6`. A perfect instrument on today's inputs still would not settle this row. (3) ⛔ **The size of the prize versus the resolution** — the margin arithmetic in [§1](#1--the-thesis-the-north-star-and-the-operating-regime). ⚠ **This row is therefore not blocked *only* on the instrument**, which is how the page read before 2026-08-02 | ○ open — ⏸ **the existing result is parked**, not absent |
| **R8 · A linker geometry is feasible** | ✓ computed and committed ([`nr4a3-linker-covalent-reach.json`](../modalities/nr4a3-linker-covalent-reach.json), `dc0befd9c`): only **C397** of the three unique LBD cysteines is within tether range; **C420 is refuted everywhere** (0 of 60 placement×pendant cells, both conventions — `C9`, `C10`); ⚠ **C559 is NOT** — it survives at exactly one cell (`vhl|M3@term_a_exemplar | dab_branch`, 2 of 19 conformers) under through-space, and the artifact's `refuted_unique_cysteines` label is built from `best_corridor` alone, so it is stronger than its own data. ✅ **RECONCILED 2026-08-03**, claim by claim — see [§7 branch 1b](#branch-1b--computed-and-reconciled-to-its-artifact-2026-08-03) | reconciling the prose to the artifact ($0), then the pose re-run `R5` that every anchor depends on | ✓ work complete · claim **conditional on `R5` and unreconciled** |
| **R9 · Our ternary is correctly assembled** | ⛔ **nothing. `n_recovered: 0` of 3 arms**, and the existing prediction was built by the ⏸ route from a molecule that is unrecoverable | ✓ **REBUILT — rung `5b-T`, priced and gated 2026-08-02, RAN 2026-08-03 at $0, and its pre-registered three-arm gate returns `NO-GO`** (deepened 2026-08-05 by the `V1` read: zero sequence-encoded discriminating contacts in zero of 16 models per arm). ⚠ *Superseded, retained: "rebuild by the assembly route (`V2`) from a recorded molecule — ⛔ and it has no rung, no gate and no price"; §2.1 recorded the rung and price on 2026-08-02 and §10.1 recorded the run on 2026-08-03* | ✓ **work complete 2026-08-03 · $0 · claim UNMET (gate `NO-GO`)** ⚠ *(superseded, retained: "○ future — NOT STARTED · 🔒 unpriced")* |
...
| **R11 · The ternary adds selectivity** | one sequence-encoded candidate (Glu208 → Pro in NR4A1, Tyr in NR4A2); five further hits were placement artifacts; reproducibility untested at one model per arm. ⚠ **And the causal test has run**: `V16` returned **S = −0.1297 ± 0.3264**, a preregistered null whose recorded uncertainty is a two-seed between-seed dispersion (⛔ corrected 2026-09-08, F02: the earlier \|S\| ≳ 0.65 kcal/mol bound is withdrawn) — *"the **only** test in this program that asks whether a designed element **causes** discrimination"* (`:1889–1890`) | credible ternaries × ≥3 models per paralogue, scored by `V1` — gated on `R9`. And a known-answer calibrator for `V16`, which is unpriced | ○ future | ⚠ *Superseded 2026-08-05: rung `5b-T` read **16 models per arm against the bar of 3** and returned ZERO discriminating contacts — over the assembly-route ternaries, a DIFFERENT structure set from the sequence-only co-folds this ceiling was written about ([`nr4a3-5bt-signature.json`](../modalities/nr4a3-5bt-signature.json)).*
...
[`map-merge-inventory.md`](program/map-merge-inventory.md) records *"**76 rows**"*. Both are wrong, and the
...
| **Modulating the fusion through an NR4A–RXR heterodimer** *(would have served `R4`/`R7` by borrowing a solved pharmacology)* — ⭑ **added 2026-08-03** | *Premise false, on the receptor's own biology.* NR4A1 and NR4A2 form permissive RXR heterodimers that RXR ligands demonstrably modulate — and **NR4A3 does not heterodimerise with RXR**. The single place where this receptor family already has a working ligand handle is the one place our paralogue is absent from, so no rexinoid campaign reaches EWSR1::NR4A3 | reasoning, citation and its verification status: [`emc-post-degrader-options.md`](program/emc-post-degrader-options.md) tier 4 |
| **6-mercaptopurine / AF-1 agonism of the fusion** *(would have served `R4` with an APPROVED drug — the cheapest imaginable entry)* — ⭑ **added 2026-08-03** | *Premise false, on direction of effect.* 6-MP is the one approved drug that activates NR4A3, and it acts **through the AF-1 in the A/B domain, independently of the LBD**. The fusion **RETAINS** that domain — so the closure is not that the drug cannot reach its site, it is that **the drug ENHANCES NR4A3 activity while the fusion is gain-of-function**, so the one approved entry point pushes the wrong way. ⚠ *Superseded, retained: "Definitional, over a domain the disease deletes … the fusion replaces exactly that domain, swapping NR4A3 AF1 (1–260) for EWSR1's low-complexity region (1–264). A ligand whose entire mechanism lives in the deleted half cannot act on the chimera at any dose." Every clause of that is retracted; the measurement is in the ⛔ block at the end of this row.* | the swap is measured in [`target-route-census.json`](../modalities/target-route-census.json) `af1_to_lc_swap`; route write-up in [`emc-post-degrader-options.md`](program/emc-post-degrader-options.md) tier 4 | ⛔ **PREMISE CORRECTED 2026-08-06 (route framing audit).** *Superseded, retained: the fusion "replaces"/"deletes" NR4A3's AF-1.* **MEASURED:** NR4A3 transcript exons 1-2 are entirely NON-CODING, so "NR4A3 exon 3" IS residue 1 and `EWSR1(1-264)::NR4A3(1-626)` **RETAINS** the AF-1, DBD, hinge and LBD — in all 9 DBD-retaining breakpoint windows. EWSR1-LC is **additive**. RT-6MP is now closed on DIRECTION OF EFFECT (6-MP enhances NR4A3; the fusion is gain-of-function), `closure_kind: premise_false`, not `definitional`. See [`systems/AUDIT-2026-08-06-routes.md`](../../systems/AUDIT-2026-08-06-routes.md) X9.
...
Ids and costs are the plan's and the [schedule JSON](program/degrader-paper-schedule.json)'s
...
paper says *"**four** NR4A3-unique cysteines"* (`:1626–1628`) and
...
### Branch 1b — COMPUTED, AND RECONCILED TO ITS ARTIFACT (2026-08-03)

**✓ computed · ✅ reconciled**
⚠ *Superseded, retained: the heading "COMPUTED, NOT RECONCILED TO ITS ARTIFACT" and the badge "⚠ not reconciled". The hold was discharged 2026-08-03 and this section said so four lines below its own heading for three days.*
...
⚠ *Superseded, retained — this paragraph was left in LIVE VOICE directly beneath the ✅ RECONCILED line above, so the section asserted both that the hold was lifted and that the reconciliation was still owed:*
*"The prose below was written from the agent's reported values and **has not been reconciled to the landed artifact**, and at least one disagreement is readable today: result 3 names NR4A1/NR4A2 C534 as the residue that closes C397's window … That reconciliation is a $0 item on the roadmap."*
✅ **It was done on 2026-08-03 and the disagreement it names is resolved in the artifact's favour:** the widest graded cell records `closed_by: "NR4A1 C505"` at 17 backbone atoms (NR4A2 C534 also 17, NR4A1 C534 18), and the direction was never in question — *"in 30 of the 30 graded cells the FIRST cysteine to come into reach is a PARALOGUE one"*. Figures below are the artifact's.
...
- **⚠ "5 stay pocket-facing" is neither confirmed nor committed.** L406, T410, I484, I531, L534 (T407 and
  ...
  (`:587–601`; the number is §2.3, not §2.4). ⛔ **Against this page's own banner** — status is read from
...
- ⚠ Statistical hedging this page also dropped: *"a two-test Bonferroni correction moves p = 0.028 to
  **0.056**, i.e. borderline"*, plus spatial-correlation and selection caveats (`:718–732`).
...
`zinc_finger_window`) and read out in [`target-route-options.md`](program/target-route-options.md) finding 1. **Two
...
**unique-lysine** term, not a second cysteine"* (`:1671–1672`). Route B as drawn has a **single point of
...
⚠ **AND THE CATEGORICAL AXIS IS WIDER THAN C397 — ON THE RULER THIS PAGE MANDATES, WHICH IS THE ONLY ONE IT PERMITS — AND BOTH READINGS
...
counts). **The sweep cuts both ways — and the reading that BINDS is the threshold-free RANK, per [`path-family-synthesis.md`](program/path-family-synthesis.md) §2 row 6. The cutoff reading is registered as SUPERSEDED, not offered as an alternative** ([`sufex-second-handle.json`](../modalities/sufex-second-handle.json) `the_reported_ruler`):
...
- ★ **Under the threshold-free RANK this page mandates instead** ([§3.1 row `V17`](#31--the-instrument-table)),
  **Y419** — a SuFEx tyrosine one residue from C420 — sits **above NR4A1 Cys551**, the family's one
  literature-anchored covalent site and the very false negative that discredited the cutoff. It ranks **19 of 31** tetherable unique handles at RSA **0.221**, against the reference site's **0.165**. ⛔ **Two limits travel with it and neither is optional.** **(a)** The SuFEx tyrosine precedent is for **binding-site** tyrosines whose local environment lowers the phenol pKa and whose ligand supplies residence time; Y419 is an **exit-vector** residue and no pKa, microenvironment or residence time is modelled anywhere here. **(b)** ⛔ **`S1`'s chemoselectivity window may not be inherited** — it is computed over **cysteines only**, and inside Y419's own distance shell a SuFEx warhead faces **45** reactive competitors against the cysteine warhead's **7** (**6.43×**). [`sufex-second-handle.json`](../modalities/sufex-second-handle.json) owns those counts. **This is a REAGENT-level statement**: a second candidate handle, not a second engagement claim, and it does **not** unblock `R5`
- ⛔ **M398/M399 fall below the reference on BOTH readings and are dropped, not carried.** Naming them is the
  point: a sweep that only reports what survives is a sweep nobody can grade. The numbers: RSA **0.106** / **0.051** against the reference site's **0.165**, ranks **23** and **27** of **31** — [`sufex-second-handle.json`](../modalities/sufex-second-handle.json) `explicitly_dropped`.
...
| **§2.10e causal matched-pair test** — **S = −0.1297 ± 0.3264 kcal/mol** | *"the **only** test in this program that asks whether a designed element **causes** discrimination"* (`:1889–1890`) | `V16` → `R11` | ⛔ **A dependency, not a result.** It is the causal test of `R11` and of Route B's mechanism, it has **run**, and it returned a preregistered null **with a quantified bound** — the design's recorded uncertainty is a two-seed between-seed dispersion — ⛔ **corrected 2026-09-08 (F02): the earlier "wedge contribution of roughly \|S\| ≳ 0.65 kcal/mol (2σ)" bound is withdrawn** (`:1906–1908`). A dependency graph with no causal node cannot express the paper's own Tier-2/Tier-3 structure. ⛔ And it has **no calibrator** ([§3.4](#34--four-instrument-facts-this-page-used-to-be-missing)) |
...
revision](degrader/nr4a3-ternary-selectivity-strategy-revision-2026-07-24.md)):
...
| **18** | **≥3 ternary models per paralogue, then `V1`** | `R11` | ✓ **complete — models 2026-08-03, the `V1` read over them 2026-08-05, all 16 per arm against a bar of 3, and it returns ZERO** | — | ✅ **PRICED AND RAN — it is the second half of row 1's rung `5b-T`, at $0. The `V1` read covered all 16 models per arm against a bar of 3 and found no sequence-encoded discriminating contact in any of them** ([`nr4a3-5bt-signature.json`](../modalities/nr4a3-5bt-signature.json) → `sentence_replicated`). ⚠ **Superseded, retained:** *"✓ complete 2026-08-03 — the `V1` read covered all 16 models per arm against a bar of 3"* — the models are from 2026-08-03, but that run's `V1` step crashed on every attempt and the read did not exist until 2026-08-05; the row asserted a completed read for two days on the strength of a step that had never produced anything | `R11`'s reproducibility bar — currently **1 model per arm against a bar of 3**. `5b-T` reads **all 16** models per arm, which clears the bar five times over and costs seconds of free CPU, and its gate arm (B) turns the bar into a threshold with a stated null instead of a word. ⚠ **Superseded, retained:** *"— (gated on row 1) · unpriced."* |
...
| **21** | **5c — explicit ternary-ensemble refinement** | `R12` | ○ future | 🔒 (behind row 19) | **priced** | which lysine the ubiquitin actually reaches, per construct, as a distribution over unique-vs-conserved sites. ⛔ **SCOPE LINE, DECLARED BEFORE THE SPEND RATHER THAN RETROFITTED AFTER IT:** as currently scoped this rung is anchored on ONE docked pose of the carried candidate, so whatever distribution it returns is conditional on that pose and inherits `R5` unresolved. That is not a reason to refuse the rung — it is a line its result must carry from the moment it exists ([`pose-conditionality-census.json`](../modalities/pose-conditionality-census.json) `PC-15`) |
...
| **28** | ⛔ **Rule on `nr4a3-orientation-basins.json`'s `term_a_feasibility_envelope` — ⚠ AND THIS ROW'S STATED PREMISE IS FALSIFIED BY MEASUREMENT** | `R15` → rung `5b`, and it is an INPUT to the linker library | ○ **decision + $0** | **—** | **$0** | ⛔ **THE 3-BALL KERNEL IS A NO-OP FOR THIS FIELD AND THE DEFECT IS MONTE-CARLO CONVERGENCE.** A controlled A/B — pre-fix (`864a9518f`) against HEAD's exact solver, with poses, structure, distance field, cysteines, parameters and seed held identical — returns the SAME `shortest` / `geometrically_closed` verdict for all three cysteines and agrees to four decimals. What moves the answer is the MC draw at the committed budget: across seeds at the committed `n_mc`, `C559.geometrically_closed` flips and `C420.shortest` reads 16 or 20, while at 25× the budget every seed converges to C397 10 · C420 16 · C559 20, `closed` false — so two of three committed values do not reproduce. ⛔ **And the direction is NOT the conservative one this row assumed:** `geometrically_closed` is a CLOSURE, and an under-claiming reach estimate makes a closure MORE confident. ✅ **C397 is stable at 10 in every arm, seed and budget**, so no gate verdict, basin nomination or committed downstream number is at risk. ⭑ **The convergence limit was already measured on 2026-07-25** and is recorded in [`nr4a3-handle-ensemble.json`](../modalities/nr4a3-handle-ensemble.json) → `pooled_unbiased.*.shortest_linker_atoms._MC_CONVERGENCE`; this artifact carries the two unconverged readings with no such caveat. Options, evidence and the recommendation: [four-open-decisions-2026-08-07.md](degrader/four-open-decisions-2026-08-07.md) → decision 4. ⚠ *Superseded, retained:* *"`382c36947` named this field as built on the wrong kernel and explicitly did not regenerate it; the bias is the same one-sided, conservative under-claim, so nothing here is wrong in the dangerous direction"* — the commit's registration gap is real, the geometric consequence is not. Worked precedent for the ruling shape: [`nr4a3-linker-library-canonical.json`](../modalities/nr4a3-linker-library-canonical.json) |
...
| **30** | **The non-cysteine chemoselectivity window — run the reach enumeration for a SuFEx warhead at Y419** | `R8` `R15` (Route B's single-point-of-failure hedge) | ○ **not started** | — ($0) | **$0** — CPU/CI | ⛔ **The window that decides the cysteine handle has never been computed for any other warhead.** `nr4a3_linker_covalent_reach.cysteines_in` reads atom `SG` and nothing else, so `S1`'s *closed by a paralogue cysteine in 30 of 30 graded cells* is a statement about a **cysteine-only** competitor set. Inside Y419's own distance shell a SuFEx warhead faces **45** reactive competitors (Tyr/Lys/His/Ser/Thr, all three models) against the cysteine warhead's **7** — **6.43×** the population, on the SMALLER of which the existing window already closes ([`sufex-second-handle.json`](../modalities/sufex-second-handle.json)). ★ **What it settles:** whether Y419 is a usable second handle or a handle whose window is shut before it opens — i.e. whether Route B's single point of failure is a fact about the protein or a gap in the enumeration. ⛔ **What it does NOT settle:** the chemistry. SuFEx tyrosine precedent is for **binding-site** tyrosines with a pKa-perturbing environment and ligand residence time; Y419 is an **exit-vector** residue, and closing that gap needs a bench ([`sufex-second-handle.json`](../modalities/sufex-second-handle.json) `literature_record`) |
...
> [targets §1.2](program/target-route-options.md) · [instruments §0](../modalities/instrument-options.md)).
...
| **Q10** | **Re-run the linker-reach enumeration with the E3 arm REMOVED** — the covalent inhibitor / probe configuration at C397 | [route 2](program/target-route-options.md) ★★ *promote* | `R8` `R15` | **⚖ ALT** (to the degrader modality — see [§10.1b](#101b--the-family--what-picking-one-costs-you)) | ○ | **—** | **$0 CPU** | the 30-of-30 chemoselectivity closure **survives** without the E3 arm ⇒ the counter-result was never an artefact of the E3 constraint and the inhibitor configuration buys nothing. ⚠ Run it **after** [row 5](#101--open-rows-ordered-by-what-unblocks-the-most) or there is no baseline |
| **Q11** | **Resolve the PPARG agonism-vs-antagonism direction from the published literature, in CI** | [route 5](program/target-route-options.md) ★ | — (a route gate) | **⇢ PRE** (it unblocks the highest-readiness route that REMOVES the selectivity requirement) | ✅ **DONE 2026-08-06** | **—** | **$0 CI** | ⭐ **RAN AND ANSWERED** — [`pparg-direction-emc.md`](repurposing/pparg-direction-emc.md): **UNRESOLVED, leaning AGONISM, tier T1 with a model-identity caveat.** The literature does not settle a direction, and the *reason* is the finding: **two primary studies answered it in OPPOSITE directions from the same observation** (Subramanian 2005 → inhibitors; Filion 2009 → agonists), and the one functional test favouring agonism ran in H-EMC-SS (`OBJ-LINE-HEMCSS`, identity disputed). ⛔ **The agonist-redundancy premise is WITHDRAWN — it is not in the source it was cited from, and Filion et al. argue the opposite in their own discussion** (`EV-FILION-2009.misattributed_as`). What remains missing is a PPARγ **ACTIVITY** readout in EMC — abundance is already measured twice, concordantly — and that is `BLK-NO-EMC-DATA`, not a literature gap. ⚠ *Superseded, retained: "the downstream-node route stays parked on a literature question … because nobody has spent an hour on it." The hour was spent.* |
| **Q12** | **Verify the TCIP citation through `verify-refs`, then grade the mechanism against the failure record** | [route 6](program/target-route-options.md) ★ | `R4` `R5` `R7` `R9` `R10` (retires `R12`) | **⚖ ALT** + **⇢ PRE** | ○ | **—** | **$0 CI** | ⛔ **the citation is an auto-captured lead and has never cleared `verify-refs`** — until it does, no manuscript may quote it, so the verification is a hard gate on the route existing at all |
| **Q13** | ⛔ **Regenerate the junction neoepitope set against the CORRECTED exon index** — a **correction owed**, not an option | [route 7](program/target-route-options.md) | `R13` | — (data-integrity repair) | ○ | **—** | **$0 CI** (MHCflurry) | the seams are already **RETRACTED at source** (`⛔_RETRACTED_SEAMS` in [`fusion-breakpoint-neoantigens.json`](../modalities/fusion-breakpoint-neoantigens.json), *"DO NOT QUOTE ANY PEPTIDE, BINDER OR AFFINITY"*). Retracted is **not** repaired: the binders are withdrawn and no replacement set exists |
| **Q14** | **Re-query DepMap for CRISPR data on `ACH-001519` / H-EMC-SS** | [route 9](program/target-route-options.md) ⏸ | — (a route gate) | **⇢ PRE** | ○ | **—** | **$0 CI** | ⛔ **CLOSED 2026-08-05 — the question is answered AND its premise is withdrawn.** Still no CRISPR data since 24Q4 (a measured absence, not an unasked question), **and** the line is recorded as not carrying an EWSR1 fusion, so an answer would not be an EMC datum ([`emc-surface-target-landscape.md` → Amendment 1](surface-targets/emc-surface-target-landscape.md); `emc-atr-vulnerability.json` → `part_a_hemcss_identity`). Route 9 now waits on a **patient-derived** model. *(Superseded, retained: "still no CRISPR data since 24Q4 ⇒ the synthetic-lethal route stays parked **on data, not on ideas**".)* |
| **Q15** | **Add a `method-watch.md` trigger for prospective molecular-glue design** | [route 10](program/target-route-options.md) ⏸ | — | **⇢ PRE** | ○ | **—** | **$0** | none — it is a trigger. Its purpose is that the modality **most likely to arrive from outside** stops being re-litigated from scratch each time it is raised |
| **Q16** | **Restate the design brief ASYMMETRICALLY, in its harder measured form** — hard vs NR4A1, hard-but-lower-priority vs NR4A2, **both molecular** | [route 1](program/target-route-options.md) ★★ *adopt now* | `R7` ([§2.4](#24--the-selectivity-requirement-is-asymmetric--and-this-page-stated-it-symmetrically)) | **⇢ PRE** (it re-weights every selectivity row) | ○ | **—** | **$0** | no result can invalidate a brief. ⛔ What **today** removed is its easy half: [row 26](#101--open-rows-ordered-by-what-unblocks-the-most) bounded NR4A2 **and closed the exposure lever**, so *"treat the residual as an exposure question"* is no longer available. Reopening it needs single-cell or region-resolved expression — bulk averages dilute a small nucleus |
| **Q17** | **Bring the decoy null's primary output into a COMMITTED artifact** — it exists only in S3 | [framings](program/paper-framing-options.md) `P1` `P5` | `V20` | **⇢ PRE** (for every framing, and for the paper as written) | ○ | **—** | **$0** | ⛔ it is the audit's own rule *persist the primary artifact*, failed by the audit's own headline result. Two more instances landed the same day — the anti-target ΔG column and rung `5b-T`'s inputs — which is what makes it a pattern rather than an oversight |
| **Q18** | **One INSTRUMENT-CENSUS artifact** holding every instrument with its test, its result and its scope | [framings](program/paper-framing-options.md) `P1` `P7` | all `V*` | **⇢ PRE** | ○ | **—** | **$0** | it is a read over [§3.1](#31--the-instrument-table); it cannot fail. Its value is that **`P1` and `P7` are unwritable without it** and it costs nothing under any framing |
| **Q19** | **Put the `V17` disclosure into the manuscript, and fix the SI heading** | [framings](program/paper-framing-options.md) `P3` | `R8` | **⇢ PRE** | ○ | **—** | **$0** | a disclosure the roadmap already mandates and the manuscript does not carry. ⚠ One of the framing register's four must-fix items is already **done** (`nr4a-resistance-map.json` is on `main`); these are the remainder |
...
|---|---|---|
| ⭑ **WHAT THE PAPER IS** — the clearest case in the program, and **it is not four things to do, it is four things to BE** | `P1` the known-answer audit · `P3` the target-enablement dossier · `P2` the co-folding-assembly paper · `P6` the candidate paper (the current plan) — plus `P4` `P5` `P7`, all seven in [`paper-framing-options.md`](program/paper-framing-options.md) | **Each excludes the others as a TITLE.** Choosing `P1` retires the candidate title and keeps every result; choosing `P6` keeps the title and inherits `R4`, the one row on this board that **cannot be bought at all**. ⛔ **THIS PAGE DOES NOT DECIDE IT** — it is trimcrae's, it is recorded at [§13](#13--the-deliverables-framing--an-open-question-with-a-register-and-no-decision), and **no `Q` row waits on it**: `Q17`–`Q19` are prerequisites for whichever one is chosen, and the mechanism work strengthens all of them |
| **THE MODALITY at C397** | degrader (the current plan, rung `5b-T` and the ternary tail) **vs** `Q10` covalent inhibitor / probe **vs** `Q12` TCIP | Picking the **inhibitor** retires the ternary/ubiquitin stack (`R9` `R10` `R12`) and **one fewer terminus to satisfy** — and loses the degradation mechanism, which is the program's stated reason for choosing degradation over inhibition. Picking **TCIP** keeps `R4` `R5` `R7` `R9` `R10` and retires **only `R12`** — a TCIP is bivalent and still induces a ternary, so it retires the ubiquitin-transfer half and nothing else. ⚠ *Superseded, retained: "keeps `R4` `R5` `R7` and retires the same three." R9 is "OUR ternary is correctly assembled" and R10 is "A ternary forms"; a molecule that bridges the target and an effector forms one, so only R12 — "compatible with DEGRADATION — productive unique-lysine geometry" — is retired by not degrading. The graph already encoded the correct version (`RT-TCIP.blockers_retired` is `[BLK-TERNARY-GEOMETRY]` alone while `BLK-INDUCED-COMPLEX` is INHERITED), so this page and the model had been disagreeing; corrected 2026-08-06 from `nr4a3-tcip-reach.json` → `map_edits_required`.* ⚠ These are **cheap to evaluate and expensive to switch**: `Q10` and `Q12` are $0 *studies* of the alternative, not commitments to it |
...
|---|---|---|
| [**row 4**](#101--open-rows-ordered-by-what-unblocks-the-most) — a second, independent pose method | `Q1` `Q2` `Q5`, rung `5b-T`'s site 1 (which its `NO-GO` is still conditional on), and **every pose-conditional claim in the paper** | ⛔ **the largest prerequisite on the board.** ⚠ **THE PREREQUISITE HAS BEEN MET AND THE ROW SURVIVES IN A CHANGED FORM.** *Superseded, retained: "`cross_method_evidence` is **NONE** — every pose the program holds is one method's top pose, so the 6-pose disagreement cannot currently be attributed to anything."* A second method was run (rDock) and `cross_method_evidence` is now **PRESENT** — and it **DISAGREES**: 0 of 6 systems agree inside the 2.0 Å recovery line, median 6.696 Å. ⛔ So the disagreement is no longer *unattributable*; it is attributed, and the attribution is that neither engine produces a singular pose on this fold. ⚠ Read with its other half: the second method brought no known-answer calibration of its own, so this is two uncalibrated methods disagreeing and not a demonstration that either is wrong |
...
| **Owned by a different deliverable** | route 3, the fusion-junction ASO / siRNA | it is **already priority paper 2** in [emc-treatment-strategy.md](program/emc-treatment-strategy.md) and [IDEAS.md](../IDEAS.md); its gate is delivery, not selectivity. ⛔ **RETRACTED 2026-08-06 — *superseded, retained:* "✅ The exon audit **corroborated** it."** The audit **retracted** that lane's committed design panel (seam at NR4A3 residue **361** against a corrected resume range of **[1, 1]**), and a second defect found the same day means the repaired code could not rebuild the two junctions the paper leads with either. Regeneration is dispatched and **unread**; nothing from that panel may be quoted. ✅ **CLOSED 2026-08-06 — the regeneration LANDED and the retraction is lifted for the two panels that exist (E7::N3, E12::N3), which are quotable; *superseded, retained:* 'dispatched and **unread**'.** It refuted rather than restored the headline: `n_oligos_no_true_cleavage_risk = 0` and `n_candidates_zero_offtarget = 0` at both. ⭐ **And the obvious escape hatch from that refutation is now graded and closed too (2026-08-07):** the hybrid intron adds kilobases of target but **no additional fusion-unique window** — its bulk is wild-type pre-mRNA in the same nuclear compartment, so the budget stays `GAP − 1` and the seam becomes per-patient. [`hybrid-intron-aso-target.md`](aso/hybrid-intron-aso-target.md) → [`hybrid-intron-model.json`](../modalities/hybrid-intron-model.json). See X14 and [§WHAT THE LANDED RESULTS CHANGE](#-what-the-landed-results-change-about-the-remaining-plan). ⚠ **This row is why a retraction must sweep its copies:** the narrowing reached the paragraph in §12 and left this cell asserting the opposite for a day |
...
2. **The specific defect is one this program can fix for free and cannot fix any other way.**
   ...
   ✅ **THAT OBSERVATION HAS NOW BEEN TAKEN, and it did not resolve the fork — it moved it.** The second engine agrees with the first on 0 of 6 systems inside the 2.0 Å line, and the cavity-attribution follow-up finds the two engines in the SAME sub-cavity on 4 of 5 gradeable systems and in DIFFERENT ones on 1. ⛔ So *"the method is uncertain"* and *"the site is wrong"* are **both** live, and the reading that the disagreement is orientation-only does not hold across the census.
...
- ⛔ **And the negative this section carried for twenty minutes, kept because it was OVERTAKEN rather than
  ...
  **not** change what to start; it removes `5b-T` from the queue and **strengthens** the case for row 4,
  because the largest $0 item on the board has now been spent on a pose the program cannot yet call singular. ⭑ **AND THE REASON HAS CHANGED WHILE THE VERDICT HAS NOT.** *"Cannot YET call singular"* read as a gap awaiting a second opinion; the second opinion exists and disagrees, so site 1 is one member of a measured non-convergent ensemble and the rung's `NO-GO` is conditional on that member. ⛔ Nothing about the `NO-GO` is softened — arm C still failed on geometry registered at risk in advance and arm B still returned zero passing columns. What changes is that re-reading the rung on another pose is now a named, priced consequence rather than a hypothetical: it is the rung's own cost, once per committed pose

---

### 10.2 · The readout — derived from the column, not typed

- **29 rows. 7 are RESOLVED (1, 3, 5, 6, 18, 24, 29), 2 are ◐ partly in work (9, 10), and 20 are ○.** ⚠ Count RE-DERIVED from §10.1's leading state glyph on 2026-08-03, not typed — and re-derived **again** at 8:29 AM ET when rung `5b-T` landed, which is why the routed edit that moved row 1 carried `proposed_text: null` and the flag *"DERIVED COUNT — do not hand-edit"*. ⚠ **Superseded, retained:** *"5 are RESOLVED (3, 5, 6, 24, 29) … and 22 are ○"* — **rows 1 and 18 both resolved together**, because row 18 *is* the second half of row 1's rung. ⚠ **And row 4 is ○, not ✓**: its cell reads *"○ (the test ✓ ran, INCONCLUSIVE)"*, so a scan keying on any ✓ in the cell miscounts it — the glyph that binds is the **leading** one. **THREE of the seven resolved AGAINST the program** — row 3 (the submission gate FAILS), row 1 (rung `5b-T`'s pre-registered three-arm gate returns **`NO-GO`**: arm A PASS, arm B **INDETERMINATE** with **zero** passing columns, arm C **FAIL under both reach conventions**) and row 29's caveat (the decoy background does not score the headline residue) — which is the shape of the day and is why the ✓ count rising is not good news by itself. ⚠ *Superseded, retained: "Three of the five resolved AGAINST the program"*, and **"FOUR of the seven resolved AGAINST the program"**, which then listed **row 10** — a row this same bullet counts as ◐ **partly in work**, not as resolved. A count and its own examples must draw from one set. ⛔ **Row 10 is a fourth item resolving against the program and it is NOT in the resolved seven:** its `R14-a` half FAILS (the anti-target panel's self-control) and its `R14-a2` repair has RUN and not fixed it. ⭑ **Row 1 is the sharpest instance**: it was the program's largest open gap, it cost $0, its harness positive controls both PASS so the run is interpretable — and it still returned NO-GO, on **geometry the corridor conflict already predicted** rather than on anything the assembly route got wrong.
  ⛔ **Neither ◐ is a billing job, and [invariant 5](#05--six-invariants--structural-not-stylistic) requires each to NAME what is in work — [§0.2](#02--work-state--the-five-glyphs) records seven wrong ◐ glyphs on this page, so the naming is the guard and not a courtesy.** Row 9 is ◐ because `R13-a` ran and `R13-b` has not; row 10's ◐ **no longer names anything in work** — ⚠ *superseded, retained: "row 10 is ◐ for `R14-a2`, the anti-target receptor-preparation repair, on free CPU", a string §10.1 already registers as superseded and which this same section contradicts 76 lines below.* `R14-a2` RAN (2026-08-03, run `30809217139`) and did **not** restore readability: `panel_readable: false` still, CYP3A4 8.653 → 12.337 with `HEM` kept. That is exactly what ◐ is for — *don't start a second copy* — and it is the whole of what it claims: **nothing is billing**, and four independent $0 reads say so.
...
| items on **no list at all**, because a landed result was never wired into any layer of this page (2026-08-03) | — | ⛔ **1 added, and it is a different failure from every row above it: row 29.** The cross-system decoy null **ran, landed and was committed**, and this page cited it **nowhere** — it existed only in [`path-family-synthesis.md`](program/path-family-synthesis.md), one indirection from the source of truth. ★ **That is not "a caveat with nowhere to go" and not "nobody enumerated it" — it is a RESULT the plan did not know about**, which is the drift this page was built to prevent, reproduced inside this page's own week. Five further results landed the same day and are now wired into the layer that owns each (§2.1, §3.1, §3.2, §3.4, §5, §6a, §6b, §8, §10.1) rather than added as rows |
...
2. **The paper's Tier-3 row says the causal matched-pair test is *"priced, **not run**"* / "pending"**
   (`:2892–2899`) while §2.10e reports its result (`S = −0.1297 ± 0.3264`) and `:2851` says it *"retires the
...
3. **The paper contradicts itself on how far the co-folding DockQ failure extends.** §2.12a confines it to
   *"this co-folding pipeline on a **VHL** neosubstrate interface and about nothing else"* (`:2238–2240`),
   while §4 (`:2201–2203`) applies it to structures that are a **CRBN** ternary. A reader arriving via §2.12a
   and one arriving via §4 get opposite answers, and `R10`/`R11` inherit whichever is chosen.
4. **SI `:264` — *"Lead — NR4A3-selective (the validated path)"*** is the strongest residual over-claim in
   either file: it heads the indication table and pairs the R1 term with "validated" in four words, where
   `:2593` says *"**Every paralogue-selectivity statement in this work is therefore an unvalidated
   prediction.**"*
...
9. ✅ **CLOSED BY THIS MERGE. STRATEGY.md's banner said *"three selectivity results came to be withdrawn"***;
   ...
   search. ⚠ The audit record [`map-audit-strategy.md`](program/map-audit-strategy.md) A18 still quotes it as a live
...
24. **[`map-merge-inventory.md`](program/map-merge-inventory.md) counts Appendix A at *"76 rows"*.** Read directly, it
...
23. ⛔ **THE REPO HELD TWO INCOMPATIBLE MODELS OF THE FUSION PROTEIN, AND ONE OF THEM WAS AN OFF-BY-TWO.**
    ...
    - ⛔ **The consequence the neoantigen lane owes:** all 7 committed junctions delete NR4A3's AF1 **and the
      first zinc finger**, so its **26 predicted binders span seams that do not exist**. ✅ **CLEARED 2026-09-08 — this sub-bullet is stale and is closed here, its original text kept above.** The breakpoint artifact was regenerated on the corrected transcript model on 2026-08-19 and re-derives: `fusion-breakpoint-neoantigens.json` now carries `plausible_nr4a3_resume_range [1, 1]`, **5** in-frame junctions all with `nr4a3_first_residue 1` and `nr4a3_cds_nt_at_resume 0`, and `n_distinct_binders 11` — no junction resumes at a retracted residue (318 / 361 / 419) and none sits at CDS nt 1081. `fusion-neoantigen-retraction.json` grades the artifact **CLEARED**. The neoantigen manuscript quotes **11** binders and **0** occurrences of the 26-binder figure, retiring it explicitly in its superseded-figures appendix. Verified by the parent against the committed artifacts, not inferred.
    ...
    - ✅ **What it does to this page, and it is favourable in both directions.** The model flagged as an
      ...
      **brackets** the right answer, so what looked like conservatism was load-bearing.

      ⛔ **NARROWED 2026-08-06 — THIS WAS TRUE OF ONE MODULE, NOT OF THE LANE.** *Superseded, retained:* the unqualified *"the ASO lane is unaffected"*. The claim rested on `junction_breakpoint_scan.py` deliberately refusing the exon→CDS mapping — true of **that script**. `junction_aso.py`'s `FUSION_JUNCTION_MODE=real` path DID do the mapping, with the defective arithmetic, and produced the committed design panel the paper leads with: seam at NR4A3 CDS nt 1081, resuming at residue **361**, against a corrected range of **[1, 1]**. Code fixed and the six artifacts bannered 2026-08-06; regeneration needs Ensembl and must run in CI. See [`systems/AUDIT-2026-08-06-routes.md`](../../systems/AUDIT-2026-08-06-routes.md). ⛔ **AND THE 2026-08-06 FIX WAS ITSELF INSUFFICIENT — a SECOND defect, found the same day.** It concatenated CDS to CDS, resuming NR4A3 at its ATG and discarding the 5′UTR that transcript exon 3 carries; a fusion transcript retains the acceptor exon whole, and those bases sit immediately 3′ of the seam an ASO hybridises to. Under that model only **e11** closes the frame, so the "fixed" code would have refused **e7n3 and e12n3 — the two junctions the paper leads with**. The corrected model is mRNA-level and the frame closes on `(cut + U) mod 3 == 0`, where `U` is the acceptor-exon 5′UTR length **retained in the fusion — a number no artifact in this repo records**, which is what the CI Ensembl fetch measures. ⚠ *Superseded, retained: 'a number no artifact in this repo records'.* It was true of `nr4a3-exon-audit.json` and false of the repo — [`emc-construct-inputs.json`](../modalities/emc-construct-inputs.json) carries the spliced cDNA and `utr5_len` for both genes, and `U` is a subtraction from them. Re-derived offline, with no network call, twice: by the 2026-08-06 regeneration and again by [`hybrid_intron.coordinate_convention`](../modalities/hybrid_intron.py) on 2026-08-07. Until it lands, the corrected seam, GC values and off-target load are **UNKNOWN, not "expected to be similar."** And **`R13`'s object
...
22. ✅ **"What do I do next" needed two documents, and the first attempt at fixing that was a RELABELLING.**
    ...
    [`map-merge-inventory.md`](program/map-merge-inventory.md) classified **~2,430** of those lines as live plan
...
names a **single deliverable** — [the paper](degrader/nr4a3-degrader-paper.md) + its SI — and this page has never
