...
the CA and the side-chain centroid, and the placement loop increments per probe. A threshold of 12
probes can therefore be satisfied by between six and twelve distinct residues, and no single
residue-count conversion exists. Second, in a paired enumeration from fixed anchors, changing only
...
paired ratio interval requires the joint acceptance counts, and no cross-arm joint counts are
retained, so the joint covariance is missing and no design-aware interval can be computed from what
exists. The counts are also conditional on heterogeneous fixed anchor strata rather than on
biologically sampled units, so a pooled-binomial interval would not quantify uncertainty across
...
The committed value of 12 is therefore **12 probes, which between six and twelve distinct residues
could supply**. Six is the minimum number of residues that could supply exactly 12 probes; it is not
a maximum, and a threshold on probes does not impose any threshold on residues. The earlier version's
reading — "12 points, as few as 6 residues" combined with "a floor of at most 6 residues' worth of
probes" and "roughly half either way" — reversed that inequality and is withdrawn. For completeness:
the retained 4 distinct residues per side divided by six is 2/3, and divided by twelve is 1/3; there
is no single residue conversion, and the paper does not attempt one. A probe count is also not a
...
4. **The four-body design identifies no controlling variable** and neither establishes nor refutes an
   effect of body size (§4a). Arms share random streams; joint acceptance counts are not retained, so
   no design-aware uncertainty for the ratio can be computed.
