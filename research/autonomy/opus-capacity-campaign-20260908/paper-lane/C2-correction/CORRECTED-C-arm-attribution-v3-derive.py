"""CORRECTED-C v3 (arm attribution) -- NEW EXPLICITLY VERSIONED SIBLING producer.

v3 is the finite root-admitted C2 correction of v2. It does NOT overwrite v2, v1 or Job 3, and it
re-derives NO new clinical fact: the inputs, the cache, the leaves, the row-selection rule and the
552-row key set are identical. What changes is one arithmetic complement, one schema violation,
one branch order, the honesty of two evidence descriptions, and the machine VOCABULARY used to
name how strong the evidence actually is.

THE FIVE CORRECTIONS (root memo C2-Integration2-and-PAB2-root-adjudication-20260908.md)
--------------------------------------------------------------------------------------
 1. COUNT COMPLEMENT. v2's `not_source_confirmed_rows` was a hand-listed subset of four states
    (PROPOSED, UNRESOLVED, CONTESTED, UNKNOWN) that silently omitted the 132 LABEL_MATCH rows, so
    it reported 362 where the complement of the named state is 494. v3 computes every complement
    as total minus the named state (`partition_counts`) and TESTS that the partition sums. All 18
    of v2's saved checks passed with this error present. NO ROW IS PROMOTED to make a count agree.
 2. CONTESTED BINDING. v2 downgraded a bound arm on its own contrary registered description but
    LEFT `bound_arm_index` set, contradicting this table's own schema and check. v3 CLEARS the
    binding and preserves the candidate index, the candidate count, the original relation, the
    contrary description text and both locators in their own columns. No delivered row exercises
    this branch; it is checked by fixture.
 3. ZERO-ARM CONTRACT. v2 evaluated the comparator-text proposal branches BEFORE the zero-arm
    branch, so group text could have produced a comparator role in a record with no registered
    arms. v3 decides zero arms FIRST: link, registry type statement and comparator role are all
    UNKNOWN, and any comparator wording is preserved as unverified evidence in its own column.
    UNKNOWN does not mean no comparator existed. The six current zero-arm rows were already
    UNKNOWN; this closes the branch, it does not restate any row.
 4. EVIDENCE REPRESENTATION. The leaf narrative was described as "verbatim" while newlines were
    replaced, the text cut at 400 characters and whitespace normalised again at TSV write. It is
    now labelled a TRANSFORMED EXCERPT with its transform and original length recorded, and the
    original leaf locator retained. (No current narrative was observed at the cap.) The
    source-module-absence check said "12 payloads" while reading TEN files; the wording is
    corrected and the original result is kept as a ten-file scoped observation, not a newly
    verified twelve-file absence. No additional payload was audited.
 5. NO PROMOTION OF TEXT CORRESPONDENCE TO REGISTRY IDENTITY OR INDEPENDENCE. Agreeing label and
    description fields are two CORRELATED textual fields of one record, not two independent
    sources, and their agreement is not a registry assertion that this results group IS that arm.
    SOURCE_CONFIRMED becomes SOURCE_FIELD_MATCH (58 rows) and TWO_INDEPENDENT_FIELD_IDENTITIES
    becomes TWO_AGREEING_FIELD_MATCHES. A comparator TYPE read through such a relationship is
    COMPARATOR_TYPE_INFERRED_VIA_SOURCE_FIELD_MATCH and carries its unproved identity assumption
    in machine state: the eight such rows are NOT eight independently clinically validated
    comparators. The 132 label-only rows and the 58 description-supported rows remain DIFFERENT
    evidence strengths and are never summed into one "confirmed" figure. Every row carries
    `source_join_verification` = NOT_ESTABLISHED: the registry's own explicit join fields are
    absent from this cache, so explicit source-join verification has not been established here.

This is an inference and label correction over existing observations. No new source adjudication,
no re-run of jobs 2/3, no new eligibility, rate, pooling or endpoint work, and no count quota.
The endpoint paper remains PARKED under its existing ten scientific findings.

The v2 text below is retained unchanged for provenance; read "v2" as "the version v3 corrects".
 ...
 * CURRENT, 2026-09-08 -- two assertions in the v2 rules below are no longer accurate of this
 ...
 2. Exact / case-and-whitespace LABEL correspondence is a DISTINCT state (LABEL_MATCH), not
    source-field-matched group-arm identity. SOURCE_FIELD_MATCH needs two independent exact field
...
# root item 5: no row in this table was verified by the registry's own explicit join fields
# (resultsSection.participantFlowModule / interventions[].armGroupLabels). Those fields are absent
# from this cache. Every state here is a computed correspondence between free-text source fields.
SOURCE_JOIN_VERIFICATION = "NOT_ESTABLISHED_REGISTRY_JOIN_FIELDS_ABSENT_FROM_THIS_CACHE"
...
def decide_link(gtitle, gdesc, armgroups, leafrec):
    ...
    out = dict(arm_link_state="", arm_link_relation="", arm_link_source="", arm_link_locator="",
               ...
               leaf_claim_verification_before_downgrade="",
               downgraded_candidate_arm_index="", contrary_description_locator="",
               contrary_description_evidence_excerpt="", contested_flags=[])
    n = len(armgroups)
    ...
    if lab_i is not None and des_i is not None and lab_i != des_i:
        out.update(arm_link_state="CONTESTED",
                   arm_link_relation="LABEL_FIELD_MATCH_AND_DESCRIPTION_FIELD_MATCH_DISAGREE",
        ...
        out["contested_flags"].append("LABEL_AND_DESCRIPTION_FIELD_MATCHES_NAME_DIFFERENT_ARMS")
    ...
    # ---- label correspondence ALONE: useful evidence, but not source-field-matched identity --------
    elif lab_i is not None:
        out.update(arm_link_state="LABEL_MATCH",
                   arm_link_relation="LABEL_FIELD_MATCH_ONLY:%s" % lab_code,
                   arm_link_source="SOURCE_RECORD", bound_arm_index=lab_i, n_candidate_arms=1,
                   candidate_arm_indices=str(lab_i),
    ...
    # v3 correction (root item 2): v2 downgraded the state to CONTESTED but LEFT bound_arm_index
    # set, which violates this table's own schema and check ("an arm is bound only by
    # SOURCE_FIELD_MATCH or LABEL_MATCH"). The BINDING IS NOW CLEARED. Nothing is discarded: the
    # candidate index, the candidate count, the original relation, the contrary description text
    # and both field locators are all preserved in their own columns, and the disagreement itself
    # is the state. Clearing a binding is not a claim that the candidate is the wrong arm.
    if out["bound_arm_index"] != "":
        bi = out["bound_arm_index"]
        bd = armgroups[bi].get("description") or ""
        if NOT_ENROLLED_DESC.search(bd):
            ...
            out["arm_link_relation"] = ("CONTRARY_DESCRIPTION_BINDING_CLEARED:"
                                        + out["arm_link_relation"])
            out["downgraded_candidate_arm_index"] = str(bi)
            out["contrary_description_locator"] = "%s[%d].description" % (AG, bi)
            out["contrary_description_evidence_excerpt"] = bd.replace("\n", " ")[:400]
            out["bound_arm_index"] = ""
            out["candidate_arm_indices"] = str(bi)
            out["n_candidate_arms"] = 1
            if out["leaf_claim_verification"] in ("LEAF_CLAIM_AGREES_WITH_SOURCE_RELATION",
                                                  "LEAF_CLAIM_DISAGREES_WITH_SOURCE_RELATION"):
                out["leaf_claim_verification_before_downgrade"] = out["leaf_claim_verification"]
                out["leaf_claim_verification"] = (
...
def decide_comparator(link, gtitle, gdesc, armgroups, arm_set_type_state, types):
    ...
    linked = link["arm_link_state"] in ("SOURCE_FIELD_MATCH", "LABEL_MATCH") and bound != ""

    # v3 correction (root item 3): the comparator wording found in the RESULTS-GROUP text is
    # recorded in its OWN field as unverified evidence, whatever the arm registration says. It is
    # never lost, and it never becomes a comparator role on its own when there is nothing in the
    # registry to attach it to.
    wording = []
    if PLACEBO_TXT.search(gtitle or "") or PLACEBO_TXT.search(gdesc or ""):
        wording.append("GROUP_TEXT_PLACEBO_OR_SHAM_WORDING")
    if t_noi:
        wording.append("GROUP_TEXT_NO_INTERVENTION_WORDING")
    if t_ctl:
        wording.append("GROUP_TEXT_CONTROL_OR_COMPARATOR_WORDING")
    if seq:
    ...
    if len(armgroups) == 0:
        # root item 3: with zero registered arms the registry type statement is UNKNOWN, exactly as
        # the link and the role are. The reason is kept inside the value.
        stmt, assume = "UNKNOWN_NO_ARMS_REGISTERED_IN_THIS_CACHE", ""
    elif linked and btype:
        stmt = "BOUND_ARM_REGISTERED_TYPE_IS_%s" % btype
        assume = identity_assume
    elif linked and not btype:
        stmt, assume = "BOUND_ARM_TYPE_ABSENT_IN_THIS_CACHE", identity_assume
    ...
    # root item 3: zero registered arms is decided FIRST. In v2 the comparator-text proposal
    # branches preceded it, so a results group whose title said "placebo" in a record with no
    # registered arms could have been given a comparator role with nothing to attach it to,
    # contradicting the broad saved invariant. UNKNOWN here does not mean no comparator existed.
    if len(armgroups) == 0:
        role = "UNKNOWN_IN_THIS_CACHE"
        basis = ("zero registered arm groups: link, registry type statement and comparator role "
                 "are all UNKNOWN in this cache. Any comparator wording in the results-group text "
                 "is retained as unverified evidence in group_text_comparator_wording_unverified; "
                 "it is not evidence that no comparator existed")
    elif linked and btype in CONTROL_TYPES:
        if link["arm_link_state"] == "SOURCE_FIELD_MATCH":
            role = "COMPARATOR_TYPE_INFERRED_VIA_SOURCE_FIELD_MATCH"
            basis = ("arm %d bound by %s; registered type %s carried verbatim. The comparator TYPE "
                     "is INFERRED THROUGH a computed correspondence between two source text "
                     "fields; the registry does not state that this results group IS that arm, so "
                     "this is not an independently validated clinical comparator"
                     % (bound, link["arm_link_relation"], btype))
        else:
            role = "COMPARATOR_PROPOSED_BY_LABEL_FIELD_MATCHED_ARM_TYPE"
            basis = ("arm %d bound by label correspondence only (%s); registered type %s; the "
                     "group-arm identity is not independently source-field-matched"
                     % (bound, link["arm_link_relation"], btype))
        ...
        role, basis = ("COMPARATOR_PROPOSED_BY_GROUP_TEXT_PLACEBO",
                       "group record states placebo/sham; no source-field-matched registry link")
    elif t_noi:
        role, basis = ("COMPARATOR_PROPOSED_BY_GROUP_TEXT_NO_INTERVENTION",
                       "group record states no-intervention/observation; no source-field-matched link")
    elif t_ctl:
        ...
        role, basis = ("UNKNOWN_IN_THIS_CACHE",
                       "no source-field-matched arm link and no comparator wording in the group record")
    ...
    role_assume = identity_assume if role.startswith("COMPARATOR_TYPE_INFERRED") or role.startswith(
        "COMPARATOR_PROPOSED_BY_LABEL_FIELD_MATCHED_ARM_TYPE") else ""
    return role, basis, stmt, assume, btype, flags, role_assume, group_text_wording


# ================================================================================================
# root item 4: v2's column name and schema called this text "verbatim". It is not: newlines are
# replaced with a space, the value is cut at 400 characters, and the TSV writer normalises tabs and
# newlines again on the way out. The ORIGINAL text is not held here at all -- only the leaf locator
# below points at it. The label is corrected; the transform is recorded; the text is unchanged.
NARRATIVE_TRANSFORM = ("NEWLINE_TO_SPACE;CUT_AT_400_CHARS;TSV_WRITE_NORMALISES_TAB_AND_NEWLINE;"
    ...
    for path, ffield, vfield, lab, typ in LEAVES:
        ...
        with open(path, encoding="utf-8") as fh:
            for ln, r in enumerate(csv.DictReader(fh, delimiter="\t"), start=2):
                ...
                original = r.get(ffield) or ""
                excerpt = original.replace("\n", " ")[:400]
                rec = dict(leaf_source=src, leaf_row=ln, leaf_verdict=r.get(vfield, ""),
                           leaf_locator="LEAF-OUT/%s:line %d" % (src, ln),
                           leaf_narrative_excerpt=excerpt,
                           leaf_excerpt_original_chars=len(original),
                           leaf_excerpt_transform=NARRATIVE_TRANSFORM,
                           leaf_excerpt_at_400_char_cap=("YES" if len(original) > 400 else "NO"),
...
def partition_counts(states, matched_state="SOURCE_FIELD_MATCH"):
    """root item 1. v2 reported `not_source_confirmed_rows` as the size of FOUR named states
    (PROPOSED, UNRESOLVED, CONTESTED, UNKNOWN_IN_THIS_CACHE), silently dropping LABEL_MATCH, so a
    table of 552 rows with 58 matched rows reported a complement of 362 instead of 494.

    The complement of an explicitly named state is computed HERE as total minus that state, never
    as a hand-written list of the other states, and the partition is returned so a caller can test
    that it sums. No row is moved to make any count agree."""
    per = collections.Counter(states)
    total = sum(per.values())
    matched = per.get(matched_state, 0)
    complement_by_state = {k: v for k, v in sorted(per.items()) if k != matched_state}
    return dict(total_rows=total,
                matched_state=matched_state,
                rows_in_matched_state=matched,
                rows_not_in_matched_state=total - matched,
                rows_not_in_matched_state_by_state=complement_by_state,
                partition_sums=(matched + (total - matched) == total
                                and sum(complement_by_state.values()) == total - matched))


def v3_class(state, role):
    """Four comparable classes, for the change map only. Never written back onto a row.

    v2's class token COMPARATOR_SUPPORTED is renamed COMPARATOR_TYPE_INFERRED on BOTH sides of the
    comparison (here and in `v1_class`), so the partition is unchanged and only the word changes.
    "Supported" overstated what the rule does: a registered arm TYPE read through a matched
    group-arm relationship whose identity is assumed, not proved."""
    if role == "CONTESTED" or state == "CONTESTED":
        return "CONTESTED"
    if role.startswith("COMPARATOR_TYPE_INFERRED"):
        return "COMPARATOR_TYPE_INFERRED"
...
def main(out_tsv, out_checks):
        ...
        for s in payload(name).get("studies") or []:
            ...
            for om in (rs.get("outcomeMeasuresModule") or {}).get("outcomeMeasures") or []:
                ...
                for gid, cells in cells_for_groups(om).items():
                    ...
                    (role, basis, stmt, assume, btype, flags, role_assume,
                     group_text_wording) = decide_comparator(
                        link, gtitle, gdesc, ai, arm_set_type_state, types)
                    b = link["bound_arm_index"]
                    dc = link["downgraded_candidate_arm_index"]
                    cand_i = [int(x) for x in link["candidate_arm_indices"].split("|") if x != ""]

                    p1 = v1.get(k3) or {}
                    p3 = job3.get(k3) or {}
                    a, bcl = v1_class(p1.get("control_status", "")), v3_class(link["arm_link_state"], role)
                    ...
                    rows.append(dict(
                        ...
                        downgraded_candidate_arm_index=dc,
                        downgraded_candidate_arm_label=(
                            "" if dc == "" else (ai[int(dc)].get("label") or "")),
                        downgraded_candidate_registered_type_verbatim=(
                            "" if dc == "" else
                            (ai[int(dc)].get("type") or "ARM_TYPE_ABSENT_IN_THIS_CACHE")),
                        contrary_description_locator=link["contrary_description_locator"],
                        contrary_description_evidence_excerpt=(
                            link["contrary_description_evidence_excerpt"]),
                        comparator_role=role, comparator_role_basis=basis,
                        comparator_role_identity_assumption=role_assume,
                        group_text_comparator_wording_unverified=group_text_wording,
                        source_join_verification=SOURCE_JOIN_VERIFICATION,
                        ...
                        leaf_claim_verification_before_downgrade=(
                            link["leaf_claim_verification_before_downgrade"]),
                        leaf_narrative_transformed_excerpt_not_parsed=(
                            lf["leaf_narrative_excerpt"] if lf else ""),
                        leaf_excerpt_transform=(lf["leaf_excerpt_transform"] if lf else ""),
                        leaf_excerpt_original_chars=(
                            lf["leaf_excerpt_original_chars"] if lf else ""),
                        leaf_excerpt_at_400_char_cap=(
                            lf["leaf_excerpt_at_400_char_cap"] if lf else ""),
                        ...
                        group_description_transformed_excerpt=gdesc.replace("\n", " ")[:400],
                        group_description_transform=NARRATIVE_TRANSFORM.replace(
                            "leaf_claim_locator", "the cached source record"),
                        group_description_original_chars=len(gdesc)))
    ...
    check("no_narrative_token_corroboration_predicate_remains",
          ...
          "tests the leaf narrative. v3 correction: it is carried as a TRANSFORMED EXCERPT, not "
          "verbatim -- newlines become spaces, the text is cut at 400 characters and the TSV "
          "writer normalises tabs and newlines again. suspect_lines=%d" % len(narrative_tested))
    ...
    bad = [r for r in rows if r["arm_link_state"] not in ("SOURCE_FIELD_MATCH", "LABEL_MATCH")
           and r["bound_arm_index"] != ""]
    check("an_arm_is_bound_only_by_SOURCE_FIELD_MATCH_or_LABEL_MATCH", not bad,
          "violations=%d" % len(bad))
    ...
    bad = [r for r in rows if r["arm_link_state"] == "SOURCE_FIELD_MATCH"
           and not (r["arm_link_relation"].startswith("TWO_AGREEING_FIELD_MATCHES")
                    or r["arm_link_relation"].startswith("DESCRIPTION_FIELD_MATCH_ONLY"))]
    check("SOURCE_FIELD_MATCH_only_from_an_exact_field_equality", not bad, "violations=%d" % len(bad))
    ...
    check("absent_registry_join_modules_stay_absent_in_the_ten_payload_files_read_here",
          absent["participantFlowModule"] == 0 and absent["armGroupLabels"] == 0,
          "occurrences across the %d cached payload files this check actually reads (FILES = 5 BOR "
          "eras + 5 placebo eras): %s (0 expected; never fetched). v2 described the same loop as "
          "'12 payloads', which was wrong about its own scope; the observation is unchanged and "
          "stays scoped to these ten files. It is NOT a verified twelve-file absence, and no "
          "additional payload was audited to make this correction."
          % (len(FILES), dict(absent)))
    check("the_payload_scope_named_in_the_absence_check_equals_the_files_actually_read",
          len(FILES) == 10 and len(set(FILES)) == 10,
          "FILES=%d distinct=%d: %s" % (len(FILES), len(set(FILES)), FILES))

    # ---------------- root item 1: the complement of an explicitly named state ----------------
    part = partition_counts([r["arm_link_state"] for r in rows])
    check("state_counts_partition_every_row_exactly_once",
          part["total_rows"] == len(rows) and part["partition_sums"],
          "total=%d matched(%s)=%d complement=%d complement_by_state=%s"
          % (part["total_rows"], part["matched_state"], part["rows_in_matched_state"],
             part["rows_not_in_matched_state"], part["rows_not_in_matched_state_by_state"]))
    check("the_complement_is_total_minus_the_named_state_not_a_hand_listed_subset",
          part["rows_in_matched_state"] + part["rows_not_in_matched_state"] == len(rows)
          and part["rows_not_in_matched_state"]
          == sum(part["rows_not_in_matched_state_by_state"].values()),
          "%d + %d == %d; v2 reported this complement as %d because LABEL_MATCH (%d rows) was "
          "excluded from a hand-listed four-state subset. No row was promoted or moved."
          % (part["rows_in_matched_state"], part["rows_not_in_matched_state"], len(rows),
             sum(1 for r in rows if r["arm_link_state"] in
                 ("PROPOSED", "UNRESOLVED", "CONTESTED", "UNKNOWN_IN_THIS_CACHE")),
             sum(1 for r in rows if r["arm_link_state"] == "LABEL_MATCH")))
    check("label_only_and_description_supported_matches_stay_separate_strengths",
          "LABEL_MATCH" in part["rows_not_in_matched_state_by_state"]
          and part["matched_state"] == "SOURCE_FIELD_MATCH",
          "LABEL_MATCH rows=%d are NOT counted as SOURCE_FIELD_MATCH rows=%d; they are different "
          "evidence strengths and are never summed into one 'confirmed' figure"
          % (part["rows_not_in_matched_state_by_state"].get("LABEL_MATCH", 0),
             part["rows_in_matched_state"]))
    # ---------------- root item 2: a downgraded binding is cleared, not retained ----------------
    bad = [r for r in rows if r["arm_link_state"] == "CONTESTED" and r["bound_arm_index"] != ""]
    check("no_CONTESTED_row_retains_a_bound_arm_index", not bad, "violations=%d" % len(bad))
    bad = [r for r in rows if r["downgraded_candidate_arm_index"] != ""
           and not (r["arm_link_state"] == "CONTESTED" and r["contrary_description_locator"]
                    and r["contrary_description_evidence_excerpt"]
                    and r["candidate_arm_indices"] == r["downgraded_candidate_arm_index"])]
    check("a_cleared_binding_keeps_its_candidate_its_evidence_and_its_locator", not bad,
          "violations=%d; rows exercising the contrary-description downgrade in this delivery=%d "
          "(the branch is checked by fixture, not by a delivered row)"
          % (len(bad), sum(1 for r in rows if r["downgraded_candidate_arm_index"] != "")))
    # ---------------- root item 3: zero registered arms ----------------
    bad = [r for r in rows if r["n_arms_registered"] == 0
           and not (r["arm_link_state"] == "UNKNOWN_IN_THIS_CACHE"
                    and r["comparator_role"] == "UNKNOWN_IN_THIS_CACHE"
                    and r["registry_type_statement"].startswith("UNKNOWN_"))]
    check("zero_registered_arms_makes_link_type_and_role_all_UNKNOWN", not bad,
          "violations=%d; zero-arm rows=%d (UNKNOWN means not established in this cache, NOT that "
          "no comparator existed)"
          % (len(bad), sum(1 for r in rows if r["n_arms_registered"] == 0)))
    bad = [r for r in rows if r["group_text_comparator_wording_unverified"]
           and r["n_arms_registered"] == 0 and r["comparator_role"] != "UNKNOWN_IN_THIS_CACHE"]
    check("zero_arm_group_text_wording_is_kept_as_evidence_and_never_becomes_a_role", not bad,
          "violations=%d; zero-arm rows carrying comparator wording=%d"
          % (len(bad), sum(1 for r in rows if r["n_arms_registered"] == 0
                           and r["group_text_comparator_wording_unverified"])))
    # ---------------- root item 4: evidence representation ----------------
    bad = [r for r in rows if r["leaf_narrative_transformed_excerpt_not_parsed"]
           and not r["leaf_excerpt_transform"]]
    check("every_carried_narrative_excerpt_names_the_transform_applied_to_it", not bad,
          "violations=%d; rows carrying an excerpt=%d; rows observed AT the 400-char cap=%d"
          % (len(bad), sum(1 for r in rows if r["leaf_narrative_transformed_excerpt_not_parsed"]),
             sum(1 for r in rows if r["leaf_excerpt_at_400_char_cap"] == "YES")))
    check("no_column_in_this_table_calls_a_transformed_excerpt_verbatim_text",
          not any("narrative" in c and "verbatim" in c for c in cols),
          "narrative columns: %s" % [c for c in cols if "narrative" in c])
    # ---------------- root item 5: strength of the evidence, in machine vocabulary ----------------
    bad = [r for r in rows if r["source_join_verification"] != SOURCE_JOIN_VERIFICATION]
    check("no_row_claims_explicit_registry_source_join_verification", not bad,
          "violations=%d; every row carries %s" % (len(bad), SOURCE_JOIN_VERIFICATION))
    bad = [r for r in rows if r["bound_arm_index"] != ""
           and not r["registry_type_statement_assumption"]]
    check("every_bound_row_names_its_unproved_identity_assumption", not bad,
          "violations=%d" % len(bad))
    bad = [r for r in rows if (r["comparator_role"].startswith("COMPARATOR_TYPE_INFERRED")
                               or r["comparator_role"].startswith(
                                   "COMPARATOR_PROPOSED_BY_LABEL_FIELD_MATCHED"))
           and not r["comparator_role_identity_assumption"]]
    check("comparator_type_inferred_through_a_field_match_carries_the_identity_assumption",
          not bad,
          "violations=%d; type-inferred-via-source-field-match rows=%d (these are NOT independently "
          "clinically validated comparators)"
          % (len(bad), sum(1 for r in rows if r["comparator_role"]
                           == "COMPARATOR_TYPE_INFERRED_VIA_SOURCE_FIELD_MATCH")))
    banned = ("SOURCE_CONFIRMED", "TWO_INDEPENDENT_FIELD_IDENTITIES", "COMPARATOR_SUPPORTED",
              "INDEPENDENTLY_CONFIRMED")
    hits = sorted({t for t in banned for r in rows for c in
                   ("arm_link_state", "arm_link_relation", "comparator_role",
                    "registry_type_statement") if t in r[c]})
    check("no_machine_value_claims_independent_confirmation_from_two_textual_fields", not hits,
          "banned tokens present=%s" % (hits or "none"))
    ...
    summary = dict(
        ...
        rows_in_a_source_field_match_state=part["rows_in_matched_state"],
        rows_not_in_a_source_field_match_state=part["rows_not_in_matched_state"],
        rows_not_in_a_source_field_match_state_by_state=part[
            "rows_not_in_matched_state_by_state"],
        rows_in_PROPOSED_UNRESOLVED_CONTESTED_or_UNKNOWN=unresolved,
        v2_not_source_confirmed_rows_withdrawn=dict(
            v2_reported=362, actual_complement_of_the_named_state=part["rows_not_in_matched_state"],
            cause="v2 excluded the LABEL_MATCH rows from a hand-listed four-state subset while "
                  "naming the field for the complement of one state",
            rows_moved_or_promoted_to_fix_it=0),
        rows_with_a_cleared_contrary_description_binding=sum(
            1 for r in rows if r["downgraded_candidate_arm_index"] != ""),
        rows_at_the_400_character_narrative_cap=sum(
            1 for r in rows if r["leaf_excerpt_at_400_char_cap"] == "YES"),
        payload_files_read_by_the_absence_check=len(FILES),
        source_join_verification=SOURCE_JOIN_VERIFICATION,
        comparator_role_identity_assumption=dict(collections.Counter(
            r["comparator_role_identity_assumption"] for r in rows)),
        group_text_comparator_wording_unverified=dict(collections.Counter(
            r["group_text_comparator_wording_unverified"] for r in rows)),
