...
`<scratchpad>/s33_pmid_title_coverage.py` — a scratchpad script that does not exist in this
repository (read-only, writes nothing, contents reproduced in §4)
...
Measured tonight with `<scratchpad>/s33_pmid_title_coverage.py` (not ours — a scratchpad file), which extracts every PMID from
...
11. `s33_pmid_title_coverage.py` (not ours — a scratchpad file) → 24/24 reference PMIDs resolve to a committed title; 0 missing.
