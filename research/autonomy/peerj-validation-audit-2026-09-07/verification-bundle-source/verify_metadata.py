...
EXPECTED_HASHES = {
    'GSE6481_family.soft.gz': 'eec1f676f7b05f3115d2b04c926e6085a429350ec6b84a1d637ff27ae8375887',
    'GSE24369_family.soft.gz': '98c83c8ca23b7052cf0d4d0099a7bf1af6c3c972276038c3a633e2a5349b3c37',
    'peerj-21497-article.xml': '2d9a6ee4ed417860867eb954e0b0a4fa3fa3042787f5af3c0a1fea9bb5fbe768',
    'peerj-21497-figure-s6.png': '29932259e722fa46b84f01e2b662bcd58b66a108db669eca5035913eb9d8df03',
}
EXPECTED_COUNTS = {
...
def verify_geo(accession, source_dir):
    path = source_dir/(accession+'_family.soft.gz')
    ...
    return {'accession':accession,'sample_count':len(rows),'diagnosis_counts':counts,
            ...
            'platform_accessions':sorted(platform_counts),'complete_sample_roster':[{k:r[k] for k in ('gsm','diagnosis','platform','source_line')} for r in rows],
...
def main():
    ...
    parser.add_argument('--output', default='results', help='Output directory relative to this supplement')
    ...
    manifest = json.loads((ROOT/'retrieval-sources.json').read_text(encoding='utf-8'))
    ...
    for relative, expected in EXPECTED_HASHES.items():
        blob = (source_dir/relative).read_bytes()
    ...
    results = {acc:verify_geo(acc, source_dir) for acc in EXPECTED_COUNTS}
    output = ROOT/rel
    ...
    summary = {'status':'passed','source_hashes_checked':EXPECTED_HASHES,
               ...
               'expression_values_parsed_or_computed':False,
               'scope':'Factual sample identifiers, diagnoses, platforms and audit counts; original source narrative excluded.'}
    write_json(output/'verification-summary.json',summary)
    write_json(output/'execution.json',{'status':'passed','exit_code':0,'command':'python -X utf8 -B verify_metadata.py --sources '+args.sources+' --output '+args.output,
