"""Render publication figures only after exact input/script bytes are committed.
...
def check(render=False):
    ...
    pinned=json.loads((HERE/'figure-inputs.json').read_text())
    paths=[PACKET/x for x in INPUTS]+[HERE/'plot_figures.py',HERE/'figure-inputs.json']
    state=[]
    for p in paths:
        key=p.relative_to(PACKET).as_posix()
...
def render(output):
    ...
    plt.rcParams.update({'font.family':'DejaVu Sans','font.size':9,'axes.titlesize':10,'axes.labelsize':9,'pdf.fonttype':42,'ps.fonttype':42,'savefig.dpi':300})
    ...
    titles=['A  LGFMS: same histology, separate cohorts','B  Hofvander: marginal','C  Hofvander: year matched']
    labels=[['Array\n6 EMC / 17 LGFMS','TPM marginal\n9 EMC / 13 LGFMS','TPM matched\n3 EMC'],['MLPS\nn=14','LGFMS\nn=13','Synovial\nn=18'],['MLPS\n3 EMC','LGFMS\n3 EMC','Synovial\n3 EMC']]
    ...
    for ext in ['pdf','png']:fig.savefig(output/('figure1-fixed-panel.'+ext))
    plt.close(fig)
    ...
    for ext in ['pdf','png']:fig.savefig(output/('figure2-cspg4-specimens-years.'+ext),bbox_inches='tight')
    plt.close(fig)
    record={'git_commit':subprocess.check_output(['git','rev-parse','HEAD'],cwd=HERE,text=True).strip(),'inputs':provenance,'matplotlib':matplotlib.__version__,'numpy':np.__version__,'figures':{p.name:sha(p) for p in output.glob('figure*')},'scope':'descriptive source values and completed estimates; no uncertainty bars in these figures'}
    (output/'figure-provenance.json').write_text(json.dumps(record,indent=2)+'\n')
if __name__=='__main__':
    a=argparse.ArgumentParser();a.add_argument('--render',action='store_true');a.add_argument('--output',type=Path,default=HERE/'figures');v=a.parse_args()
