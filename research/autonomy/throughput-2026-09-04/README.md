...
The user subsequently confirmed disabling the Claude driver. A read-only ownership audit found
415 research-ledger entries with no assigned owner and no live manuscript writer. Main continued
receiving monitor-state commits; those changes were reconciled separately from the paper files.

The first writing run produced the [three-file ASO submission support package](../../release-candidates/PUB-ASO/2026-09-04/submission/README.md)
in 320.562 seconds, but finalization rejected later journal requirements as task blockers. Its
[failed receipt](codex-writing-initial-receipt.json) and [original outcome](codex-writing-initial-outcome.json)
are preserved. The runner now separates actual task blockers from downstream `follow_up`, while
still refusing completion with real task blockers. It supplies execution limits and the Python
interpreter path to prevent guessed metadata and missing-command discovery work.

One [focused writing repair](codex-writing-completed-receipt.json) completed in 182.219 seconds
with one dispatch. It removed an unnecessary approval step before draft upload preparation and
corrected execution metadata. The coordinator read the result, verified all 25 inventory hashes
and confirmed the two-file edit scope. The repair used 170,313 input tokens, including 136,064
cached input, and 4,057 output tokens. These are observed usage totals, not an end-to-end speed
comparison or an account-capacity estimate. Updated runner tests passed: 15 in 9.196 seconds.

The `EMC research coordinator` thread heartbeat is enabled every two hours. Its first scheduled
run has not yet been observed; the real writing run above established the prerequisite. The local
machine and Codex app must be available. Next: generate and inspect explicitly unsubmitted Word
and anonymous upload variants from the frozen candidate; author approval applies to the concrete
final submission package. No manuscript was submitted. Current status is in [codex-handover.json](../codex-handover.json).
...
are listed above; no full scientific test suite has been run for this patch. The initial changes
were saved on `codex/research-throughput-2026-09-04`. The subsequent confirmed handover and real
writing run are described above; `codex-handover.json` records the current integration evidence.
No preprint or journal submission occurred.

The settled Codex handover normal preflight then **passed in 68.703 seconds**; its
[complete log](handover-preflight.log) and [snapshot, source hashes and scope](handover-preflight-result.json)
are preserved. The preceding 78.938-second attempt failed on two unsupported document audience
labels; those metadata fields and their inventory hashes were corrected. The [failed log](handover-preflight-failed.log)
remains available. This handover check is also normal document/artifact verification, not FULL
publication evidence. Existing runtime monitor updates were reconciled without rewriting paper data.
