...
All 42 relevant controls passed at commit `0044fb0fb296f154bc440986718848ad49195e7e`, followed
by successful normal preflight (fast document and artifact gates). Both actual exits are zero;
[validation receipt](VALIDATION.json) records the exact run and log digest. The earlier local
attempt could not start because Python lacked pytest and remains recorded as an environment
failure. Normal preflight is not FULL publication evidence. The ledger now records the measured
completion, preserving its original wording and zero old-fetch retry budget. Only these scoped
files are proposed for main, after verifying that its affected source blobs and input are still
identical to the original base. The precise integration receives separate checks before a
non-forced main push. No journal, preprint, source fetch or original scientific protocol changes.
