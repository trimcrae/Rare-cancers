...
| **Degrader — NR4A3-LBD PROTAC** | **LEADING driver-directed route; WARHEAD PLAN RE-GRADED 2026-06-25 — the orthosteric (warhead-target) pocket is ~0.026 (undruggable to small molecules), not 0.495 (that was a fixed fpocket off-by-one; 0.495 is a different AF-2-surface cavity). Small-molecule orthosteric warhead now gated entirely on the cryptic-pocket MD; de-novo protein binder + junction ASO promoted to co-leads. See `emc-treatment-strategy.md` reassessment + `modalities/ASSUMPTIONS.md`.** Premise still in-silico-supported. Degradation is *mechanistically ideal* (NOR-1 activity scales with expression level). **Fusion-addiction premise supported by analogy: FLI1 in Ewing = −0.93 gene effect, 74% dependent → FET-fusion sarcomas are fusion-addicted, so degrading EWSR1::NR4A3 should be lethal** (`depmap-insilico-findings.md`). NR4A3-specific warhead starting points exist (inverse NOR-1 agonists, Munck 2022); NR4A1 PROTAC works but doesn't cross-degrade NR4A3. Design spec: `nr4a3-degrader-design-spec.md`. | ★ **Make-or-break:** per-frame fpocket druggability over the cryptic-pocket MD — does the static ~0.026 orthosteric pocket reach ≥0.5? (never ⇒ small-molecule orthosteric route dead). ★ **CPU:** dock NOR-1 inverse agonists into the orthosteric pocket. ★ **AI (now co-lead):** de-novo protein-binder degrader (pocket-independent). Publish the addiction-analogy + designed warhead together; dTAG test = what the paper asks others to run. |
