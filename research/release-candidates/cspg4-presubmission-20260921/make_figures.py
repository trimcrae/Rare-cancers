...
def atlas(selected,n,title):
    ...
    step=13.3; h=112+step*len(selected);d=Drawing(950,h)
    d.add(Rect(0,0,950,h,fillColor=colors.white,strokeColor=None))
    x0=385;x1=857;top=h-70
    text(d,15,h-20,title,14,bold=True)
    text(d,15,h-55,'Original diagnosis',10,bold=True);text(d,363,h-55,'n',10,anchor='end',bold=True);text(d,938,h-55,'Median TPM',10,anchor='end',bold=True)
    text(d,15,h-37,'Burgundy: EMC    Blue: primary Malignant reference    Ochre: added Intermediate reference',11)
    ...
    for i,r in enumerate(selected):
        y=top-i*step;emc=r['original_diagnosis']=='Extraskeletal myxoid chondrosarcoma'; col='#9c2d49' if emc else ('#a56415' if r['source_class']=='Intermediate' else '#1d647d')
        ...
        for sid,v in samples[r['original_diagnosis']]:
            ...
            d.add(Circle(pos(v),y+jitter,1.75,strokeColor=None,fillColor=colors.HexColor('#be6980' if emc else ('#c89f6e' if r['source_class']=='Intermediate' else '#89aab7'))))
...
atlas(selected,'figure-1','CSPG4 RNA across the primary and broader reference categories')
...
for i,(label,na,nh,a) in enumerate(comparisons):
 ...
 xx=x0+(x1-x0)*(a-.5)/.5
 d.add(Circle(xx,y+5,4,fillColor=colors.HexColor('#1d647d'),strokeColor=None))
 d.add(Rect(x1-4,y-13,8,8,fillColor=colors.HexColor('#9c2d49'),strokeColor=None))
...
d.add(Circle(20,18,4,fillColor=colors.HexColor('#1d647d'),strokeColor=None));text(d,33,14,'Hofvander RNA sequencing',11)
d.add(Rect(315,14,8,8,fillColor=colors.HexColor('#9c2d49'),strokeColor=None));text(d,334,14,'GSE24369 arrays',11)
text(d,630,14,'Points are descriptive; no new tests.',11)
save(d,'figure-2')
