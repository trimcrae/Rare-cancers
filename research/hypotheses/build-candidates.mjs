#!/usr/bin/env node
// Build research/hypotheses/candidates.json: a broad, scored drug-repurposing
// landscape for EMC. Candidates and their evidence are defined here with explicit
// sub-scores; the composite priority score and ranking are computed. Reproducible
// and self-contained (citation metadata hardcoded from verified records).
// Run: node research/hypotheses/build-candidates.mjs

import { writeFileSync } from "node:fs";
import { fileURLToPath } from "node:url";
import { dirname, join } from "node:path";

const pmc = (id) => `https://www.ncbi.nlm.nih.gov/pmc/articles/${id}/`;
const CITES = {
  remiszewski2025: { short: "Remiszewski 2025 (review)", title: "From pathogenesis to the patient's bedside: a comprehensive review of extraskeletal myxoid chondrosarcoma", authors: "Remiszewski P, et al.", journal: "J Cancer Res Clin Oncol", year: 2025, pmcid: "PMC12504171", doi: "10.1007/s00432-025-06316-5", url: pmc("PMC12504171"), verified: true },
  zaltoprofen2023: { short: "Higuchi 2023 (zaltoprofen)", title: "A nonsteroidal anti-inflammatory drug, zaltoprofen, inhibits the growth of extraskeletal chondrosarcoma cells by inducing PPARγ, p21, p27, and p53", authors: "Higuchi T, et al.", journal: "Cell Cycle", year: 2023, pmid: "36636023", doi: "10.1080/15384101.2023.2166195", url: "https://doi.org/10.1080/15384101.2023.2166195", verified: true },
  emcCellLine2025: { short: "Iwata 2025 (NCC-EMC1-C1)", title: "Establishment and characterization of NCC-EMC1-C1: a novel patient-derived cell line of extraskeletal myxoid chondrosarcoma", authors: "Iwata S, et al.", journal: "Human Cell", year: 2025, pmid: "40580361", doi: "10.1007/s13577-025-01250-7", url: "https://doi.org/10.1007/s13577-025-01250-7", verified: true },
  kitMutation2018: { short: "Urbini 2018 (KIT)", title: "Identification of an Actionable Mutation of KIT in a Case of Extraskeletal Myxoid Chondrosarcoma", authors: "Urbini M, et al.", journal: "Int J Mol Sci", year: 2018, pmcid: "PMC6073125", doi: "10.3390/ijms19071855", url: pmc("PMC6073125"), verified: true },
  emcNGS2017: { short: "Davis 2017 (NGS)", title: "Next generation sequencing of extraskeletal myxoid chondrosarcoma", authors: "Davis EJ, et al.", journal: "Oncotarget", year: 2017, pmcid: "PMC5400622", doi: "10.18632/oncotarget.15568", url: pmc("PMC5400622"), verified: true },
  ...
  warmke2023: { short: "Huang SC 2023 (TAF15::NR4A3)", title: "Extraskeletal Myxoid Chondrosarcomas: The Uncommon Clinicopathologic Manifestations and Significance of TAF15::NR4A3 Fusion", authors: "Huang SC, et al.", journal: "Mod Pathol", year: 2023, pmid: "36948401", doi: "10.1016/j.modpat.2023.100161", url: "https://doi.org/10.1016/j.modpat.2023.100161", verified: true },
  ...
  transcriptionMech2016: { short: "Kim 2016 (TFG-NR4A3 transcription)", title: "The TFG-TEC oncoprotein induces transcriptional activation of the human β-enolase gene via chromatin modification of the promoter region", authors: "Kim AY, et al.", journal: "Mol Carcinog", year: 2016, pmid: "26310886", doi: "10.1002/mc.22384", url: "https://doi.org/10.1002/mc.22384", verified: true },
...
const CANDIDATES = [
  ...
  { id: "zaltoprofen-pparg", drug: "Zaltoprofen (PPARγ-inducing NSAID)", drugClass: "NSAID / PPARγ inducer", regulatoryStatus: "Approved NSAID (Japan)", notTriedInEmc: true,
    ...
    supportingEvidence: [
      { claim: "Zaltoprofen induced PPARγ and inhibited proliferation of EMC cells in vitro AND inhibited tumour growth (with apoptosis) in a mouse EMC model, while being well tolerated.", context: "EMC (in vitro + in vivo)", sourceId: "zaltoprofen2023" } ],
    rationale: "An approved, low-toxicity NSAID with direct in vivo anti-EMC activity through a defined mechanism — one of the strongest repurposing leads in EMC.",
    evidenceTier: "T1-preclinical-or-analog", speculationLevel: "moderate",
    keyRisks: "Preclinical only; PPARγ induction by an NSAID may not translate to clinical efficacy; NSAID class GI/renal/CV effects with chronic use.",
  ...
  { id: "vegfr-tki-extension", drug: "Regorafenib, cabozantinib, lenvatinib, nintedanib, sorafenib, axitinib, vandetanib, tivozanib, fruquintinib (approved multi-target VEGFR-TKIs)", drugClass: "anti-angiogenic multikinase (VEGFR) TKI", regulatoryStatus: "FDA/EMA-approved (other indications)", notTriedInEmc: true,
    ...
    rationale: "If VEGFR blockade is the most active class, other already-approved VEGFR-TKIs with deeper sarcoma data are rational, shelf-ready candidates never reported in EMC. A systematic public-database screen (DGIdb) independently surfaced this exact cluster - nintedanib, sorafenib, axitinib, vandetanib and tivozanib were among the highest target-reach approved agents untried in EMC - converting this from a hand-picked example into a reproducible shortlist.",
  ...
  { id: "pioglitazone-pparg", drug: "Pioglitazone (and other PPARγ agonists)", drugClass: "PPARγ agonist (thiazolidinedione)", regulatoryStatus: "FDA/EMA-approved (type 2 diabetes)", notTriedInEmc: false,
    mechanism: "Direct PPARγ agonism; whether this is beneficial in fusion-driven EMC remains unresolved.",
    emcVulnerability: { claim: "Davis et al. discuss the NR4A3/PPARγ axis as a research rationale; this does not establish an antitumour dependency.", sourceId: "emcNGS2017" },
    supportingEvidence: [
      { claim: "One patient with diabetes received pioglitazone and had stable metastatic EMC for 13 months before unequivocal progression. The authors explicitly state that pioglitazone's contribution to disease control is unknown.", context: "EMC (reported clinical exposure, n=1; uncontrolled)", sourceId: "emcNGS2017" } ],
    rationale: "Pioglitazone has already been administered to an EMC patient. The uncontrolled observation supports checking the mechanism and treatment timeline, but establishes neither efficacy nor a novel untried therapy.",
    evidenceTier: "T1-preclinical-or-analog", speculationLevel: "moderate",
    keyRisks: "PPARγ agonism is preclinically rational but unproven in EMC; pioglitazone class warnings (fluid retention, bladder-cancer signal).",
    openQuestions: ["Does PPARγ agonism alter growth in molecularly confirmed EMC models under matched controls?", "Can the reported clinical observation be separated from EMC's sometimes indolent natural history?"],
    correctionNote: "2026-09-12: corrected prior untried-in-EMC claim after reading Davis 2017 Discussion. Clinical exposure does not upgrade efficacy evidence; novelty score reduced from 3 to 1.",
    scores: { emcEvidence: 1, mechanisticFit: 2, availability: 3, safety: 3, biomarker: 1, novelty: 1 } },
  ...
  { id: "hdac-inhibitors", drug: "Romidepsin / panobinostat (HDAC inhibitors)", drugClass: "histone deacetylase inhibitor", regulatoryStatus: "Approved (CTCL/PTCL; myeloma)", notTriedInEmc: true,
    ...
    supportingEvidence: [
      { claim: "High-throughput screen of 221 anticancer drugs on a patient-derived EMC cell line (NCC-EMC1-C1) identified romidepsin, panobinostat and brigatinib as low-IC50 hits.", context: "EMC (cell line)", sourceId: "emcCellLine2025" } ],
    rationale: "An unbiased drug screen on an EMC cell line flagged two approved HDAC inhibitors — concrete, EMC-derived preclinical support for an epigenetic strategy.",
    evidenceTier: "T1-preclinical-or-analog", speculationLevel: "moderate",
    keyRisks: "Single cell-line screen; HDAC-inhibitor toxicity (cytopenias, cardiac); in vitro IC50 may not translate.",
  ...
  { id: "ntrk-inhibitors", drug: "Larotrectinib / entrectinib", drugClass: "TRK inhibitor", regulatoryStatus: "Approved (NTRK-fusion cancers)", notTriedInEmc: true, weakRationale: true,
    ...
    supportingEvidence: [
      { claim: "Pan-Trk positive in 35.4% of EMC with NTRK2/3 upregulation by RNA sequencing — but NO NTRK gene fusion was identified.", context: "EMC", sourceId: "warmke2023" } ],
    rationale: "Pan-Trk/NTRK expression raises the question of TRK inhibition, but EMC lacks NTRK fusions and expression alone rarely predicts TRK-inhibitor response — included to document the lead and its weakness.",
    evidenceTier: "T0-mechanistic", speculationLevel: "high",
    keyRisks: "Expression is not an NTRK fusion; TRK inhibitors work in fusion-positive disease, so the mechanistic rationale here is weak.",
  ...
  { id: "brigatinib-screen-hit", drug: "Brigatinib", drugClass: "ALK/ROS1 (multikinase) inhibitor", regulatoryStatus: "Approved (ALK+ NSCLC)", notTriedInEmc: true,
    ...
    supportingEvidence: [
      { claim: "Low IC50 in the NCC-EMC1-C1 patient-derived EMC cell-line screen.", context: "EMC (cell line)", sourceId: "emcCellLine2025" } ],
    rationale: "A hypothesis-generating screen hit worth mechanistic follow-up; included for completeness with the caveat that its EMC mechanism is unknown.",
    evidenceTier: "T1-preclinical-or-analog", speculationLevel: "high",
    keyRisks: "Single-cell-line screen; no mechanistic rationale; ALK/ROS1 not implicated in EMC.",
  ...
  { id: "nr4a3-modulation", drug: "NR4A3/NOR1-directed modulators (e.g. prostaglandin-A2-related compounds, NR4A ligands)", drugClass: "nuclear-receptor modulator", regulatoryStatus: "No clinical-grade agent; research compounds", notTriedInEmc: true,
    ...
    supportingEvidence: [
      { claim: "Prostaglandin A2 binds the NOR1 ligand-binding domain and transactivates it, indicating NOR1 is druggable.", context: "biochemical", sourceId: "nor1Pga2_2005" },
      { claim: "NR4A-family receptors regulate proliferation/apoptosis and are an emerging therapeutic target class.", context: "general", sourceId: "nr4aReview2019" } ],
    rationale: "The highest-value but hardest target: hit the fusion's DNA-binding receptor directly. Mechanistically central; pharmacologically immature.",
    evidenceTier: "T1-preclinical-or-analog", speculationLevel: "high",
    keyRisks: "No clinical-grade NR4A3 drug exists; whether to agonise or antagonise the fusion is unclear; selectivity within the NR4A family.",
...
const used = new Set();
for (const c of CANDIDATES) { used.add(c.emcVulnerability.sourceId); (c.supportingEvidence || []).forEach((e) => e.sourceId && used.add(e.sourceId)); }
const citations = {};
for (const k of Object.keys(CITES)) if (used.has(k)) citations[k] = CITES[k];
...
const doc = {
  ...
  scoring: {
    ...
    criteria: {
      ...
      availability: "0 preclinical-only … 2 investigational w/ human safety … 3 approved.",
      safety: "Tolerability/known safety (0 narrow window … 3 well-tolerated).",
...
const out = join(dirname(fileURLToPath(import.meta.url)), "candidates.json");
writeFileSync(out, JSON.stringify(doc, null, 2) + "\n");
console.log(`Wrote ${CANDIDATES.length} candidates (${Object.keys(citations).length} citations).`);
console.log("Ranked by priority score:");
for (const c of CANDIDATES) console.log(`  ${String(c.priorityScore).padStart(2)}  ${c.id}  [${c.evidenceTier}]${c.subsetRestricted ? " (subset)" : ""}${c.weakRationale ? " (weak rationale)" : ""}`);
