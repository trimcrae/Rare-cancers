...
// --- gap analysis: which approved drugs are already in our catalog? -----------
function analyseGaps(approvedDrugIndex, catalog) {
  ...
  for (const [name, info] of Object.entries(approvedDrugIndex)) {
    const n = norm(name);
    // token-level containment so "imatinib" matches "Imatinib (and other KIT inhibitors)"
    const hit = n.length >= 4 && hay.includes(n);
    (hit ? inCatalog : newlySurfaced).push({ drug: name, targets: info.targets });
  }
  const byReach = (a, b) => b.targets.length - a.targets.length || a.drug.localeCompare(b.drug);
  return { inCatalog: inCatalog.sort(byReach), newlySurfaced: newlySurfaced.sort(byReach) };
}
...
function selftest() {
  ...
  const gaps = analyseGaps(index, catalog);
  const ok =
...
async function main() {
  ...
  const gapAnalysis = analyseGaps(approvedDrugIndex, catalog);
