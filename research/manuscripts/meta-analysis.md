...
denominator}` counts, a diagnosis-period, provenance, and population-overlap keys. A PRISMA flow
diagram is generated for the manuscript (`research/figures/prisma.svg`); a formal per-study
risk-of-bias appraisal (adapted Newcastle–Ottawa) is planned and **not yet complete** — see
Limitations.

**Pooling.** For the manuscript we pool logit-transformed proportions by random effects
(DerSimonian–Laird), reporting I² and τ² heterogeneity and SVG forest plots
