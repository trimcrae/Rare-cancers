...
POLARITY = [
    ...
    # ⛔ THE `deposit-not-yet-posted` ROW WAS REMOVED 2026-08-25, AND TWO INDEPENDENT REASONS
    # ARRIVED FOR IT THE SAME DAY. Either alone would have retired it.
    # (a) ITS SUBJECT LEFT. The row anchored on "inside that deposit; it is not posted as a
    #     preprint", a sentence about the EXTENDED REPORT's preprint status. That document left the
    #     gate that day and Data availability was rewritten to cite the archive itself, so the
    #     phrase the row bound to no longer exists — and a polarity row whose anchor cannot be
    #     found is inert, which is what this module's own
    #     `test_every_polarity_row_still_finds_the_claim_it_guards` exists to catch.
    # (b) ITS CONDITION CLOSED, AND THAT IS CHECKABLE. The row names its artifact as
    #     "deposit-state.json: a `pending` Zenodo draft and no preprint server record". That file
    #     now carries a `published` block alone — DOI 10.5281/zenodo.22061075, published
    #     2026-08-23 — with no `pending` key at all. The window between drafting a corrected
    #     version and publishing it, which is the whole thing the row made legible, had already
    #     closed. Separately, checklist item 17 removed the argumentative half of the sentence
    #     anyway: the captured Sage guidelines say "Accepts preprints? Yes".
