...
certified by peer review [2]. Search-index results retained on 2026-08-10 identify an apparent
peer-reviewed counterpart of that work, in the *Journal of Biological Chemistry* in 2022. That
identification is search-index-level and is not independent publisher verification: the publisher
page, the PMC record, the bioRxiv page and the institutional repository were all unreachable from
this environment. So the existence of a published counterpart is indicated rather than confirmed,
on the same footing as the bibliographic detail given in reference 2, and agreement between repeated
index queries is not verification of the publisher's own record, nor of the peer-review process
itself. Both the existence and the detail must be confirmed against the publisher record before
submission. The findings cited above were read from the preprint text and stay attributed to it; the
...
unconfirmed. That caveat travels with every use of them.
The preprint is no longer the only support, since the Ewing result [3] is peer-reviewed and is the
one showing a fusion-dependent PRMT5 requirement.
...
2. PRMT5 as a Novel Druggable Vulnerability for EWSR1-ATF1-driven Clear Cell Sarcoma. bioRxiv preprint, posted 2022-03-23. doi 10.1101/2022.03.23.485409. The preprint states that it is not certified by peer review, and the findings cited here were read from this preprint. Search-index results identify an apparent peer-reviewed counterpart: Li BX, David LL, Davis LE, Xiao X. Protein arginine methyltransferase 5 is essential for oncogene product EWSR1-ATF1-mediated gene transcription in clear cell sarcoma. *Journal of Biological Chemistry* 2022;298(10):102434. doi 10.1016/j.jbc.2022.102434. PMC9513783. That record was identified from a search index and was not read here, so its existence, together with its title, authors, volume, issue, article number, DOI and PMCID, is indicated by a search index rather than confirmed; all of it requires confirmation against the publisher record before submission, and whether that record is the same work in content was not checked.
...
preprint that neither retrieval returned, so its own entry carries its identifier and posting date
alone. An apparent peer-reviewed counterpart is indicated by search-index results retained on 2026-08-10 and recorded in
[`prmt5-ccs-preprint-publication-status-2026-08-10.json`](../../literature/prmt5-ccs-preprint-publication-status-2026-08-10.json);
that identification is search-index-level, since the publisher page, the PMC record, the bioRxiv page
and the institutional repository were all unreachable, so the counterpart's existence and its
metadata alike are indicated rather than confirmed and must both be checked against the publisher
record before submission. Where a record lists more than six authors the
first six are given.
...
|---|---|---|---|
| *"The fusion-class transfer is an assumption."* | The transfer is argued rather than assumed, and an argument is not a result | §7 of the earlier draft, now §4.4 | Two things changed it and neither is an EMC measurement: a peer-reviewed fusion-dependent PRMT5 requirement in a second EWSR1-fusion sarcoma [3], and the finding that the commonest EMC fusion and the commonest clear cell fusion retain the same number of PRMT5-motif sites (§3.7). What did not change: EWSR1::ATF1 and EWSR1::NR4A3 still do not share a DNA-binding domain, a target repertoire or a disease biology |
