...
|---|---|---|---|
| `cycle_delivering` | **advises** | no receipt within 2 expected cycle periods | §2.2 — a fired Routine is not a delivered one |
| `advancing_live_work` | **advises** | 3 consecutive receipts with `route_advanced: none` | CLAUDE.md §0 — the documentation-drift failure |
| `evidence_moving` | **advises** | an entry `running` with `last_evidence_utc` unchanged over 2 cycles | §4's unproven-pipeline rule: progress checks, not liveness pings |
| `blocks_are_real` | **redirects** | any `blocked` entry with empty `blocked_evidence` | §0 — "blocked" usually means "nobody checked" |
| `queue_is_takeable` | **redirects** | no entry is unowned, unblocked and still holding retry budget | added 2026-08-26 — every other row asks whether the loop works WELL; this asks whether there is work it CAN do. A fully-owned queue makes a loop that fires, finds nothing, writes a receipt saying so, and repeats: a stall wearing the costume of a quiet week. ⚠ *Declared late — the condition shipped before this row existed.* |
| `scores_are_reachable` | **advises** | an OPEN ledger row carries no `score`, so nothing can ever offer it | added 2026-08-28 (AUT-PD-143) — **`queue_is_takeable` cannot see this row, which is why both must exist.** That condition reports TAKEABLE off the rows that ARE rankable, so a ledger could reach 100% unrankable real work and still print green on one derived entry. Measured at 95071429c: `priority.py` creates a score in exactly ONE place, inside `build_entries`, which derives rows from systems/graph — so a HAND-FILED row could never acquire one, and four readers (the sort's -1e9 sentinel, `_table`, `queue_is_takeable`, `handoff.py`) all read that absence as 'not work'. 94 of 271 rows had never been offered to any cycle, live RT-SGK1 / RT-ALK-HIT / RT-JUNCTION-NEOANTIGEN work among them. ⚠ `advises` rather than `redirects` because most residual rows serve RT-AUTONOMY, which is not a route in systems/graph, so no pass can derive a floor for them and no cycle could clear the row. |
| `cycles_are_sized` | **advises** | one `session_id` carries more than `max_cycles_per_session` receipts | added 2026-08-26 — **the session-shape rule (§4.1, skill §3) had no measurement at all, and it failed by being UNREACHABLE rather than unheeded.** It lived only in `.claude/skills/research-loop/SKILL.md`, a skill binds only when loaded, and every one of that skill's load triggers was a Routine firing a cycle — so on the INTERACTIVE path, where a human asks for research work directly, it never bound. Measured: `"name":"Skill"` appears **0 times** in the transcript of the session that broke it, which ran CYC-0005 and CYC-0006 end to end, compacted 23 times and reached 7.6 MB; an earlier session ran three cycles. Reachability was repaired in CLAUDE.md §6, which loads every session; this row is the enforcement half, because a rule nothing measures decays into a suggestion. ⚠ It bounds CYCLES, a proxy for context — one enormous single cycle passes it. Nothing here can read a context window, and receipts already carry `session_id`. |
| `fanout_is_governed` | **advises** | a receipt records `subagents.max_concurrent` above `subagent_width` | added 2026-08-26 — **§9's most important dial had never been read by anything.** `grep -rn subagent_width` over the whole repository returned TWO hits: the JSON defining it and one test asserting it equals 5. No code consulted it and no receipt recorded a dispatch, so compliance was luck. The unit had never been written down either, which made it UNENFORCEABLE rather than unenforced — now CONCURRENT agents, recorded in `_subagent_width_means`. ⚠ Retrospective: nothing can intercept a dispatch, so prevention lives at CLAUDE.md §1's spawn authorisation and this row makes an overrun visible. ⚠ It governs concurrency and NOT the serial total, which stays honestly ungoverned until `utilisation_denominator` has an observed value. |
| `budget_recovering` | **advises** | backoff level > 0 for > 24 h | §9 — a limit that never clears is a stuck loop |
| `gates_green` | **redirects** | preflight red on `main` for > 24 h | a red trunk stops every cycle at step 8 |
| `authority_respected` | **blocks** | any outward act in the log without a matching grant in `publication-authority.json` | §6 — the one thing that must never be self-granted |
| `stalls_are_named` | **advises** | `stuck_clock.py` reports a row gone `stalled_needs_human` — automation retried it for `STUCK_AFTER_CYCLES` cycles and nothing advanced it | added 2026-08-28 (AUT-PROP-029) — **the module was fully built, fully tested, and NOTHING CALLED IT.** Its two-clock model catches the row that is claimed, abandoned, re-queued and re-claimed forever: maximally alive on the touch clock, correctly dead on the advance clock. No board condition read it, `priority.py`/`handoff.py` did not exclude a terminal row from ready work, and no job ran `--check`. Verified live post-merge: three rows were genuinely terminal and invisible to everything but a hand-run CLI. ⚠ Fed by a caller-supplied verdict file, exactly like `gates_green`, because this module has no subprocess by design and stuck_clock needs `git log`; an absent verdict is `unmeasured`, never green. ⚠ `advises` because `redirects` would tell the same automation to retry the same row this cycle — the busy-retry loop the module exists to unmask, rebuilt one level up. |
