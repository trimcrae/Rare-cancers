...
| **L2** | ★ **The prespecified site is SPLIT across two real cavities** — 4 shared residues, Jaccard **0.21**, centroids **9.853 Å** apart, *further than the gate's own 8.0 Å ceiling*. Pocket 1 = the helix-3 face; pocket 2 = the helix-11/12 face | [`r3-site-choice-audit.json`](../../modalities/r3-site-choice-audit.json) ✅ **on `main` and this branch** *(was flagged branch-only; reconciled — verified 2026-08-06)* | ⭑ **and it re-ranks path 2** — see the finding in [§8 C1](#8--cross-checks-taken-while-writing-all-0) |
...
| **L7** | ★★ **The steric mechanism does NOT invert.** The NR4A1-sparing ("inverse NR-V04") direction fires at **0.96× its own null** over the same frame, poses and threshold, against the forward direction's **5.34×**. Structural reason: the best forward bulk margin is **+4** heavy atoms; the best inverse margin anywhere is **+1** | [`nr4a1-sparing-axis.json`](../../modalities/nr4a1-sparing-axis.json) ✅ **on `main` and this branch** *(was flagged branch-only; reconciled — verified 2026-08-06)* | answers a route trimcrae asked about, **⏸ parked at its own measured null** — and it bounds what path 2 may generalise to |
| **L8** | **The canonical-library question is SETTLED and `5b-T` is invariant to the ruling** — every one of its four candidates is present with identical SMILES in both enumerations | [`nr4a3-linker-library-canonical.json`](../../modalities/nr4a3-linker-library-canonical.json) → `release_condition` | **one** of `5b-T`'s two stated blockers is gone. ⚠ Its **scope is explicit and narrow**: it discharges the row-25 hold **and nothing else** — the corridor conflict stands and the rung's geometry still inherits `R5` |
| **L9** | ⛔⛔ **`R14-a`'s cognate-ligand self-control RAN — on `main` — AND FAILED.** 10 targets, **7 pass**; CYP3A4, PXR and PPARG miss by **9.503 / 6.761 / 6.87 Å** against a 2.0 Å recovery criterion. `panel_readable: false`, so **all four SI §S1 anti-target clauses are now measured-unreadable.** MR was added and passes. ⛔ **And a second, independent block:** the NR4A3 ΔG column those published margins subtract **is not committed anywhere in this repo** | [`antitarget-selfcontrol.json`](../../modalities/antitarget-selfcontrol.json) ✅ **on `main` and this branch** *(was flagged `main`-only; reconciled — verified 2026-08-06)* | ★ **the largest single change to the queue.** A $0 instrument repair now gates four sentences the SI already publishes. It was not on this page at all |
...
| # | path | note |
|---|---|---|
| 11 | **Junction ASO / junction neoantigen** | genuinely removes the paralogue requirement. ✅ The ASO lane was **corroborated** by the exon audit (its deliberate refusal of the exon→CDS mapping bracketed the correct answer). ⚠ The neoantigen lane still owes a correction — its 26 binders span seams that do not exist | ⛔ **NARROWED 2026-08-06 — THIS WAS TRUE OF ONE MODULE, NOT OF THE LANE.** *Superseded, retained:* the unqualified *"the ASO lane is unaffected"*. The claim rested on `junction_breakpoint_scan.py` deliberately refusing the exon→CDS mapping — true of **that script**. `junction_aso.py`'s `FUSION_JUNCTION_MODE=real` path DID do the mapping, with the defective arithmetic, and produced the committed design panel the paper leads with: seam at NR4A3 CDS nt 1081, resuming at residue **361**, against a corrected range of **[1, 1]**. Code fixed and artifacts bannered 2026-08-06; regeneration needs Ensembl and must run in CI. See [`systems/AUDIT-2026-08-06-routes.md`](../../../systems/AUDIT-2026-08-06-routes.md).
| 12 | **TCIP** | retires `R12`; keeps `R4`/`R5`/`R7`/`R9`/`R10` — a TCIP is bivalent and still induces a ternary, so only the degradation-specific requirement goes (*superseded, retained: "retires `R9`/`R10`/`R12`"*, corrected 2026-08-06 against `nr4a3-tcip-reach.json` → `map_edits_required`). Its citation is still an auto-captured lead and must clear `verify-refs` before any manuscript quotes it |
| 13 | **Downstream / dependency target** (PPARG axis) | removes the requirement by leaving the target. ⭐ **The literature question — agonism vs antagonism — is ANSWERED** ([`pparg-direction-emc.md`](../repurposing/pparg-direction-emc.md), 2026-08-06): **UNRESOLVED, leaning agonism, T1 with a model-identity caveat.** ⚠ *Superseded, retained: "Stalled on one literature question … that nobody has spent an hour on."* The hour was spent and the finding is not that the question was unasked — it is that **two primary studies answered it in OPPOSITE directions from the same observation** (Subramanian 2005 → inhibitors; Filion 2009 → agonists), and the single functional test favouring agonism ran in a disputed-identity line. What is actually missing is a PPARγ **ACTIVITY** readout in EMC; abundance is already measured twice, concordantly |
...
| **C5** | Checked which ref each cited artifact actually lives on, before writing any claim from it (CLAUDE.md §7) | ✅ **THE DRIFT IS RECONCILED — re-measured 2026-08-06 and all five artifacts are on BOTH refs.** `git cat-file -e` against `HEAD` and `origin/main` for `antitarget-selfcontrol.json`, `r3-site-choice-audit.json`, `pocket-accepted-candidates.json`, `nr4a1-sparing-axis.json` and `apo-pose-site-in-regime.json`: ten of ten present. ⚠ *Superseded, retained: **"BRANCH DRIFT IS LIVE, IN BOTH DIRECTIONS, ON THE SAME DAY"** — `main`-only: `antitarget-selfcontrol.json`, "the largest result on this page is absent from the branch these registers were written on"; branch-only: the other four.* That was true when written and is not now. ⛔ **A drift warning that outlives its drift is not harmless caution:** it tells the next session that committed artifacts are unsafe to quote, and it spends the force of a ⛔ on a condition that has been fixed — so the next real one reads as noise. Re-measure this row rather than trusting it; the check is two `git cat-file` calls and costs nothing |
