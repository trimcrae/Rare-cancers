...
> The repo's overarching research plan + the #1 program's execution ladder live in **[`/nr4a3-program-map.md`](../nr4a3-program-map.md)**
...
> two conflict on the degrader program's plan or ordering, **nr4a3-program-map.md wins.**
>
...
> [`cancer-modality-census.md`](../modality-census/cancer-modality-census.md).
>
...
**[`what-a-civilian-can-buy.md`](../modality-census/what-a-civilian-can-buy.md)**, against three filters (≲$1,000 · no
...
| B7-H3 (CD276) ADC / bispecific | ⛔ **Confirm-gated — and the gate is no longer one IHC.** This repo's own computed screen finds B7-H3/CD276 **NOT significantly selective across the translocation-sarcoma class, BH q = 1.0** ([emc-surface-target-landscape.md §3.2](../surface-targets/emc-surface-target-landscape.md)). ⚠ *Superseded, retained: "needs EMC B7-H3 IHC" / "fastest such route"* | **Targeted (indirect)** — surface antigen (not the fusion), and expression ≠ selectivity |
...
| FAP radioligand therapy | ⛔ **Confirm-gated — and FAP is likewise NOT class-selective** in the same screen ([emc-surface-target-landscape.md](../surface-targets/emc-surface-target-landscape.md):127, alongside B7-H3 and EGFR). ⚠ *Superseded, retained: "needs EMC FAP-PET"* — a PET scan still cannot make a non-selective target selective | **Targeted (indirect)** — stromal target; theranostic (tracer = diagnostic) |
...
>   [`emc-surface-target-landscape.md`](../surface-targets/emc-surface-target-landscape.md). ⛔ And it returned a NEGATIVE
...
- **PRAME is the best antigen-directed bet:** **53% of sarcoma lines**; MAGE-A4 (7%) and NY-ESO-1
  ...
  ([`emc-surface-target-landscape.md` → Amendment 1](../surface-targets/emc-surface-target-landscape.md);
...
1. **NR4A3-degrader result paper** ([`nr4a3-degrader-paper.md`](../degrader/nr4a3-degrader-paper.md)) — the
   target-centric cryptic-pocket/selectivity/degrader-design result. NR4A3-selective but **not**
   fusion-selective (shared LBD).
2. **Fusion-junction ASO paper** ([`fusion-junction-aso-working-record.md`](../aso/fusion-junction-aso-working-record.md)) — the
   ...
   **Delivery is a stated limitation INSIDE the deliverable, not a gate on posting it.**
   ...
   ([`fusion-selective-approaches-overview.md`](../fusion-direct/fusion-selective-approaches-overview.md), which houses the
...
- **This is a LONG-LIVED, REVISITABLE project on a RISING in-silico frontier — not a one-shot (trimcrae, standing
  ...
  [`../method-watch.md`](../../method-watch.md) (**weekly** auto-digest + trigger table — the cron is
  `0 11 * * 5`; ⚠ *superseded, retained: "monthly"*) plus Q2/Q3 below; (b) expect
...
**BUILT** — `scripts/method-watch.mjs` + `.github/workflows/method-watch.yml` (**weekly** cron, `0 11 * * 5`),
...
- A **cron CI job** (**weekly**) running targeted searches (Europe PMC + GitHub releases) for our gaps
