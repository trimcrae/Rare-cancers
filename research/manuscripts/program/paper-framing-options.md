...
([cryptic-pocket-atlas-concept.md](../occupancy/cryptic-pocket-atlas-concept.md)) is explicitly scoped as *the next
...
1. ✅ **CLOSED 2026-08-07 ($0 CI) — the MM-GBSA decoy null's primary run output is now committed.**
   `archive-results-aws.yml mode=archive prefixes=nr4a3-decoy` mirrored the surviving S3 objects into
   [`results/nr4a3-decoy/`](../../../results/nr4a3-decoy), and the chain is **verified rather than asserted**:
   the single-snapshot arm's 38 per-drug records reproduce the committed constant `DECOY_2026_06_30`
   **exactly**, at 22 of 38 positive and `confirmed_selective: 15`.
   ⛔ **The verification is the deliverable, not the copy.** THREE MM-GBSA arms were archived and only one
   is the run the constant came from — the two multi-snapshot arms return materially different margins
   from the same docked poses (that difference *is* §2.7's de-noising result), so a file merely sitting in
   `results/` would not have told anyone which one the paper quotes. One home:
   [`decoy-null-provenance.json`](../../modalities/decoy-null-provenance.json), generated by
   [`decoy_null_provenance.py`](../../modalities/decoy_null_provenance.py) and held by
   `tests/test_decoy_null_provenance.py`, which fails if **zero or more than one** arm matches.
   ⚠ *Superseded, retained: "The MM-GBSA decoy null's primary run output lives in S3, not in a committed
   JSON … for the paper whose headline this is, the chain must be end-to-end."* The roadmap's
   [§6d](../nr4a3-program-map.md#6d--superseded--not-here-and-that-is-deliberate) sentence it quoted —
   *"the only row whose refutation is not readable end-to-end from a committed artifact"* — is retired by
   this, and §6d is where that retirement belongs.
2. ✅ **CLOSED 2026-08-07 ($0) — the instrument census exists**, GENERATED from roadmap §3.1 and §3.2
   rather than transcribed, so it stays true when those tables move:
   [`instrument-census.json`](../../modalities/instrument-census.json) /
   [`instrument-census.md`](../../modalities/instrument-census.md), by
   [`instrument_census.py`](../../modalities/instrument_census.py).
   ⛔ **And counting it moved a number this list itself was carrying.** §3.1 holds **22** instrument rows,
   not 20 — `V21` (the anti-target docking panel) and `V22` (the second pose method) were added
   2026-08-03 as new rows. **Six of the 22 have no known-answer test of their own**, which is the count
   `P1` and `P7` are actually about and which no prose statement of this register had. Per CLAUDE.md
   rule 1 the census is now the one home of both figures; do not re-type them here.
   ⚠ *Superseded, retained: "all 20 instruments".*
...
confirmed in writing ([preprint-plan.md](../degrader/nr4a3-degrader-preprint-plan.md)) and the audience is exactly the
...
([outreach-emails.md](../degrader/nr4a3-degrader-outreach-emails.md)). **A dossier is a much more natural thing to hand
...
| 9 | Ran `lint_claims.py` against **this file** | **0 ERROR, 15 WARN** — stated rather than claimed clean, because the first draft of this row said *"0 WARN"* and running it refuted that. ⚠ **Re-measured 2026-08-07 and the figure had ALREADY DRIFTED before that pass touched the file** — `git show origin/main:` this file, linted, returns **15**, so the **12** below was stale on `main` and the 2026-08-07 edits added **zero** warnings. That is the drift a self-reported count always develops: nothing checks it, because this file is not in `DEFAULT_TARGETS`. ⚠ *Superseded, retained: **0 ERROR, 12 WARN**, and its split "3 quoting / 6 validated / 1 confirmed / 1 proof that / 1 Establishes".* ⛔ **AND WRITING THAT CORRECTION MOVED THE NUMBER AGAIN, WHICH IS THE REAL FINDING.** The sentence above quotes the flagged terms while explaining them, so the linter counts them; the reading went 15 → 17 the moment the row was rewritten. **A row that reports a count over the file it lives in is a fixed-point problem, not a fact** — it cannot be maintained by hand and there is no gate that would catch it drifting. So the row now records a DATED READING and nothing else, and the number is not to be trusted between readings: run `python3 research/manuscripts/lint_claims.py research/manuscripts/program/paper-framing-options.md` and read the last line. Every warning is benign and each was read individually — the classes are (a) this file quoting the offending strings it is flagging or naming its own headings, (b) the roadmap's instrument vocabulary used as vocabulary, (c) one journal-fee sentence, (d) one mathematical identity, (e) one word about publishing calibration. **No regulated claim is asserted anywhere in this file.** If it is ever added to `DEFAULT_TARGETS` the gate becomes real and this row can go back to naming a figure |
...
| **B** | [nr4a3-degrader-paper.md](../degrader/nr4a3-degrader-paper.md) + [SI](../degrader/nr4a3-degrader-paper-SI.md) | The `V17` positive-control failure (NR4A1 Cys551, 0 of 25 frames, median RSA 0.064) appears **nowhere**, while §5's Tier 0 gate is reported as *"pass on both axes"* on the word *exposed*. Already logged as a manuscript finding in the roadmap's §12; repeated here because it bears on **three** of the framings above |
| **C** | [nr4a3-degrader-paper-SI.md](../degrader/nr4a3-degrader-paper-SI.md):229 | `**Lead — NR4A3-selective (the validated path):**` — see [§6](#6--the-four-substantive-lint-warnings-named) |
