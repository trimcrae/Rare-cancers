...
| **7** | **Junction neoantigen** (vaccine / TCR-T / soluble TCR) | ⛔ **REMOVES it** — peptide identity is categorical | ○ drafted — **correction PAID 2026-08-07** | ✅ **regenerated** on the transcript model: 5 junctions, 11 binders (4 strong). ⚠ **But the route got weaker, not stronger:** on the **ten-allele** junction screen the public e7::e3 junction is now presented on **B\*15:01 alone**, so its HLA coverage falls **29.7% → 8.51%** and the pooled panel **58.0% → 27.4%**. ⭐ **RE-GRADED 2026-08-28 on coverage, and the grade does not move up.** *(Superseded, retained: "Next: regenerate `patient-cd4-demo.json` and the TAF15 panel, both still on the retracted seam")* ⛔ Both were already regenerated and committed on the transcript model when that sentence was written, and so were `vaccine-construct.json` and `coverage_scan.py`'s outputs — the class-II arm was never withheld. Panel matters: the 34-allele screen adds HLA-A\*30:02, and [`emc-vaccine-development-path.md`](../neoantigen/emc-vaccine-development-path.md) §2.3 owns what the figures become. **Next, and free:** add **HLA-C** to the class-I panel and re-run the coverage scan in CI |
...
| **9** | **Synthetic-lethal / dependency partner** | ⛔ **REMOVES it** — different target | ⏸ parked on data, not on ideas | ⛔ **2026-08-05:** the DepMap line labelled EMC (**ACH-001519 / H-EMC-SS**) has no CRISPR data *and* is recorded as not fusion-positive ([Amendment 1](../surface-targets/emc-surface-target-landscape.md)), so the park now waits on a **patient-derived** model, not on a DepMap release. *(Superseded, retained: "check whether the one EMC line in DepMap (**ACH-001519 / H-EMC-SS**) has gained CRISPR data since 24Q4")* |
...
[`nr4a3-emc-biology-evidence.md`](../degrader/nr4a3-emc-biology-evidence.md) (hypothesis 1) with its numbers
...
3. Occupancy must do something. This is where an inhibitor is *weaker* than a degrader here, and it must
   ...
   [`degrader-vs-synthetic-lethal.md`](../dependency/degrader-vs-synthetic-lethal.md) §1), which is exactly why degradation was chosen — *"an inhibitor would have to block a function NOR-1
...
[`nr4a3-monovalent-pocket-route.md`](../occupancy/nr4a3-monovalent-pocket-route.md) §3.
...
([`fusion-selective-approaches-overview.md`](../fusion-direct/fusion-selective-approaches-overview.md)).
...
([`fusion-junction-neoantigen-paper.md`](../neoantigen/fusion-junction-neoantigen-paper.md)), and the class has real
...
binders span seams that do not exist in the disease protein.

✅ **THE CORRECTION WAS PAID ON 2026-08-07, AND THIS PARAGRAPH IS RETAINED ONLY AS THE RECORD OF WHY.** *(Superseded, retained: "⛔ **This is a correction owed, not a caveat**: the source module is fixed, the artifact is not regenerated, and nothing in the artifact should be quoted until it is. Regeneration needs MHCflurry in CI and belongs to that lane.")* The regeneration ran in that lane. [`fusion-breakpoint-neoantigens.json`](../../modalities/fusion-breakpoint-neoantigens.json) carries `_coordinate_system` TRANSCRIPT and every junction it emits grades `EMITTABLE`; the counts it replaced the seven-junction set with are owned by that artifact and by [`fusion-junction-neoantigen-paper.md`](../neoantigen/fusion-junction-neoantigen-paper.md), and are not re-typed here.

⛔ **AND EVERYTHING DOWNSTREAM WAS REGENERATED TOO — verified artifact by artifact on 2026-08-28, because this register had gone on saying otherwise.** `patient-cd4-demo.json` and the TAF15 panel each carry `source.coordinate_system` TRANSCRIPT with `source.grade` EMITTABLE at the corrected seam; re-running `vaccine_construct.py` and `coverage_scan.py` offline reproduces their committed artifacts byte-for-byte. ⚠ The class-II arm and the combined CD8∧CD4 figure were recorded as **withheld** for at least six days after they were computed — the rebuild is dated no later than 2026-08-22 by this repository's own supersession marker in [`fusion-junction-neoantigen-paper.md`](../neoantigen/fusion-junction-neoantigen-paper.md), and git cannot date it more tightly because the history is a squashed import, committed and written up in [`emc-vaccine-development-path.md`](../neoantigen/emc-vaccine-development-path.md) §B4, which owns them.

**Grade: ○ unchanged** (tolerance to a mostly-self junction, a cold tumour, and partial HLA coverage were
already the stated doubts), **and the free correction is paid.** ⚠ **The re-grade on coverage does not move the route up.** What now bounds it is the size of the addressable population — low on the public junction, lower again for the both-arms construct, which is the product of two arms — and above that, presentation, which has never been measured on EMC tissue. **The one free step left is to add HLA-C to the class-I panel**, which the vaccine path's own Table 2 names as the first extension that could raise the figures at a fixed acceptance threshold.
...
**Already run, and negative.** [`degrader-vs-synthetic-lethal.md`](../dependency/degrader-vs-synthetic-lethal.md) §2b:
...
EWSR1 fusion ([`emc-surface-target-landscape.md` → Amendment 1](../surface-targets/emc-surface-target-landscape.md)), so even
...
[`nr4a3-emc-biology-evidence.md`](../degrader/nr4a3-emc-biology-evidence.md)), so the *functional* site is shared as
...
| **7** | ✅ **DONE — and its premise is withdrawn (2026-08-05).** The re-query ran: **no CRISPR data** for ACH-001519 in 24Q4 (`fet-ddr-axis-scan.json` → `/emc_line/has_crispr_gene_effect`). And the line is recorded as **not carrying an EWSR1 fusion** ([Amendment 1](../surface-targets/emc-surface-target-landscape.md)), so an answer would not be EMC-specific | route 9 | *(superseded, retained: "the only EMC-specific dependency datum that could exist without a lab")* |
...
| **9** | ✅ **DONE 2026-08-07 in the CI lane, and re-verified 2026-08-28** — the junction neoepitope set and every artifact downstream of it are on the transcript model. **What replaces it, and is also free:** add **HLA-C** to `coverage_scan.py`'s HLA-A/-B-only class-I panel and re-run it in `modalities-run.yml` | route 7 | *(Superseded, retained: "⛔ its 26 predicted binders span seams that do not exist. Needs MHCflurry in CI — free, but it belongs to that lane")* The replacement is the route's own revival condition: HLA-C is the one locus the class-I panel omits |
...
| *NR4A1/NR4A3 as functionally redundant suppressors of AML.* **Blood** 2018. PMID **29343483** | conditional double-KO unmasks HSC-homeostasis defects (route 1) | [`nr4a3-emc-biology-evidence.md`](../degrader/nr4a3-emc-biology-evidence.md) |
| Safe S, Karki K. *The Paradoxical Roles of Orphan Nuclear Receptor 4A (NR4A) in Cancer.* **Mol Cancer Res** 2021. doi:10.1158/1541-7786.mcr-20-0707 | wild-type NR4A3 tumour-suppressor roles (context for routes 1, 3, 8) | [`fusion-selective-andgate-degrader-paper.md`](../degrader/fusion-selective-andgate-degrader-paper.md) |
| Chen J, López-Moyado IF, … Rao A. *NR4A transcription factors limit CAR T cell function in solid tumours.* **Nature** 2019;**567**:530–534 | the pan-NR4A / exhaustion result behind route 4 | [`nr4a3-degrader-carT-and-family-druggability-framing.md`](../degrader/nr4a3-degrader-carT-and-family-druggability-framing.md) |
| Filion C, et al. **J Pathol** 2009. **PMC4429309** | the fusion transactivates a PPARG response element — routes 5 and 12, and the discriminating evidence in finding 3 | [`nr4a3-emc-biology-evidence.md`](../degrader/nr4a3-emc-biology-evidence.md) |
| Boulay G, et al. *Cancer-specific retargeting of BAF complexes by a prion-like domain.* **Cell** 2017. doi:10.1016/j.cell.2017.07.036 | FET prion-like-domain biology — routes 6, 9, 11 | [`fusion-selective-andgate-degrader-paper.md`](../degrader/fusion-selective-andgate-degrader-paper.md); [`degrader-vs-synthetic-lethal.md`](../dependency/degrader-vs-synthetic-lethal.md) |
| Brien GL, et al. *Targeted degradation of BRD9 reverses oncogenic gene expression in synovial sarcoma.* **eLife** 2018 | the ncBAF/BRD9 comparator behind route 9 | [`degrader-vs-synthetic-lethal.md`](../dependency/degrader-vs-synthetic-lethal.md) |
| **Zaienne D, Arifi S, Marschner JA, Heering J, Merk D. 2022**, **PMID 35704774** — NOR-1/NR4A3 druggability evaluation, low-µM inverse agonist cmpd 19, **PMC9542104**; ⭐ **the hits act on a `hinge+LBD` Gal4 chimera — the AF-1 is not in the construct** ([`nr4a3-druggability-reconciliation.md` §5a](../../modalities/nr4a3-druggability-reconciliation.md)); ⛔ **this row previously appeared TWICE in this table — once here and once as a separate "Munck 2022" entry, retained-and-merged 2026-08-03 because no such paper exists ([§5b](../../modalities/nr4a3-druggability-reconciliation.md))**; ⚠ **no NR4A1/2 counter-screen** (and *"Safe 2025 selective analogues"* is a review's loose paraphrase of the same compounds, **not** a distinct selective series — corrected in the repo 2026-07-12) | NR4A3 is experimentally ligandable | [`nr4a3-degrader-carT-and-family-druggability-framing.md`](../degrader/nr4a3-degrader-carT-and-family-druggability-framing.md) |
| Nabet B, et al. *The dTAG system for immediate and target-specific protein degradation.* **Nat Chem Biol** 2018. doi:10.1038/s41589-018-0021-8 | the delegated fusion-dependence test that gates every driver-directed route | [`fusion-selective-andgate-degrader-paper.md`](../degrader/fusion-selective-andgate-degrader-paper.md) |
| Modern Pathology 2023, PMID **36948401** (58 EMC, 58/58 NR4A3-rearranged); Agaram et al., **Hum Pathol** 2014, **PMC4015728** | the fusion is near-invariant and clonal — the premise of the whole memo | [`nr4a3-emc-biology-evidence.md`](../degrader/nr4a3-emc-biology-evidence.md) |
