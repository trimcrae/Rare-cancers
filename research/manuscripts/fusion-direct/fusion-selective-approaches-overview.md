...
> [`nr4a3-degrader-strategy-ternary-first.md` → "DEFERRED DECISION — selectivity TARGET"](../degrader/nr4a3-degrader-strategy-ternary-first.md).
> That decision may route Level 2 to the **ASO** (paper 1) rather than the AND-gate degrader (paper 3).

## The five manuscripts
1. **RNA — fusion-junction ASO / siRNA** → [`fusion-junction-aso-working-record.md`](../aso/fusion-junction-aso-working-record.md)
2. **Immune — fusion-junction neoantigen** (vaccine / TCR-T / soluble-TCR) → [`fusion-junction-neoantigen-paper.md`](../neoantigen/fusion-junction-neoantigen-paper.md)
3. **Protein — AND-gate (coincidence-detection) bivalent degrader** → [`fusion-selective-andgate-degrader-paper.md`](../degrader/fusion-selective-andgate-degrader-paper.md)
...
| **Junction neoantigen** | Immune | **Highest against the parents** — absent from wild-type EWSR1 and wild-type NR4A3 — ⚠ the ONLY novelty test run here compares against those two parent proteins (`fusion_breakpoints.emit_junction`'s novelty filter); **no proteome-wide search has ever been performed** | **MODERATE — platform proven in humans, but self-adjacent junction (tolerance) + cold tumour + coverage that FELL to ≈8.5% on the public junction / ≈27% pooled after the 2026-08-07 seam correction cast real *efficacy* doubt** ⚠ *(superseded, retained: "~16% both-arm coverage" — that both-arms figure is withdrawn; its class-II half still rests on the retracted seam)* | High: breakpoints + coverage computed | Immunogenicity (tolerance, cold tumour) | Polish to preprint; per-patient pipeline already runs |
