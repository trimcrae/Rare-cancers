...
the author on 2026-08-22; the page charge is unchanged and is now a budget the page count is built
to. Cancer Gene Therapy's £145/$238 per typeset page, read at HTTP 200, stands as recorded.*

---

**To:** The Editor-in-Chief, *Nucleic Acid Therapeutics*
...
pre-rename title, which survived here after the manuscript was retitled to its finding. Second, and
the same defect one document over: the extended report's title, which stood here after the
submission was split into a condensed journal article and the extended report it cites, so the line
an editor reads first named a manuscript the envelope does not contain. The title has ONE home —
`fusion-junction-aso-journal-article.md`'s own H1 — and the line above is copied from it verbatim;
retitle the manuscript and this line must be recopied, not retyped.*
...
built at real exon termini of the same transcripts meet the same screen at 40.6% against the panel's
45.8% — so most of the liability is what joining two exon termini of these genes gives rather than
anything specific to this disease, and it says in its own abstract that the ten-base-pair criterion
is a convention rather than a measurement. It then ranks within each junction to name the best
available reagent at the two most frequently reported junctions with a published breakpoint, and
names the test articles, the controls and the pre-registrable decision threshold for the experiment
that would falsify the ranking.
...
the complete bounds on each claim are deposited at doi:10.5281/zenodo.22061075, cited in the
