...
| `R5` pose (`C14` `C15`) | `V3` | INCONCLUSIVE — ⛔ and its site arm is now **0 of 14 in regime, by two independent transfer routes**, while the pose itself is **not singular** across 6 same-method poses | **no usable answer, and it got worse on 2026-08-03** |
| `R6` ΔG_open | — | — | ⛔ **HOLE** |
...
**23 items.** Status: ✅ **frozen** · ⚠ **CONTESTED** (a defensible alternative would change a verdict) ·
...
| **R5** the binding pose is right | `V3` | `C14` `C15` | **`C14`** — the protocol ceiling reads 2.849 Å against this 2.0 Å line, so `V3`'s INCONCLUSIVE is a verdict *at this criterion* |
...
1. ⛔ **`V14`'s "12.5 % druggable" is orthogonal in its SAMPLING and not in its DETECTOR — and this page said
   ...
   independent cross-check **together and in the same direction**. An orthogonal axis that shares a detector
...
| **Q3** | ★ **Carry the anti-handle set as a design CONSTRAINT, not as an after-the-fact report** — reject any construct whose reach envelope admits NR4A1 C505/C551 or NR4A2 C534. ⛔ **`S15` appeared ZERO times in this file before this pass**, and it is the register's cleanest positive control | [`S15`](../modalities/selectivity-mechanism-options.md) **B** | `R8` `R15` | **⊕ CMP** + **⇢ PRE** (it tightens `Q1`, `Q2`, `Q4`, `Q5` and rung `5b-T`) | ○ | **—** | **$0** — the closure data is already committed; only the constraint is missing | it is a **filter**, so its failure mode is its result: **no committed construct survives it** ⇒ the enumeration has been optimising reach TO C397 while admitting the paralogue liability that actually closes the window in 30 of 30 graded cells |
...
| `Q3` the anti-handle constraint | `Q1` `Q2` `Q4` `Q5` and rung `5b-T` | it **strictly tightens** every construct-level row and can never widen one |
...
| **5** | Rung `5b-T` is [row 1](#101--open-rows-ordered-by-what-unblocks-the-most), the program's largest open gap at $0, with **one** stated blocker cleared (the library ruling) | **ITS THREE ARMS ASSEMBLED. THAT IS NOT THE SAME AS PASSING.** All three arms return `ok: true` with snap masks inside 1 Å of their reference atoms — and the run then **died in its single end-of-run writer**: `resolve_e3_binary` returned a raw RDKit `Mol`, `json.dump` streams, and it had written part of the document before raising, leaving the file cut mid-key. The `if: always()` publish step committed the truncation (run **30778084770**) | ⭑ **THE ARTIFACT NOW PARSES AND SAYS SO ITSELF — and it refuses to be read as a verdict**: `⛔_TRUNCATED_ARTIFACT.state = "PARTIAL — RECOVERED FROM A TRUNCATED WRITE"`, *"the run's roll-up fields were never written and have NOT been reconstructed here; a repaired artifact that filled them in would be a record no run produced."* ⇒ **`5b-T` has assembled inputs and NO gate verdict.** ⚠ **Assembling is not passing**, and a row that reads *"it ran"* would be [§4b](#4--the-dependency-graph)'s populated-field-is-not-a-measured-field failure exactly |
...
- ⛔ **And the ceiling it cannot raise, which travels with every `Q1` score:** the paralogue *relocates* these
  ...
  no window.
- ⛔ **What is NOT started, and it is the most useful negative here:** rung **`5b-T`**. It reads as the obvious
  move — largest open gap, $0, one blocker cleared today — but its **site 1 is the pose row 4 is about**, and
  its assembled-inputs artifact is a **recovered truncation carrying no verdict**. Running it now would spend
  free compute producing a result conditional on two things that are about to change.
...
- ⭐ **AND THE BOARD IS NO LONGER 29 ROWS — [§10.1a](#101a---the-option-queue--q1q22-the-graded-family-promoted-to-rows-you-can-start-from)
  ...
  - ⛔ **And the shape of the free tier is now decisive, which is the point of counting it.** §10.2 already
    ...
    the board carries **10 free §10.1 rows plus 22 free `Q` rows against 10 money-gated rows**. **A program
    that reads as blocked on spend is, on its own board, blocked three-to-one on nobody having done the free
    thing.**
