...
**Either outcome is decision-relevant, which is what makes it worth doing before anything else:**

- **fpocket recovers and the transfer does not** ⇒ the defect is site selection, it is fixable at $0, and every
  pose-conditional row on this board is re-runnable rather than dead.
- **neither recovers** ⇒ this protocol cannot recover a known pose on NR4A3's own fold, and every
  pose-conditional claim in the program is bounded by that — a real finding, and the one the known-answer-audit
  framing ([`paper-framing-options.md`](paper-framing-options.md) `P1`) is actually about.
