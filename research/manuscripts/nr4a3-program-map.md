...
| [`GPU economics`](#gpu-economics-full-provenance-in-pricingmdcomputepricingmd) | a pointer to [pricing.md](../compute/pricing.md), plus the six cost levers | `bid-strategy.md` names it |
...
| **V3** | Ligand pose prediction (dock + MM-GBSA) | recover a known holo pose in a nuclear receptor from apo | **INCONCLUSIVE by its own pre-registered rule** — the C1 holo self-dock control failed through the pipeline's own box on **6 of 6 pairs across 3 receptors** (17.3–29.3 Å), so the primary arm measured the *site*, not the docking. With an fpocket-chosen box the same protocol lands in `C14`'s **PARTIAL** band — past `recovered ≤ 2.0 Å`, inside `partial ≤ 4.0 Å` — with **fnat 0.778** clearing the 0.5 secondary. ⛔ **THE BAND IS THE READOUT AND THE DIGITS ARE NOT** (2026-08-03, §1 rule 1): `nr4a3_warhead.dock_into` passes smina no `--seed`, so this arm is a re-seeded Monte-Carlo search and its RMSD is a DRAW. It has been committed at **3.122 / 3.437 / 3.464 / 3.503 / 3.04 / 3.142 Å** across six CI runs of identical code — and this page typed the latest draw as the measurement twice, which is the drift `C6` was built to catch. `C6` measures what is stable: **`max_spread_A` 0.434 Å, `all_bands_stable: true`** over five explicit seeds, so PARTIAL survives re-seeding even though no digit does. Current draw and every arm: [`apo-pose-recovery.json`](../modalities/apo-pose-recovery.json) → `verdict.blind_arms_each_against_its_own_control`, never re-typed here | it cannot grade the docking: the protocol ceiling itself MISSED — `C1c_self_dock_holo_oracle_box` sits in `C14`'s **PARTIAL** band against a `recovered ≤ 2.0 Å` criterion, i.e. this protocol cannot reach 2.0 Å on this system even docking into the receptor the ligand was solved in with the box on the ligand itself. ⚠ **The digit is a draw here too** — this page carried **2.849 Å** while the artifact read **2.845 Å**, an un-noticed re-typing of a number nothing re-read. ⭑ **AND THE ARM'S OWN CONTROL WAS NEVER ON THIS PAGE AT ALL:** the fpocket arm is graded by `C1_self_dock_holo_fpocket`, which also misses — so the 'docking is fine' reading rested on an arm whose own control this page never quoted. ⛔ **AND THE SITE HALF IS NOW MEASURED IN REGIME, AT n = 14 AND BY TWO INDEPENDENT ROUTES (2026-08-03):** over every gradeable apo/holo pair on the three proteins the pipeline actually transfers Pocket-5 onto, the **sequence** transfer put the crystallographic ligand inside its own box in **0**, the **Pocket-5 structure** transfer in **0**, and an fpocket-chosen box in **11**. Two independent site routes, both zero — so *"the site selection missed"* is no longer an n = 6 observation ([`apo-pose-site-in-regime.json`](../modalities/apo-pose-site-in-regime.json)). ⚠ It is a **geometric site supplement**, emits no RMSD, and by its own rule **cannot change the pre-registered INCONCLUSIVE** | ✓ **complete — and now for the first time ON ITS OWN FOLD.** ⛔ **The instrument's failing stage is IDENTIFIED and it is SITE SELECTION, not docking:** in-regime, 11 NR4A pairs, the Pocket-5 transfer docks **+16.05 Å worse (median) than this protocol's own ceiling on the same pair, 11 of 11**, while an fpocket box using no NR4A3 information sits at **+0.39 Å** and beats the oracle on 3 of 11. ⛔ **AND `C14` CANNOT GRADE THIS FOLD** — the ceiling's median is 3.147 Å against a 2.0 Å line it clears once in 11, so the band counts print ungradeable for a reason about the CRITERION. ⚠ **`✓ complete` was WRONG before this ran** and is earned now ([§3.4a](#34a--the-family-positive-control-existed-the-whole-time)) | `R5` `R8` |
...
| **R5** the binding pose is right | `V3` `V22` | `C14` `C15` | **`C14`** — the protocol ceiling lands in the PARTIAL band against this `recovered ≤ 2.0 Å` line (digit not re-typed here; it is a re-seeded draw — see [`V3`](#31--the-instrument-table)), so `V3`'s INCONCLUSIVE is a verdict *at this criterion* ⚠ **AND `V22` INHERITS THE SAME `C14`, DELIBERATELY** — a second method graded by a different line would not be a check, it would be a different question. So `C14` moving still moves BOTH readings of `R5` together, exactly as it moves `R14`'s. |
...
4. ⛔ **"The docking is fine — 3.04 Å" is a PARTIAL recovery under the panel's own preregistered line, and
   this page reported it as a pass.** `C14` sets `recovered ≤ 2.0 Å` and `partial ≤ 4.0 Å`; the arm is in the
   ...
   ⭑ **AND THE CRITICISM ITSELF QUOTED A DRIFTING DIGIT, WHICH IS THE DEEPER VERSION OF THE SAME BUG
   (2026-08-03).** `3.04 Å` is one draw of an unseeded Monte-Carlo search; the arm has been committed at
   **3.122 / 3.437 / 3.464 / 3.503 / 3.04 / 3.142 Å** across six CI runs of identical code, and this page
   typed the newest draw each time — once already carrying a *"Superseded, retained: 3.46 Å"* note beside
   it, which is the drift being **recorded and then repeated in the same sentence**. What `C6` measured is
   what may be quoted: the **band is PARTIAL on every seed** (`all_bands_stable: true`, `max_spread_A`
   **0.434 Å**). ⛔ So the rule for this arm, everywhere on this page: **quote the band, link the artifact,
   never type the RMSD.** Two more digits fell to the same audit — the ceiling at `2.849` against the
   artifact's `2.845`, and the fpocket arm's own control (which also misses) never appearing here at all.
...
### 3.4a · The family positive control existed the whole time

⭑ **AND THIS PAGE RECORDED IT AS NON-EXISTENT.**

**(trimcrae, 2026-08-03: *"And fix the map if it's so blatantly broken like you describe"*, after asking what
we are actually lacking structurally.)**

⛔ **THE SENTENCE THAT WAS WRONG, AND IT IS ON THIS PAGE IN SEVERAL FORMS:** that no known answer exists to
calibrate pose prediction here. **It is true of NR4A3 and FALSE of the family, and nothing separated the two.**

| | |
|---|---|
| NR4A3 itself | **no ligand-bound structure exists.** The only experimental NR4A3 data is the **apo** solution-NMR ensemble 8XTT (20 conformers). So there is genuinely no NR4A3 pose to recover — that half was right |
| NR4A1 / NR4A2 | ⭑ **14 gradeable apo/holo pairs with deposited crystallographic ligand positions**, at **60.0 %** / **65.6 %** aligned identity to NR4A3 — 11 NR4A1 pairs against apo `4RZF`, 3 NR4A2 pairs against apo `1OVL`. Real known answers, on NR4A3's own fold, already sourced and already in a committed artifact |

**How a resource this size stayed invisible: the two modes that could have used it each held one half.**

| mode | in regime? | docks? | what it therefore could not answer |
|---|---|---|---|
| `MODE=run` (the pre-registered panel) | ✕ mostly **not** — its rank order picked PPARG / RXRA, and `pair_questions` writes an `OUT OF THE PIPELINE'S REGIME` disqualifier on them itself | ✓ | whether the protocol works **on the NR4A fold** |
| `MODE=site` (the in-regime supplement) | ✓ all 14 NR4A pairs | ✕ geometric containment only | whether a pose is **recovered** — it emits no RMSD by design |

⇒ The docking answer on the family was never computed, by either mode, and **neither mode's artifact says so** —
which is why `V3` read `✓ complete` while its decisive in-regime test did not exist. **An instrument is not
complete when the test that would grade it has never been run in its own regime**, and [§0.2](#02--work-state--the-five-glyphs)'s
bar already said that; the row simply was not held to it. `V3` is now ◐.

**What the site half already established, and it is the reason this matters** — over those same 14 pairs
([`apo-pose-site-in-regime.json`](../modalities/apo-pose-site-in-regime.json)):

| route that drew the box | put the crystallographic ligand inside it |
|---|---|
| the pipeline's **sequence** transfer of NR4A3 Pocket-5 | **0 of 14** |
| an independent **structural** (CE superposition) transfer of the same site | **0 of 14** |
| the receptor's own **fpocket top pocket**, using no NR4A3 information | **11 of 14** |

⚠ **And the usual escape hatch does not apply.** Apo→holo movement at these sites is **1.2–2.5 Å** Cα RMSD and
both routes read the *same* apo coordinates, so "the site is cryptic and shifts" cannot produce a 0-vs-11 split.
The artifact also checks, per pair, whether the depositor declared the ligand allosteric and whether an
engineered residue shapes the pocket — neither excuses these misses.

⇒ **Every downstream NR4A3 number was computed in a box drawn by a route that has never once, in 14 in-regime
tries, contained a known NR4A ligand.** That is the single largest identified defect on this board, it is
**ours rather than the field's**, and it costs **$0** to test.

**What is now running.** `MODE=regime_dock` (added 2026-08-03) docks the same in-regime list with the full arm
set — pipeline box, fpocket box, oracle box — **each against the self-dock control that goes through the same
site route**, so "the site was wrong" and "the search was wrong" can never borrow each other's evidence.
Artifact: `apo-pose-regime-dock.json`. ⛔ **It is a supplement and cannot become the panel**: separate candidate
list, separate file, no panel-level `verdict()` call, and `apo-pose-recovery.json`'s INCONCLUSIVE is untouched.
`R2b` **is** enforced (this mode docks, and a non-covalent dock cannot reproduce a covalent pose), which costs
the three NR4A2 pairs — the closest paralogue — and that is the honest price rather than a reason to drop it.

### ⭑ IT RAN. **THE SITE IS THE ENTIRE FAILURE, AND THE DOCKING IS AT ITS RESOLUTION LIMIT.**

[`apo-pose-regime-dock.json`](../modalities/apo-pose-regime-dock.json), CI run
[30846497928](https://github.com/trimcrae/Rare-cancers/actions/runs/30846497928), 2026-08-03, **$0**.
18 candidates → **11 graded** (3 excluded covalent by `R2b` — all three NR4A2 — and 4 unreadable at the RCSB
fetch, which is a file-format bias toward older entries and is recorded, not hidden).

⛔ **FIRST, THE THING THAT DECIDES HOW EVERYTHING ELSE IS READ: `C14` CANNOT GRADE THIS FOLD.** The protocol
**ceiling** — the holo receptor the ligand was solved in, box centred on the ligand itself, i.e. the best this
protocol can possibly do — has median **3.147 Å** across the 11 pairs and clears the `recovered ≤ 2.0 Å` line
on **1** of them. So *every* blind arm prints "ungradeable", and that is a fact about the **criterion**, not a
null result. ⭑ **This is [`C7`](#3b2--contested-and-known-defective--the-four-that-are-not-merely-frozen)'s
lesson arriving at a second threshold** — and the defensible form is the same shape it was there: not the
band, but each arm against **its own pair's ceiling**, which needs no criterion at all.
⚠ **`C14` IS NOT MOVED.** It also grades `V21`, so it is not `R5`'s to move, and moving a pre-registered
threshold after seeing the answer is the tuning the module forbids. The band counts are still reported beside
this.

**Against its own ceiling, the two arms separate completely — 11 of 11 pairs, no threshold involved:**

| box drawn by | blind-from-apo, minus this protocol's own best on the SAME pair | reads |
|---|---|---|
| **fpocket top pocket** (no NR4A3 information used) | **median +0.39 Å** · range −0.52 … +21.28 · ⭑ **beats the holo oracle outright on 3 of 11** | the docking is **at its resolution limit** — handing it the crystallographic answer buys ≈ nothing |
| **the pipeline's Pocket-5 transfer** | **median +16.05 Å** · range +10.50 … +23.23 · **11 of 11 worse** | the box is the failure, and it is not marginal |

Raw blind RMSDs, for the same 11: fpocket **2.104–27.695 Å, median 3.243**, with **7 of 11** inside `C14`'s
`partial ≤ 4.0 Å`; the Pocket-5 transfer **17.971–29.646 Å, median 19.344**, with **0 of 11** inside it.
The four fpocket outliers (`3V3Q`, `4KZI`, `4RZG`, `6KZ5`) are where fpocket picks the wrong cavity — three of
them are the same entries the geometric supplement already recorded as `SITE MISSED`, so the two instruments
agree on which pairs are hard.

⇒ **THE FINDING.** *Docking is not what is broken.* Through a box chosen with **no NR4A3 information at all**,
blind from apo, this protocol lands within half an ångström of its own ceiling on the NR4A fold. The
pipeline's Pocket-5 transfer is **+16 Å worse than that, on every pair tested** — which is the same defect the
geometric supplement measured as 0 of 14, now confirmed with docking and quantified against a control.

**What it licenses, and what it does not:**

- ✅ **The site-selection step is a demonstrated defect, in regime, by two independent instruments, and it is
  ours to fix at $0.** Every downstream NR4A3 number was computed in a box drawn by that step.
- ⛔ **It does NOT say the NR4A3 pose is 2 Å-correct once the site is fixed.** The ceiling *is* ~3 Å here, so
  even a perfect site leaves this protocol placing ligands to about that — which is **larger than the
  differences the selectivity screens were being asked to resolve**, and that bound now travels with them.
- ⛔ **It re-opens no ✕ row.** The NR4A1-sparing covalent inverse stays dead on its definitional argument;
  NR-V04 stays disqualified as a *selectivity* control by a confound in the test system. This is one
  instrument's calibration, which is the only thing a known-answer test can ever be.
- ⭑ **And it is exactly the `P1` known-answer-audit paper's thesis, sharpened**: the known answer existed in
  the repo the whole time, the instrument read `✓ complete` without it, and when finally run it found the
  defect in the step nobody was checking.

⛔ **What this does NOT do.** It re-scores no leg, moves no ΔΔG, and revives nothing that is ✕ under
[§6a](#6a--dead--conclusively-unworkable-never-retry) — the NR4A1-sparing covalent inverse stays dead on its
definitional argument, and NR-V04 stays disqualified as a selectivity control by a confound that lives in the
test system. It is a statement about **one instrument**, which is the only thing a known-answer test can be.
...
#### ⭑ IT RAN — and NR4A2, the arm carrying the program's ONLY resolved selectivity claim, is docked outside its own ligand site

[`paralogue-site-correction.json`](../modalities/paralogue-site-correction.json), CI run
[30852027942](https://github.com/trimcrae/Rare-cancers/actions/runs/30852027942), 2026-08-03, **$0**.
⚠ **Third run.** The first two were REFUSED by the agreement gate rather than turned into a number; both
failures and their diagnoses are below, because the instrument being wrong twice is part of what this
result is worth.

| paralogue | deposits | agreement | verdict |
|---|---|---|---|
| **NR4A2 / Nurr1** (`P43354`) — ⭑ **NR4A3's CLOSEST paralogue, 67.3 % LBD identity** | 3, all covalent at Cys566 | ✅ **1.5 / 2.3 / 3.4 Å** from consensus | **MEASURED: the pipeline docks it 15.472 Å from where its ligands bind, and that site is OUTSIDE the 24 Å box it draws** — so no amount of searching inside that box could ever find it |
| **NR4A1 / Nur77** (`P22736`) | 11 | ⛔ **max 22.06 Å** — gate refused | **UNMEASURED**, and the refusal is itself a finding — see below |

⭑ **WHY THE NR4A2 ROW IS THE ONE THAT MATTERS.** The program's *only* statistically-resolved selectivity
number is ΔΔG(NR4A3−NR4A2) = **−4.98 ± 0.68**, 95 % *t* **[−6.67, −3.29]** — the paper's "resolved below
zero" arm, and the reason `denovo_401` is described as **NR4A2-sparing** at all (the NR4A1 interval spans
zero and is not claimed). **That arm's receptor was docked 15.5 Å from NR4A2's ligand site, outside the
search box.** ⚠ This does not make the number wrong; it means **the comparison was never made** — a real
NR4A3 pocket was compared against a box that does not contain NR4A2's binding site.
⚠ **Caveat that travels with it:** all three NR4A2 deposits are covalent at the same cysteine, so the
consensus is well-defined but **chemotype-narrow** — it is where covalent ligands attach, on three
structures, not a survey of everything that binds Nurr1.

⛔ **AND NR4A1'S REFUSAL IS A SEPARATE, DEEPER PROBLEM THAN MISPLACEMENT.** Its 11 deposits do not define
one site: **7 of 11 cluster within 1.4 Å** of their own centre (the dominant site), **3 more sit 7.8–14.3 Å
away**, and one (`6KZ5`, the only chain-B entry) is 26.5 Å out. ⇒ *"the NR4A1 box"* is **under-specified
regardless of where it is placed** — a single-box anti-target model may be the wrong model for this
receptor, not merely a mis-placed one. **No dominant-site statistic is computed here**: choosing one after
...
1. **Run 1** emitted corrected sites for both paralogues and a clean 22.8 / 24.7 Å displacement — from
   ligand positions scattered 8.8–26.8 Å and 28.6–55.2 Å around their own consensus. **The agreement data
   was already in the artifact and the summary did not read it**, the identical failure shape to
   [§3.4a](#34a--the-family-positive-control-existed-the-whole-time)'s first headline. The gate exists
   because of that, and it refused both.
2. **Run 2** tested the wrong hypothesis. I attributed the scatter to a *register error* — an LBD-only
   crystal mis-placed inside AlphaFold's full-length model — and bounded the reference to the LBD. It
   worked exactly as designed (span 361–598, identity **0.987–1.000**, CE RMS **0.7–1.2 Å**) and returned
   **coordinates identical to the untrimmed run**. ⇒ hypothesis **REFUTED by its own fix**; retained in
   the code as a guard, marked as not-the-cause so it cannot be re-cited.
3. **Run 3** found it, and the tell was arithmetic rather than a guess: the separation was bimodal at
   **~49 Å**, and a nuclear-receptor LBD is only **~40 Å** across — so two ligands 49 Å apart cannot sit on
   one monomer. They were on **different chains of a crystallographic dimer**: the code took the largest
   ligand *copy* while superposing the *whole* deposit. Selecting the chain the ligand actually touches
   put `4WHF` and `4WHG` — consecutive depositions from one paper, previously **49.4 Å** apart — **1.7 Å**
   apart. ⛔ **The helper for this already existed and was tested** (`apo_pose_recovery._chain_nearest`,
   *"the chain the ligand actually binds"*), and is exactly why **that** module's site arm worked at 11 of
   14 while this one's did not. **Bypassing a tested helper was the whole defect** — the third time in one
   day that the guard which would have caught something already existed and was not the one that ran.
...
graph TD
  ...
  Q2 -->|"INCONCLUSIVE — the control<br/>failed on 6 of 6 pairs"| SPLIT["The question was TWO questions.<br/>Docking: PARTIAL band, fnat 0.778.<br/>Site selection: 0 of 14 in regime"]
...
| **Q14** | **Re-query DepMap for CRISPR data on `ACH-001519` / H-EMC-SS** | [route 9](target-route-options.md) ⏸ | — (a route gate) | **⇢ PRE** | ○ | **—** | **$0 CI** | still no CRISPR data since 24Q4 ⇒ the synthetic-lethal route stays parked **on data, not on ideas**, and that is a measured absence rather than an unasked question |
...
2. **The specific defect is one this program can fix for free and cannot fix any other way.**
   ...
   already **0 of 14** on two independent transfer routes — and the in-regime DOCKING half had never
   been run at all until `MODE=regime_dock` ([§3.4a](#34a--the-family-positive-control-existed-the-whole-time)).
...
- **`[x]` 5b-T · Rebuild the NR4A1/2/3 ternaries by the ASSEMBLY route, from a SMILES-recorded degrader, then
  ...
  **carries the measured collision bracket** instead of claiming the 12-atom figure.
  ...
  convention on the outcome** ([§3b.2](#3b2--contested-and-known-defective--the-four-that-are-not-merely-frozen)). So arm (C)
...
state, cost and `$/ns` for every one of them: the [**⏱️ IN FLIGHT**](#-in-flight--what-is-actually-running-right-now-as-of-2026-07-30-530-pm-et) block on this page, which is their
