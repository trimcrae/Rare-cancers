...
| `R5` pose | `V3` | INCONCLUSIVE — ⛔ and its site arm is now **0 of 14 in regime, by two independent transfer routes**, while the pose itself is **not singular** across 6 same-method poses | **no usable answer, and it got worse on 2026-08-03** |
| `R6` ΔG_open | — | — | ⛔ **HOLE** |
...
| `R8` linker reach | `V17` + enumeration | `V17` fails its own positive control | rank-only, and conditional on `R5` — ⭑ **but the screen now has a measured cross-system background at the 12-atom gate**, reach-only AND exposure-filtered ([`categorical-decoy-null.json`](../modalities/categorical-decoy-null.json)); ⛔ its NR4A3 arm does **not** contain C397 |
...
| `R14` scope bound (AR/MR) | `V21` — ⛔ **assembled 2026-08-03, and it FAILED its own known-answer test** | ⛔ **`panel_readable: false`** — 7 of 10 receptors recover their cognate ligand, 3 do not | ⛔ **NO LONGER A HOLE — it is a FAILING INSTRUMENT, which is a different and worse thing.** A hole means nothing has been built; this means the thing that was built cannot be read, and four SI §S1 clauses already rest on it. ⚠ *Superseded, retained: "~8/9 built — the parts exist and were never assembled … **HOLE**".* |
...
- ⛔ **BUT THE PERCENTILE MAY NOT BE QUOTED FOR C397, AND THIS IS NOT A DETAIL.** The pre-registered pLDDT ≥ 70
  trim keeps UniProt **427–570** of the NR4A3 model, so of the committed unique set only **C559** is inside the
...
- ★ **A new mechanism grades above every non-incumbent option on this page: STERIC EXCLUSION (negative
  ...
  - ⛔ **AND THE AMBIGUITY IS NOT LOCAL TO THE GENERATION FRAME — IT IS ENDEMIC, WHICH ONLY THE FULL CENSUS
    ...
    **under a choice rule**, never under an unambiguous site. One home:
...
**(2)** ⛔ **The NR4A3 arm through that harness scores C559, not C397.** The pre-registered pLDDT ≥ 70 trim keeps
...
| **10** | **`R14` — the AR/MR superfamily cross-binding check** — now rung **`R14-a`/`R14-b`**, with **`R14-c` closed on the claim-ceiling rule**, in [the ordered plan](#rung-s--the-two-scope-rungs-r13-r14-claim-ceiling-conditions-deliberately-off-the-cum-chain) | `R14` | ◐ **`R14-a` ran 2026-08-03 · `R14-b` not started (⛔ DO-NOT-LAUNCH)** | **`R14-a`: —** ($0, needs no nod) · **`R14-b`: 🔒 + ⛔ blocked by the rate line** | ✅ **PRICED and GATED, 2026-08-03.** `R14-a` **$0**; `R14-b` **~$3.41** ($1.10–18.65, 29.87 ref-GPU-h, band 23.2–64.7) — DERIVED from the MEASURED LANE-13 metadynamics rates, not from the card table ([`scope-rung-cost.json`](../modalities/scope-rung-cost.json)). **Excluded from the pinned ladder total** | ⭑ **`R14-a` IS AN ASSEMBLY JOB, NOT A BUILD, AND IT IS THE HIGHER-VALUE HALF** — and on **2026-08-03 it RAN, at $0, and its self-control FAILED, which is precisely why that mattered.** ⛔⛔ **A SENTENCE ALREADY IN THE SI IS NOW UNREADABLE.** The panel is complete (MR/NR3C2 added by a live RCSB query under written rules, and it **passes**) and its never-run cognate-ligand self-control ran for the first time: **7 of 10 receptors recover their own cognate ligand, 3 do not** — CYP3A4, PXR and PPARG — so `panel_readable: false`, and **all four SI §S1 anti-target clauses are graded `readable: false`** because each is a *maximum* or *every-survivor* statement over the whole panel. ⛔ **A second, independent block:** the NR4A3 ΔG column those margins subtract is **not committed anywhere in this repo**, so even a passing control leaves them non-re-derivable ([`antitarget-selfcontrol.json`](../modalities/antitarget-selfcontrol.json); instrument row [`V21`](#31--the-instrument-table)). ⚠ **THE ROW'S `◐` NAMES THIS AND NOTHING ELSE, per [invariant 5](#05--six-invariants--structural-not-stylistic): the in-work item is `R14-a2`, the preparation repair, on free CPU. Nothing on this row is billing, and `R14-b` remains ⛔ DO-NOT-LAUNCH.** **NEXT ACTION — repair the receptor PREPARATION, not the criterion**, at $0: the frozen rule forbids the easy exits (a failing target may not be dropped, its box may not be re-centred, no band may be lowered), and the panel is protein-only by construction, which the artifact already names as the limit any cofactor-dependent binding mode falls outside. ⚠ *Superseded, retained: "`R14-a` IS AN ASSEMBLY JOB, NOT A BUILD, AND IT IS THE HIGHER-VALUE HALF … the panel's **cognate-ligand self-control has never been run**".* It was the higher-value half, and that is exactly why it now blocks a published number. ⛔ `R14-b` is **registered as blocked**: its `$0.022758/ns` is **3.48× the approved buy line**, but the comparison is *not* like-for-like (a biased leg against an unbiased benchmark), and **no metadynamics-anchored basis exists** — a decision for trimcrae, surfaced now rather than at launch. `R14-c` (the FEP half) is **not costed**: it is `V4`'s instrument, so it is downstream of row 2, not parallel to it |
...
- **10 rows could start today with no authorization and no bench** — **1**, **9 (`R13-a`)**,
  **10 (now `R14-a2`, the preparation repair — `R14-a` itself RAN)**, 25, 26, 27, 28 at **$0**, 4 cheap, and
  18 is the same purchase as 1 — plus rows **5**, **24** and **29**, which **were** on this list and are now
  **done**. ⭑ **Row 4 is the one that changed rank, not the count:** two $0 results on 2026-08-03 made the
  pose/site question the row that unblocks the most, and it is cheap CPU rather than a spend.
