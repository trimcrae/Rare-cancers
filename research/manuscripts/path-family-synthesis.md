...
| **3** | **Widen the categorical enumeration** — 35 unique alignment-robust positions across 11 reactive classes, 31 within linker reach. ⚠ **The sweep cuts both ways:** under the `V17` cutoff **no new handle clears at all** — the credible set collapses to the cysteines and lysines already committed. Under the **threshold-free rank** the roadmap mandates instead, **Y419** (SuFEx tyrosine, RSA 0.221, one residue from C420) sits *above* NR4A1 Cys551 (0.165) — the family's one literature-anchored covalent site and the very false negative that discredited the cutoff. **M398/M399 fall below the reference on both readings and are dropped, not carried.** | Route B's single point of failure is a **gap in the enumeration**, not a fact about the protein — but the new handle is real only on the ruler the program permits, and absent on the one it has refused | $0 | chemistry credibility is a literature label, not a computed quantity; SuFEx tyrosine is precedented rather than routine, and every added handle re-opens the chemoselectivity-window question |
