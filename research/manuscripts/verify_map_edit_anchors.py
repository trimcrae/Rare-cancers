#!/usr/bin/env python3
"""Verify `three-row-audit-map-edits.json`'s routed edits against the LIVE documents. ($0, pure stdlib)
...
ARTIFACT = os.path.join(HERE, "three-row-audit-map-edits.json")
...
def main(refs=("origin/main", "WORKTREE")):
    ...
    for e in edits:
        for ref in refs:
            ...
            print("%s %3s %-11s %-11s %s" % ("OK  " if ok else "FAIL", e["id"], ref, st,
                                             e["file"].split("/")[-1]))
    print()
    print("%d edits x %d refs -- %d unresolved" % (len(edits), len(refs), bad))
