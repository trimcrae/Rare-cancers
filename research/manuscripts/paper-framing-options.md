...
| **A blind-docking benchmark measures the box, not the docking** | the holo self-dock control failed through the pipeline's own box on **6 of 6 pairs across 3 receptors (17.3–29.3 Å)**, while the same protocol through an fpocket-chosen box lands in `C14`'s **PARTIAL** band with **fnat 0.778**, 7 of 9 native contacts. Verdict **INCONCLUSIVE by its own preregistered rule**. ⛔ **The band, not a digit — the arm is an unseeded Monte-Carlo search and its RMSD has been committed at six different values across six runs of identical code; `C6` measured the BAND to be the stable readout** (`all_bands_stable: true`, `max_spread_A` 0.434 Å). ⭑ **And the site half is now measured in regime: 0 of 14 by two independent transfer routes, against 11 of 14 for an fpocket-chosen box** | [`apo-pose-recovery.json`](../modalities/apo-pose-recovery.json) + [`apo-pose-site-in-regime.json`](../modalities/apo-pose-site-in-regime.json) (`V3`) |
| ⭑ **A known answer that EXISTED and was never used — the instrument read `✓ complete` anyway** | the program recorded, in the roadmap and in every status report, that no positive control exists for pose prediction here. True of NR4A3 (no ligand-bound structure) and **false of the family**: NR4A1/NR4A2 carry **14 gradeable apo/holo pairs with deposited crystallographic ligand positions** at **60.0 % / 65.6 %** identity, already sourced into a committed artifact. They were never docked into because the two modes that could have used them **each held one half** — one docks but chose out-of-regime receptors, the other is in regime but runs no dock — and **neither artifact said the answer was missing** | roadmap §3.4a; `apo-pose-regime-dock.json` |
