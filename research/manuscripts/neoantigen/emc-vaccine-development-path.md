...
UniProt reviewed human proteome including isoforms; the unreviewed entries of the same reference
proteome are not searched by the committed artifact, and B5 records what that leaves open. Clinical
figures come from a curated EMC registry. No wet-laboratory data were generated.
...
peptide-allele calls across 6 alleles, all of them from the single 31-residue tract, and the three
...
a best in-frame call of 0.3736; five of the ten fall below 0.2755. Section 2.3 reports that a
...
self peptide at a position that does not contact the receptor can still be cross-recognised. **The
unreviewed half of the reference proteome is not searched by the artifact this section rests on, and
the counts previously printed here for it are withdrawn.** Earlier versions reported that 12 of the
170 reviewed-novel peptides occur in at least one of 127,090 unreviewed entries, and that none of the
12 is a predicted binder. Those three counts have no committed artifact behind them: the unreviewed
pass is an optional argument of `junction_proteome_novelty.py`, and `junction-proteome-novelty.json`
records `trembl_included: false`, so it holds no unreviewed result to check them against. They are
withdrawn rather than requoted, and the state of that search is unknown here rather than negative.
Running it would settle less than the withdrawn sentence implied in either direction: a hit among
predicted-and-unreviewed entries is not evidence that a normal protein carries the peptide, and a
miss there is not evidence of absence, so it could withdraw no peptide and confirm none. What stands
is the reviewed search. It excludes one specific failure mode and leaves the others standing.
...
point values on samples as small as 450 individuals (Table 1, Polynesia), and the frequencies are pooled across
...
as a JSON artifact beside it, with one exception, named because a blanket claim with an exception in
it is worse than no claim: the unreviewed-proteome counts B5 previously printed have no committed
artifact, and B5 withdraws them rather than resting on a script run nobody can check. Everything
below is deposited. The junction and coverage figures come from `fusion_breakpoints.py`
...
one-residue pre-screen of Section B5 and its validation), `junction_transcript_sensitivity.py` (the
sweep over every protein-coding transcript pair that Section B5 and Section 7 report),
`patient_neoepitopes.py` and
...
`novelty-seam-test.json`, `junction-transcript-sensitivity.json`, `patient-cd4-demo.json` and
`vaccine-construct.json`. The four tables in
this paper — Tables 1 to 4 — are generated from those artifacts by `vaccine_path_tables.py` and a
build fails if a cell and its source diverge. The predictor versions those artifacts record are MHCflurry 2.1.4 with
...
**Use of AI tools.** Large language models were used throughout this work — Claude (Anthropic)
throughout, and OpenAI models for part of it — to write the analysis code, to run the screening
pipelines, to draft and revise this manuscript, and to conduct the internal adversarial review of
earlier drafts whose findings this version incorporates. The model
...
artifacts, with the single exception Section 8 names and B5 withdraws, and the clinical figures are
transcribed from the publications cited for them. Every literature
...
**Ethics.** This work is an analysis of public data. It recruited nobody, sampled no human material,
and administered no intervention to any person or animal; every input is a public database record
(Ensembl, UniProt, the Allele Frequency Net Database mirror, archival expression series) or a figure
transcribed from a published or publicly presented report. Because the work carried out no new
recruitment, sampling or intervention, no ethics approval was sought and none was obtained; this
manuscript makes no determination that approval was or was not required, which is a judgement for a
committee rather than for its author. The clinical figures quoted include individual published case
reports as well as aggregate trial results, and are reproduced as their sources published them.
...
16. Galitskiy E, Livingston AJ, Plair T, Tarasov A, Putintsev V, Novokrestchenova A, et al. P35-1
    ...
    **Conference abstract, one patient, not peer reviewed**; 13 of the 16 authors are affiliated to
    the corporation whose comprehensive-genomic-profiling assay is the subject and the remaining 3 are
    at the treating centre, and the abstract text held in this repository has that company's own
    publication page as its only source. Registry entry
...
## Appendix D. Statements withdrawn on verification against the committed artifacts

The batch recorded here checked each printed quantity against the artifact the manuscript names for
it, and changed the manuscript rather than any artifact, guard or threshold. Three statements did not
survive that check. They are recorded on the same principle as Appendices B and C.

| Withdrawn statement | Why it does not stand |
|---|---|
| Section 2.2, the out-of-frame screen returning "10 strong peptide-allele calls across 8 alleles" | `junction-frameshift-peptides.json` lists ten calls on six distinct alleles — HLA-A\*02:06, A\*33:01, A\*68:01, B\*07:02, B\*35:01 and B\*51:01 — because HLA-B\*07:02 carries four of the ten and HLA-A\*68:01 two. The count of calls was right and the count of alleles was not; the sentence now reads six. No coverage figure in this paper is computed from this panel, so nothing downstream moves. |
| Section 2.2, "four of the ten fall below 0.2755" | Five do: 0.1039, 0.1704, 0.2482, 0.2653 and 0.2745, the last of which is below the stated bound. The bound is the artifact's and the count was miscarried against it. |
| B5 and the Abstract, the unreviewed-proteome counts — 127,090 entries searched, 12 of the 170 reviewed-novel peptides hit, none of them a binder | No committed artifact carries them. The unreviewed pass is an optional argument of `junction_proteome_novelty.py` and `junction-proteome-novelty.json` records `trembl_included: false`, so the deposited evidence contains no unreviewed result at all. Section 8's claim that every figure in Sections 2 and 3 is committed as an artifact beside its script was false in exactly this one place, and now names the exception. The counts are withdrawn and the search is reported as not run here rather than as run and negative. |
