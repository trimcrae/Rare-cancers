...
VENUES = {
    "bioRxiv-preprint": {
        ...
        "provenance": ("⛔ THIS ROW GRADES THE EXTENDED REPORT AGAINST NO VENUE, BECAUSE IT HAS "
                       ...
                       "are in program/preprint-host-decision-round2.md. "
                       "⚠ SO THE REASON THE ROW IS UNGATED HAS CHANGED, AND THE FLAG NO LONGER "
                       "MEANS WHAT ITS NAME SAYS: `no_limits_by_policy` was set because bioRxiv "
                       "publishes no word, abstract or display-item limit — still true of bioRxiv, "
                       "and no longer why this row carries no limits. It carries none because "
                       "nobody has chosen a venue, which is an UNKNOWN wearing the same four "
                       "nulls. The flag is kept so the grader does not report the row as merely "
                       "unread, and this sentence is the only thing separating the two states. "
                       "⚠ Superseded, retained: \'the extended report is archived on Zenodo and "
                       "has since been submitted elsewhere\'. Written 2026-08-23 while the "
                       "Research Square submission was pending, it read as placement and is the "
                       "same stale-external-status defect the preprint checklist already records "
                       "against its own three-day \'⏳ Awaiting bioRxiv screening\' row: nothing "
                       "here re-reads an outside queue, so a status owned by another system decays "
                       "silently. The archive half is unchanged and true — Zenodo "
                       "10.5281/zenodo.22061075 is published. "
                       "⚠ AND CUTTING TO A CAP IS STILL PREMATURE: the surviving candidates "
                       "disagree on length and on whether a structured abstract is wanted, so a "
                       "cut made now is a cut made twice."),
    },
    ...
    #: ⭐ REOPENED 2026-08-20 (trimcrae). ⭐⭐ AND THE LIMITS ARE NOW READ — 2026-08-23, at the
    #: journal's own Submission Guidelines page, by trimcrae in a human web browser.
    #: ⛔ THE SOURCE OF RECORD IS A FILE, NOT THIS ROW: the verbatim capture with its sha256 is
    #: `research/literature/nat-submission-guidelines-2026-08-23.md`, and every NAT number this
    #: repository states cites it. Nothing below is typed from recollection.
    #: ⚠ WHY IT IS NOT IN `venue-policy-browser-fetch.json` LIKE EVERY OTHER READ PAGE: that file is
    #: generated by `scripts/venue_policy_browser_fetch.py`, and no automated fetcher can reach this
    #: one. journals.sagepub.com answers HTTP 403 with a Cloudflare "Just a moment..." body to the
    #: egress proxy, to a plain CI fetch and to a real headless Chromium alike — re-confirmed the
    #: same day by Actions run 32644971152. The page loads normally for a person, and that is the
    #: only route it has. So this row's provenance is a HUMAN READ and says so; describing it as a
    #: fetch would be the "presence is not provenance" defect (CLAUDE.md §4) in its purest form.
    #: ⚠ AND IT DECAYS. A journal's guidelines change without notice and nothing here can re-read
    #: them; the capture date is the whole warranty. Re-read before submitting.
    #: ⛔ `references: None` HERE MEANS "READ, AND THIS ARTICLE TYPE HAS NO REFERENCE CAP" — not
    #: unread. The page caps references where it intends to (Editorial 10, Letters to the Editor 4)
    #: and states none for Original Paper. The row is graded because its other three limits are set,
    #: which is what `limits_read` keys on.
    "NAT-Article": {
        "journal": "Nucleic Acid Therapeutics (SAGE)",
        #: Article type: Original Paper. Verbatim from the capture: "4,000-word limit",
        #: "Unstructured abstract of no more than 200 words", "Maximum total of five (5) figures
        #: and/or tables".
        ...
        "provenance": (
            ...
            "source of record for every figure below. Article type Original Paper: 4,000-word "
            "limit, unstructured abstract of no more than 200 words, maximum total of five (5) "
            "figures and/or tables, and no reference cap for this type. THE COUNTING RULES AGREE "
            "WITH THIS MODULE'S: 'Word limits do NOT pertain to the abstract, disclosure "
            "statements, author contribution statements, funding information, acknowledgments, "
            "tables, figure legends, or references' -- which is the boundary `TAIL` already draws, "
            "so main_words here is comparable to the cap rather than merely near it. "
            "⚠ NOT AUTOMATABLE: journals.sagepub.com returns HTTP 403 to the egress proxy, to a "
            "plain CI fetch and to a real headless Chromium alike (re-confirmed 2026-08-23, "
            "Actions run 32644971152), so this row cannot be refreshed by any tool here and must "
            "be re-read by a person before submission. "
            "⚠ THE PAGE CONTRADICTS ITSELF ON THE REVIEW MODEL and the contradiction changes what "
            "is submitted: 'Identity transparency: Single-anonymized' appears twice, while the "
            ...
            "the Liebert-to-SAGE transfer unchanged at $90 per typeset page, assessed on "
            "acceptance, with colour charged separately at $800 for the first print image. (3) The "
            "control-oligodeoxynucleotide rule is real and is scoped to efficacy: 'All manuscripts "
            "that claim antisense efficacy must, in addition to the antisense oligodeoxynucleotide "
            "construct, include at least two control oligodeoxynucleotides.' This paper claims no "
            "efficacy and runs no assay, so it is out of that rule's scope."
        ),
    },
    "BJC-Article": {
...
#: ⚠ "statements and declarations" IS LISTED SEPARATELY AND MUST BE. Nucleic Acid Therapeutics
#: requires that exact heading, and it does not start with any word already here — so on a
#: paper carrying it WITHOUT an Author Contributions heading above it, every declaration would
#: have counted as main-text prose against the word cap. Today the ASO article is saved by its
#: Author Contributions heading landing first, which is luck, not a property.
TAIL = re.compile(r"^#+\s*(?:(?:declarations?|statements and declarations|data (?:and|&) code|"
                  r"data availability|references|"
...
def main(argv=None):
    ...
    payload = {
            ...
            "⛔_the_limits_below_are_not_verified": "They fall into three states and the difference "
                    "matters. READ: the BJC row was retrieved from the journal's own guide to "
                    "authors at HTTP 200, and the NAT row was read at primary source on "
                    "2026-08-23 by a person in a web browser and captured verbatim to "
                    "research/literature/nat-submission-guidelines-2026-08-23.md — a human read "
                    "is primary-source evidence and is labelled as one, never as a fetch. SEARCH-DERIVED: the two GCC rows and the CROH row come "
