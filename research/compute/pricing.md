...
benched cards, n=3 so each fit is on two): worst error **FP32 +42%**, **bandwidth +74%**, **`dlperf` +44%** — ⚠ *superseded, retained: **bandwidth +116%**, **`dlperf` +28%**, both computed on the retired single-host throughput table; FP32's +41% is unchanged within rounding. The argument survives the correction — the proxies are still poor — but two of its three numbers did not* —
...
> `nr4a3_rbfe.py:602-603`): complex leg **~9.1 GPU-h** (not 3.6), solvent ~4.1, **unit ≈ 13.7 GPU-h**.
...
Both come from the [ternary-selectivity strategy revision](../manuscripts/degrader/nr4a3-ternary-selectivity-strategy-revision-2026-07-24.md);
...
3. **⚠ PROTEIN-MUTATION FEP DOES *NOT* REDUCE TO THESE BASES — CORRECTED 2026-07-24.** This file previously
   ...
   - OpenFE's `RelativeHybridTopologyProtocol` (what `nr4a3_rbfe.py` and `nr4a3_ternary_fep.py` both drive) is a
     **small-molecule** RBFE protocol: it builds its hybrid topology from a **ligand-to-ligand atom mapping**
     ...
     (`nr4a3_rbfe.py:392-398` — the ligand-to-ligand atom map, strict then `element_change=True` for a
     single-point substituent mutation; `rbfe_map.py:30,464`, guarded `single_site`). Nothing in either driver mutates a
     protein residue.
...
- **Ternary edge — a measured RATE, PROJECTED to a leg (2026-07-24; corrected same day).** The `valB_mini` Wurz
  ...
  **per-iteration rate**: 146,509 particles, **12** windows (`gpu-ternary-fep-gcp.yml:30,75` — the code's
  ...
  **⚠ 920 iterations is NOT a finished leg** — the protocol hardcodes 1 ns equilibration + 5 ns production at
  2.5 ps/iteration = **400 + 2000 = 2400 iterations** (`nr4a3_rbfe.py:602-603`; the openmmtools `.chk` history
