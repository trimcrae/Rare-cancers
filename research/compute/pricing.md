...
| **Alchemical ternary cooperativity edge** (3-replica, ~146k particles, 12 windows) | **~$4–7 on Vast 4090** for the full 3-replica edge (~$1.3–2.3/replicate) at a realized ~$0.15–0.25/hr host; ~$10–12 on a pricier ~$0.40/hr reliable host. ~24–30 4090-GPU-h/edge. | **MEASURED directly on Vast 4090** (2026-07-24) — card-ratio projection confirmed | **Direct 4090 throughput** from the firm ternary leg on the Vast rtx4090 nr4a3fep image (fusion-cpu-extras `firm firm_kind=ternary`, via the reusable `run_ternary_leg.sh` at 12 windows), self-staged 8G1Q system, **146,284 particles**: warmup cleared clean (no NaN) and production ran steady at **~14–18 s/iter (median ~16)**, ~**2× faster than L4** (~33 s/iter). A full leg ≈ ~920 iters × ~16 s ≈ **~4–5 4090-GPU-h** (vs the L4÷2.3 projection ~3.8 — **card ratio validated, the earlier "soft spot"**); edge = binary+ternary × 3 replicas ≈ 6 legs ≈ ~24–30 4090-GPU-h. The clean end-to-end ΔG was NOT captured (the instance was spot-preempted late in production before the firm path's MBAR+summary+upload — firm path is `commit store: LOCAL`, `resume=False`, same limitation as the RBFE row), but **ΔG is not the cost basis — throughput is, and it's directly measured**; the ΔG itself comes from the GCP valB production lane (ΔG_morph 47.28, continuous-checkpoint). The earlier direct-4090 attempt that NaN'd was a **pre-fix 16-window fallback** (its preequil NaN'd → ran on the raw complex); the 12-window `run_ternary_leg.sh` lane clears it. Old ~$65–110 (scaled off the refuted 55-GPU-h RBFE anchor) and the interim L4-on-demand ~$37 are both superseded — **Vast only.** |
...
| `valB_mini` ternary | 1 ternary edge | **~$4–7 Vast 4090** (~24–30 4090-GPU-h @ ~$0.15–0.25/hr) | **MEASURED directly on 4090** (~16 s/iter) |
...
few tens of dollars, not hundreds** (`step1_fanout` ≈ ~$12–26); the ternary edge is now **directly measured on
Vast 4090** too (~16 s/iter → ~$4–7/edge, card-ratio-confirmed). The only
