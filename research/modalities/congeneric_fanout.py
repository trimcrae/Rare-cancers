...
so one Vast instance produces one ddG. ~5-6 GPU-h/unit (MEASURED: ~3.6 GPU-h complex leg on a Vast 4090,
instance 45654998; solvent leg is a smaller box).

--------------------------------------------------------------------------------------------------------
SCOPE OF WHAT IS LAUNCHED (read this before quoting a number off this map)
--------------------------------------------------------------------------------------------------------
The frozen map describes THREE multiplicative axes. Only the first is in the priced ~$12-26 `step1_fanout`
...
than silently fanning out 100+ legs:
  ...
  AXIS 1 - EDGES x charge-CONSERVING microstate leg -> 19 units.   [TRANCHE 1 - what `default_units()` emits]
      Every one of the map's 19 edges has exactly one dq=0 microstate leg. 19 units x ~5-6 GPU-h.
...
# MEASURED per-unit GPU-h band (research/compute/pricing.md section A, Vast 4090 instance 45654998):
# complex leg ~3.6 GPU-h (2h52m sampler at ~5.2 s/iter x 2000 production iters + ~43 min boot/setup);
# solvent leg (smaller box) ~1.5-2 GPU-h extrapolated. Band, not a point estimate.
UNIT_GPU_H = (5.0, 6.0)
# Vast 4090 interruptible $/hr band actually realized on this account (pricing.md section E).
VAST_4090_USD_PER_H = (0.12, 0.25)
