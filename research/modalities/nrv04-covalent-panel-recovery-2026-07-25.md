...
**Answer: no — and it does not matter, because the panel's GO was never available to recover.** Four
...
| 3 | **The panel's inputs were CONTAMINATED**: it simulated 14-3-3 epsilon in place of Elongin B | chain census of all 12 persisted solvated systems: `A=254, E=213, F=255, G=112` |
...
**`warhead_only` may not be runnable at all.** Its defect is in the *input* — the co-fold did not pose free
celastrol in the NR4A1 pocket — which no chain-split fix repairs. With the corrected search the leg now fails
closed rather than tethering to Elongin C. Whether the clean `neg_celastrol` co-fold poses celastrol properly is
answered by [`nrv04_prespend_check.py`](./nrv04_prespend_check.py) at $0, before any spend.
