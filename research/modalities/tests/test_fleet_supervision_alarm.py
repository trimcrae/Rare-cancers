...
def test_fetch_runs_returns_a_reason_and_never_a_bare_None():
    ...
    ⚠ HERMETIC ON PURPOSE. The first version of this test made a REAL call to api.github.com for a real 404.
    It passed alone and failed inside the full suite, which is the signature of a test whose verdict depends
    on the network rather than on the code — and this file is a GATE in front of a billing fleet
    (step1-fanout-autoscale.yml runs it before the progress check), so a flaky red here withholds supervision
    from GPUs that are already spending. The live call now lives in the test below, where it can only SKIP.
    """
    ...
    real, ur.urlopen = ur.urlopen, raise404
    try:
        runs, err = fsa.fetch_runs("trimcrae/Rare-cancers", "no-such-workflow.yml", attempts=1)
    finally:
        ur.urlopen = real
    assert runs is None
...
def test_a_rate_limit_is_named_as_one_rather_than_left_as_a_bare_403():
    """403/429 is the failure this module is most exposed to — an anonymous read from a shared runner IP.
    The reader must not have to know that a 403 here means 'rate limit' rather than 'no permission'."""
    import urllib.request as ur

    def raise403(*a, **k):
        raise urllib.error.HTTPError("u", 403, "Forbidden", {}, None)

    real, ur.urlopen = ur.urlopen, raise403
    try:
        runs, err = fsa.fetch_runs("o/r", "w.yml", attempts=1)
    finally:
        ur.urlopen = real
    assert runs is None and "403" in err and "rate limit" in err


def test_LIVE_actions_api_still_answers_the_way_the_module_assumes():
    """The one test here that touches the network, and it may only SKIP, never fail.

    It is worth keeping: the hermetic tests above pin our handling of the API's answers, not the assumption
    that the API still answers that way. But egress is an ENVIRONMENTAL dependency (proxy, offline runner,
    anonymous rate limit on a shared IP), and per the suite's skip-guard convention an absent dependency
    declines explicitly with a stated reason instead of reddening a gate that guards a billing fleet.
    """
    runs, err = fsa.fetch_runs("trimcrae/Rare-cancers", "no-such-workflow-here.yml", attempts=1)
    if runs is not None or not err or "404" not in err:
        import pytest
        pytest.skip(f"Actions API not reachable as expected from here (got runs={runs!r}, err={err!r}) — "
                    f"environmental, not a code defect; the hermetic tests above cover the contract")
    assert "token=" in err
