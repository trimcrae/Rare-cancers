...
# ⚠ FOUR LEVELS, NOT TWO. This test arrived from `main` at repo-root `tests/`, which this branch
# deleted in Phase 2 — git relocated it here and its two-level climb then resolved to `research/`,
# so it looked for `research/scripts/trigger_scan.py` and errored at collection. `tests/` is not
# coming back: CI runs research/modalities/tests, research/manuscripts/tests and systems/tests, so
# this directory is where the test actually gets RUN.
