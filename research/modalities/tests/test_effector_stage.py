...
# ---------------------------------------------------------------------------------------------------------
# 5 · the STAGED record on disk, pinned so a silent re-stage cannot move it under the documents
# ---------------------------------------------------------------------------------------------------------
@pytest.mark.skipif(not os.path.exists(EF.REGISTRY), reason="no effector registry staged yet")
def test_the_staged_bcl6_arm_is_the_dimer_the_documents_describe():
    """⚠ THE DOCUMENTS NAME AN ENTRY, A BODY AND AN EXIT ATOM. A re-stage that picked a different entry —
    a new deposition, a changed RCSB ranking — would leave every one of those sentences reading as current
    while describing coordinates nobody is using. This is the cheap check that turns that into a red build.

    It pins the PROPERTIES the documents rest on, not the numbers a re-stage may legitimately refine: the
    body must be a two-chain BTB unit, the ligand must span BOTH protomers, and the exit atom must be a real
    deposited ligand atom.
    """
    reg = json.load(open(EF.REGISTRY))
    rec = reg["arms"]["bcl6"]
    assert rec["status"] == "OK"
    assert rec["partner_class"] == "transcriptional effector"
    sel = rec["assembly_copy"]["selection"]
    assert len(rec["assembly_copy"]["selected_chains"]) == 2, rec["assembly_copy"]
    assert sel["n_chains_the_ligand_spans"] >= 2, "the groove ligand must touch both protomers"
    assert sel["completed_through_interface"] is False, "the dimer was read off the ligand's own contacts"
    lig = rec["ligand"]
    assert lig["exit_atom_xyz"] in [a["xyz"] for a in lig["atoms"]]
    # BTB domain, not the full protein — the limit every document has to carry
    assert 200 <= rec["n_residues_written"] <= 280, rec["n_residues_written"]


@pytest.mark.skipif(not os.path.exists(EF.REGISTRY), reason="no effector registry staged yet")
def test_every_staged_arm_self_checked_against_the_consumer_that_will_use_it():
    reg = json.load(open(EF.REGISTRY))
    for aid, rec in reg["arms"].items():
        if rec.get("status") != "OK":
            continue
        sc = rec["self_check"]
        assert sc["ok"], (aid, sc)
        # only a real staging can make this true; a defaulted record cannot
        assert sc["exit_atom_is_a_deposited_ligand_atom"], aid
        assert sc["ring_is_absent"] and sc["transfer_anchor_is_absent"], aid
        assert sc["n_residues_loaded"] == rec["n_residues_written"], aid
