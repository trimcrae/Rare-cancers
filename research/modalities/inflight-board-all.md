...
Merged 3:45 PM ET Sat Aug 1, 2026. One row per GPU leg, for every lane that can bill.
...
_As of 3:37 PM ET Sat Aug 1, 2026 (9 min ago). Written by `ternary_vast_launch.py task=collect`._

```
LEG                           ETA (ET)          % DONE  $/ns                                     STATE     WHY (when not running)
---------------------------------------------------------------------------------------------------------------------------------
5aks_d0_to_d ternary nr4a1 r1 1:35 AM Aug 2      82.2%  RTX 3090 $0.00412/ns · 1.21× basis [bid] RUNNING   56.0 s/iter · no like-for-like expectation (not measured on this system)
```

## STEP 1 FAN-OUT — the cmpd19 congeneric RBFE map (one unit = complex + solvent legs)

_As of 3:37 PM ET Sat Aug 1, 2026 (8 min ago). Written by `congeneric_fanout_vast.py MONITOR=1`. 18 of 19 unit(s) landed; 1 permanently excluded (rows below are the rest)._
...
_As of 3:45 PM ET Sat Aug 1, 2026 (0 min ago). Written by `nrv04_vast_launch.py RETRO_COLLECT=1`. 16 of 16 authorized R1 leg(s) landed (rows below are the rest). ⛔ 2 further record(s) exist but are NOT landed legs (mode='smoke', not 'run') — they do not count toward the panel and cannot reach the frozen gate._
...
_As of 2:59 PM ET Sat Aug 1, 2026 — **47 min ago, STALE (> 15 min)**. Written by `selcal_board.py`. 12 of 12 (arm, seed) co-folds are in S3. % DONE is a COUNT of models that exist; the ETA is refused until 3 arrival intervals have been measured (selcal_board.MIN_RATE_INTERVALS) and is a LOWER BOUND whenever an arm has not started, because no measurable interval includes the MSA + model load an arm pays once._

```
LEG                    ETA (ET)          % DONE  $/ns                                  STATE     WHY (when not running)
-----------------------------------------------------------------------------------------------------------------------
selcal_smarca2 co-fold —                 100.0%  — no ns: co-fold is inference, not MD UNKNOWN   lane last reported 47 min ago — this row is THAT report, not a current reading; it then read: DONE — all 6 seeds have a co-fold in S3. 6/6 seeds; no $/ns is quoted because a co-fold integrates no dynamics — there is no ns denominator, and the lane's MD legs carry a real $/ns from inflight_usd_per_ns.
selcal_smarca4 co-fold —                 100.0%  — no ns: co-fold is inference, not MD UNKNOWN   lane last reported 47 min ago — this row is THAT report, not a current reading; it then read: DONE — all 6 seeds have a co-fold in S3. 6/6 seeds; no $/ns is quoted because a co-fold integrates no dynamics — there is no ns denominator, and the lane's MD legs carry a real $/ns from inflight_usd_per_ns.
```
