...
Merged 4:18 AM ET Sun Aug 2, 2026. One row per GPU leg, for every lane that can bill.
...
_As of 4:17 AM ET Sun Aug 2, 2026 (0 min ago). Written by `congeneric_fanout_vast.py MONITOR=1`. 18 of 19 unit(s) landed; 1 permanently excluded (rows below are the rest)._
...
_As of 4:17 AM ET Sun Aug 2, 2026 (0 min ago). Written by `nrv04_vast_launch.py RETRO_COLLECT=1`. 16 of 16 authorized R1 leg(s) landed (rows below are the rest). ⛔ 2 further record(s) exist but are NOT landed legs (mode='smoke', not 'run') — they do not count toward the panel and cannot reach the frozen gate._
