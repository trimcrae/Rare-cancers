...
Merged 7:59 AM ET Sun Aug 2, 2026. One row per GPU leg, for every lane that can bill.
...
_As of 7:59 AM ET Sun Aug 2, 2026 (1 min ago). Written by `ternary_vast_launch.py task=collect`._
...
_As of 7:59 AM ET Sun Aug 2, 2026 (0 min ago). Written by `congeneric_fanout_vast.py MONITOR=1`. 18 of 19 unit(s) landed; 1 permanently excluded (rows below are the rest)._
...
_As of 7:59 AM ET Sun Aug 2, 2026 (0 min ago). Written by `nrv04_vast_launch.py RETRO_COLLECT=1`. 16 of 16 authorized R1 leg(s) landed (rows below are the rest). ⛔ 2 further record(s) exist but are NOT landed legs (mode='smoke', not 'run') — they do not count toward the panel and cannot reach the frozen gate._
...
zaienne_cmpd19__cw_ms_free_acid r1       —                 100.0%  — free GCP trial credit (separate ledger) ⚠ NO NEW COMMIT GCE L4, RUNNING, created 2026-08-01T05:37:11.159-07:00. phase='done 2026-08-02T09:27:16Z'. both leg JSONs are in GCS; all sampling is done. ETA UNKNOWN — sampling complete — the reduce is a CPU step of seconds. [145 min since the leg started against a budget of 4 x its own measured 24.0 min commit interval — FLAGGED, and a flag is not a condemnation: nothing reaps or refuses on this] Bounded at CREATE by --max-run-duration=172800s.
