...
Merged 11:36 AM ET Mon Aug 3, 2026. One row per GPU leg, for every lane that can bill.
...
_As of 10:33 AM ET Mon Aug 3, 2026 — **63 min ago, STALE (> 15 min)**. The rows below are that report, not a current reading, and the ETA column was computed then and has not been re-measured. Written by `ternary_vast_launch.py task=collect`._
...
_As of 10:54 AM ET Mon Aug 3, 2026 — **42 min ago, STALE (> 15 min)**. Written by `congeneric_fanout_vast.py MONITOR=1`. 18 of 19 unit(s) landed; 1 permanently excluded (rows below are the rest)._
