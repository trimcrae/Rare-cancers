...
_As of 10:25 PM ET Sat Aug 1, 2026 (11 min ago). Written by `ternary_vast_launch.py task=collect`._
...
_As of 10:25 PM ET Sat Aug 1, 2026 (10 min ago). Written by `congeneric_fanout_vast.py MONITOR=1`. 18 of 19 unit(s) landed; 1 permanently excluded (rows below are the rest)._
...
_As of 10:25 PM ET Sat Aug 1, 2026 (10 min ago). Written by `nrv04_vast_launch.py RETRO_COLLECT=1`. 16 of 16 authorized R1 leg(s) landed (rows below are the rest). ⛔ 2 further record(s) exist but are NOT landed legs (mode='smoke', not 'run') — they do not count toward the panel and cannot reach the frozen gate._
...
selcal-smarca4-m2-r0   —                21/22 l  — $0.0711/hr (no measured ns/h: endpoint MD, not the 84k-atom RBFE the throughput table benches) RUNNING   instance 46560490 running, rented 2026-08-02T02:30:30Z. ETA UNKNOWN — this lane has never run an MD leg to its terminus, so there is no measured s/iter to project from; the first one that lands supplies it.
```
