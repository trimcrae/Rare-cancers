...
Merged 10:59 PM ET Sat Aug 1, 2026. One row per GPU leg, for every lane that can bill.
...
_As of 10:58 PM ET Sat Aug 1, 2026 (2 min ago). Written by `ternary_vast_launch.py task=collect`._

```
LEG                           ETA (ET)          % DONE  $/ns                                     STATE     WHY (when not running)
---------------------------------------------------------------------------------------------------------------------------------
5aks_d0_to_d ternary nr4a1 r1 2:02 AM Aug 2      94.4%  RTX 3090 $0.00412/ns · 1.21× basis [bid] RUNNING   55.1 s/iter · no like-for-like expectation (not measured on this system)
```

## STEP 1 FAN-OUT — the cmpd19 congeneric RBFE map (one unit = complex + solvent legs)

_As of 10:58 PM ET Sat Aug 1, 2026 (1 min ago). Written by `congeneric_fanout_vast.py MONITOR=1`. 18 of 19 unit(s) landed; 1 permanently excluded (rows below are the rest)._
...
_As of 10:58 PM ET Sat Aug 1, 2026 (1 min ago). Written by `nrv04_vast_launch.py RETRO_COLLECT=1`. 16 of 16 authorized R1 leg(s) landed (rows below are the rest). ⛔ 2 further record(s) exist but are NOT landed legs (mode='smoke', not 'run') — they do not count toward the panel and cannot reach the frozen gate._
...
selcal-smarca4-m2-r0   —                21/22 l  — $0.0711/hr (no measured ns/h: endpoint MD, not the 84k-atom RBFE the throughput table benches) UNKNOWN   lane last reported 21 min ago — this row is THAT report, not a current reading; it then read: RUNNING — instance 46560490 running, rented 2026-08-02T02:30:30Z. ETA UNKNOWN — this lane has never run an MD leg to its terminus, so there is no measured s/iter to project from; the first one that lands supplies it.
```
