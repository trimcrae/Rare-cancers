...
# co-fold CIF per leg-environment, in the reused S3 bucket (produced by the co-fold step; keys are frozen here).
...
# stage this leg from its co-fold CIF in S3 -> INPUT_DIR/<LEG_ID>/{complex.pdb,ligand.sdf}
$RUN aws s3 cp "$COFOLD_CIF_S3" /tmp/cofold.cif
...
assemble_leg('/tmp/cofold.cif', lg, LIGANDS[lg.ligand], os.environ['INPUT_DIR'])"
...
def cofold_cif_s3(leg, bucket):
    """Which co-fold CIF feeds this leg: the ligand's co-fold system (nrv04 | celastrol | nrv04_epimer)."""
    return f"s3://{bucket}/{COFOLD_PREFIX}/{leg.ligand}/model_0.cif"
...
def build_jobspec(leg, seed, mode, branch, bucket):
    ...
    env.update({
        "GIT_BRANCH": branch,
        "COFOLD_CIF_S3": cofold_cif_s3(leg, bucket),
    ...
    for leg, seed in units:
        ...
        if dry:
            print(f"[dry] {spec.name}: gpu={spec.resources.gpu} ram>={spec.resources.ram_gb}GB "
                  f"ckpt={spec.checkpoint_uri} cofold={spec.env['COFOLD_CIF_S3']}", flush=True)
            continue
        h = be.submit(spec)
