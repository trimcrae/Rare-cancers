...
| **B** | `S5` | Ternary interface discrimination (rung 5b-T) | — | ⛔ **NO** — `V2` 0.839 post-horizon, and `V1` now reads all 16 models per arm and returns **zero** discriminating contacts ([`nr4a3-5bt-signature.json`](nr4a3-5bt-signature.json), 2026-08-05). ⚠ *Superseded, retained: "⚠ partially — … `V1` one contact in one pair" — written when the `V1` read did not exist* | yes for assembly; ⛔ **no** for the selectivity read (`V11` 0/2) | **$0 CPU — rung `5b-T`, needs no authorization** |
