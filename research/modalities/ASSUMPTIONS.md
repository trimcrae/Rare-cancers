---
id: DOC-ASSUMPTIONS
title: Assumption audit — research/modalities compute scripts
level: L4
kind: memo
status: live
canonical_for: []
...
| 4 | PLUMED atom index = OpenMM atom.index + 1 (1-based over system order) | nr4a3_metad `_cv_ca_plumed_indices` | CV biases the WRONG atoms, silently | **GUARDED** — each selected CV residue's NAME is asserted against the AF2 ground-truth identities (406…534) before production; mismatch aborts. Chosen indices still logged |
| 5 | Rg of the CV CA atoms falls in the wall/grid window (SIGMA 0.03, walls 0.6–2.2 nm, grid 0.4–3.0) | nr4a3_metad PLUMED | mis-scaled bias / walls clip the basin | **GUARDED** — initial Rg computed after minimization and printed; aborts if outside [0.6, 2.2], warns if within 5·SIGMA of a wall |
...
| 7 | LBD trimmed contiguously from 373 (residue r at ordinal r−373) | nr4a3_md / nr4a3_metad / residue_map | ordinal shift if PDBFixer drops/adds a residue | **GUARDED** — resolver prefers resSeq; in metad the per-residue identity assertion (#4) catches a contiguity shift that selected the wrong residue. (mdpocket still relies on contiguity for SASA labels — verify in its log) |
...
1. **#8:** log resolved `pdbUrl` + PDB sha256 in every AFDB-fetching job; consider pinning a version.
2. **#10:** assert protein-residue count == expected LBD length (254) after solvation (a cheap global
   complement to the per-residue identity check now in metad).
3. **#6:** compute a proper static-model SASA baseline (from the pre-equilibration minimized structure).

*Done since last revision:* **#4 / #5** — metad now asserts each CV residue's identity against the AF2
ground truth and aborts on an out-of-window initial Rg, both before the long production run.
