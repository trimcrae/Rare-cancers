...
def nr4a3_lbd_reference(af2_reference_pdb, work):
    ...
    (`nr4a3_8xtt_benchmark.LBD_FIRST..LBD_LAST`), and the identity that comes out of the alignment is
...
def transfer_identity(ref_pdb, receptor_pdb):
    """Aligned-column identity between the two chains, for the record. Never a gate here."""
    import nr4a3_8xtt_benchmark as bm
    try:
...
def pipeline_box(receptor_pdb, af2_reference_pdb, work):
    ...
    difference is load-bearing. `map_uniprot_to_pdb` REFUSES below 80 % identity (`MIN_ALIGN_IDENTITY`)
    because it exists to map Q92570 onto a deposit of the SAME protein, where a low identity means a
    corrupt download. The benchmark receptor is a DIFFERENT protein, so that guard would raise on every
    candidate — and using it would have made the benchmark impossible to run for a reason that has nothing
    to do with the science. `map_pocket_to_paralogue` is the kernel the pipeline actually uses to carry
    the NR4A3 pocket onto NR4A1 and NR4A2, which is exactly this operation. Boxing is then
    `nr4a3_warhead.pocket_box`, so "the site" means here what it means everywhere else in the pipeline.
