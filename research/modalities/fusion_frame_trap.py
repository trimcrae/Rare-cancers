...
def grade(fusion: dict, row: dict, genes: dict) -> dict:
    """One fusion, graded on conditions (i) and (ii). Every field derived, none typed."""
    fid = fusion["id"]
    ...
    if unresolved:
        ...
        out.update(verdict="NEITHER_OR_ONE",
                   verdict_reason=("condition (i) fails: the acceptor contributes no untranslated "
                                   "sequence" if U == 0 else
                                   "condition (ii) fails: the 5' partner ends on a codon boundary"))
    return out
...
def known_answer_control(results: list) -> dict:
    ...
    row = next((r for r in results if r["id"] == "EWSR1--NR4A3"), None)
    if row is None:
        return {"status": "ABSENT", "detail": "the index case is not in the result set"}
...
def build() -> dict:
    ...
    with open(BREAKPOINTS, encoding="utf-8") as fh:
        bp = json.load(fh)
    genes = inputs["genes"]
    by_id = {r["id"]: r for r in bp["breakpoints"]}

    verified = verify_quotes(inputs, [r for r in bp["breakpoints"]
                                      if (r.get("evidence") or {}).get("quote")])
    vset = {v["fusion"] for v in verified if v.get("verified")}

    results = []
    for f in FUSIONS:
        row = by_id.get(f["id"], {})
        if row.get("five_prime_exon") is not None and f["id"] not in vset:
            row = dict(row, five_prime_exon=None, three_prime_exon=None,
                       indeterminate_reason="breakpoint present but its quote is unverified")
        results.append(grade(f, row, genes))
    ...
    return {
        ...
        "counts": {
            ...
            "neither_or_one_condition": tally.get("NEITHER_OR_ONE", 0),
            "promoter_swap_not_applicable": tally.get("NOT_APPLICABLE_PROMOTER_SWAP", 0),
            "indeterminate": tally.get("INDETERMINATE", 0),
        },
        "both_conditions_hold": [r["id"] for r in both],
        "indeterminate": [{"id": r["id"], "reason": r.get("indeterminate_reason")}
                          for r in indet],
...
def main() -> int:
    ...
    if a.check:
        ...
        with open(OUT, encoding="utf-8") as fh:
            old = json.load(fh)
        drift = []
        for k in ("counts", "both_conditions_hold", "indeterminate", "n_fusions_in_catalogue"):
            if old.get(k) != out.get(k):
                drift.append(k)
        if [f["id"] for f in old.get("fusions", [])] != [f["id"] for f in out["fusions"]]:
