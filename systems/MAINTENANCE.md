...
|---|---|---|
| every file under `systems/views/**`, **diagrams included** | `python3 systems/systems_check.py --write-views` | after any change to `systems/graph/**` |
...
⚠ **THE 49 DIAGRAMS ARE GENERATED TOO, AND THEIR FAILURE MODE IS SILENT.** A mermaid block with an
unescaped `"` renders on GitHub as a **blank space** — no error, no warning, and the file still passes its
frontmatter, drift and link checks. Nothing else in this repository would notice. That is why
`test_every_generated_diagram_parses` checks the block structurally (quoted labels, declared edge
endpoints, balanced brackets) rather than trusting that it renders, and why `mermaid_label()` exists
separately from `esc()` — `esc()` escapes table pipes, which is the wrong alphabet entirely.
...
| `[D11]` frontmatter **against the schema** | frontmatter that is not valid **YAML** · a `level` or `kind` outside the enum · a key the contract does not define. ⭐ Added 2026-08-06, when it emerged that `document.schema.json` had never been applied to anything — the row above was the whole of the enforcement, and it never read `level` or `kind`. Its first run found **24 documents whose frontmatter a real YAML parser rejects**, all of them accepted by the line splitter that had been the only thing to look at them ([MIGRATION §2.4](./MIGRATION.md#24--phase-7--the-audit-of-phases-05-and-the-four-things-it-found)) |
| `[D12]` · `[P1]` | the authoring template in CONVENTIONS.md offering an enum the schema will reject · a document naming the wrong `preflight.sh` gate number. Both are *"one fact, several homes"* caught by DERIVING the fact instead of comparing two copies |
