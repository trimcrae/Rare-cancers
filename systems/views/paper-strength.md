...
**Score** = 5 per open route · 2 per route blocked only on a human decision · −1 to −2 per closed route (by how it closed) · 3 per route whose status is `ready` · up to 5 for the fraction of remaining validation steps that are feasible today · 2 for having a drafted document, 1 for an outline.

⛔ **A LOW SCORE CAN MEAN 'CLOSED' OR IT CAN MEAN 'NOBODY GRADED IT', AND THOSE ARE OPPOSITE THINGS.** `closure_kind` is unset on 24 of 68 routes, and an unset field contributes nothing in either direction — so a paper can sit low here purely because its routes have never been graded. The `ungraded` column is that reading, and a high number in it means **go grade the routes**, not **the paper is weak** (CLAUDE.md §4: an absent reading is not a reading of absence).

| # | endpoint | score | open | closed | ungraded | ready | doable here | state | routes |
|---:|---|---:|---:|---:|---:|---:|---|---|---:|
| 1 | [**PUB-REPURPOSING**](../../research/manuscripts/repurposing-hypotheses.md) | **18.0** | 2 | 0 | 0 | 1 | 20% of 5 | ◐ `drafted` | 3 |
...
| 9 | [**PUB-ATR**](../../research/manuscripts/emc-atr-vulnerability-assessment.md) | **10.0** | 1 | 0 | 0 | 1 | 0% of 1 | ◐ `drafted` | 1 |
| 10 | [**PUB-MTAP-PRMT5**](../../research/manuscripts/emc-mtap-prmt5-hypothesis.md) | **10.0** | 1 | 0 | 0 | 1 | 0% of 3 | ◐ `drafted` | 1 |
| 11 | [**PUB-FUSION-OUTPUT**](../../research/manuscripts/nr4a3-fusion-transcriptional-output.md) | **9.5** | 1 | 0 | 0 | 0 | 50% of 6 | ◐ `drafted` | 1 |
| 12 | [**PUB-SURFACE-TARGETS**](../../research/manuscripts/emc-surface-target-landscape.md) | **9.0** | 2 | 3 | 0 | 0 | 0% of 8 | ◐ `drafted` | 6 |
| 13 | [**PUB-BIOMARKER-DEP**](../../research/manuscripts/emc-biomarker-selected-classes.md) | **7.0** | 0 | 0 | ⚠ 5 | 1 | 40% of 10 | ◐ `drafted` | 5 |
| 14 | **PUB-STRATEGY-ARCH** | **6.9** | 0 | 0 | ⚠ 3 | 1 | 57% of 7 | ◔ `outlined` | 3 |
...
**PUB-REPURPOSING** — score 18.0
- [RT-PPARG-DOWNSTREAM](L2-rt-pparg-downstream.md) — *Could a PPARγ-directed agent act on a downstream effector of the fusion?* — `blocked` / `concept` / confidence `low`
- [RT-CARFILZOMIB](L2-rt-carfilzomib.md) — *Does the best ex-vivo EMC drug-sensitivity evidence point at a proteasome inhibitor combination?* — `ready` / `concept` / confidence `moderate`

**PUB-DEGRADER** — score 16.9
- [RT-ASYMMETRIC](L2-rt-asymmetric.md) — *Are the two paralogue-sparing requirements actually the same requirement — and what changes if they are not?* — `ready` / `computed` / confidence `high`
- [RT-PANNR4A-EXVIVO](L2-rt-pannr4a-exvivo.md) — *Could pan-NR4A engagement be useful EX VIVO — during T-cell manufacturing — where selectivity does not matter?* — `ready` / `computed` / confidence `moderate`

**PUB-EMC-PROGRAM** — score 15.0
- [RT-TRABECTEDIN](L2-rt-trabectedin.md) — *Is trabectedin, an approved sarcoma agent, mechanistically well matched to a FET-fusion sarcoma like EMC?* — `ready` / `concept` / confidence `low`
- [RT-ICI-TKI](L2-rt-ici-tki.md) — *Does the checkpoint-inhibitor plus anti-angiogenic combination have an EMC signal worth pursuing?* — `delegated` / `concept` / confidence `moderate`

**PUB-METHODS** — score 15.0
...
**PUB-ASO** — score 10.7
- [RT-ASO](L2-rt-aso.md) — *Can an RNase-H gapmer or siRNA against the EWSR1::NR4A3 breakpoint junction silence the chimera while sparing wild-type NR4A3?* — `blocked` / `scoped` / confidence `moderate`

**PUB-TCIP** — score 10.3
- [RT-TCIP](L2-rt-tcip.md) — *Can chemically induced proximity recruit a transcriptional effector to the fusion instead of degrading it?* — `blocked` / `scoped` / confidence `low`

**PUB-ATR** — score 10.0
- [RT-ATR-ASSESS](L2-rt-atr-assess.md) — *Does EMC inherit a replication-stress vulnerability from its FET-fusion class, and can that be assessed computationally?* — `ready` / `computed` / confidence `low`

**PUB-MTAP-PRMT5** — score 10.0
- [RT-MTAP-PRMT5](L2-rt-mtap-prmt5.md) — *Does this tumour carry the copy-number state that selects the PRMT5 axis?* — `ready` / `computed` / confidence `moderate`

**PUB-FUSION-OUTPUT** — score 9.5
- [RT-FUSION-OUTPUT](L2-rt-fusion-output.md) — *Do the genes an NR4A3 chimera is published to bind read higher in EMC tumour tissue than in comparator tumours — beyond what an arbitrary gene set of the same size achieves on the same platform?* — `active` / `validated_in_silico` / confidence `moderate`

**PUB-SURFACE-TARGETS** — score 9.0
- [RT-PRAME-IMMTAC](L2-rt-prame-immtac.md) — *Is a PRAME-directed T-cell engager or receptor therapy applicable to EMC?* — `blocked` / `computed` / confidence `moderate`
- [RT-FAP-RLT](L2-rt-fap-rlt.md) — *Could a fibroblast-activation-protein radioligand reach EMC through its stroma?* — `blocked` / `concept` / confidence `unknown`
