...
It is at preprint readiness and now carries its central number. What would raise it further is replication rather than access.

**Missing:**
- nothing on the CODING half — and the magnitude is now IN the paper rather than deferred
- an independent replication of the 32.1% over a different window (Table 1 of PMID 32856598, a subscription article)
- a site-by-histology cross-tab, which would remove the one inferred step
...
**It is not written because:** ⭐ DRAFTED AND IT NOW CARRIES ITS CENTRAL NUMBER — see `document`. Scope and title were decided by trimcrae on 2026-08-23: publish the CODING half, demote the naming argument to a cited paragraph. ⚠ THE PREMISE OF 'PUBLISH WITHOUT THE MAGNITUDE' NO LONGER HOLDS, AND HE WAS TOLD: the magnitude was recovered the same day from a cohort already published, so the paper measures it rather than deferring it. The narrowing to the coding half is unaffected and stands. ⛔ NEITHER DECISION IS AUTHORISATION TO POST, SUBMIT OR DEPOSIT — CLAUDE.md s3 requires trimcrae to name THIS paper for THAT act, and he has not. Nothing has been posted anywhere. ⚠ TITLE HISTORY, recorded here because the file's frontmatter owns the title: this endpoint read 'One code, two diseases: what registry-based extraskeletal myxoid chondrosarcoma cohorts actually contain' until 2026-08-23, when a third published reading of 9231/3 (CBTRUS, PMC9290890) made 'two' an understatement. Renamed on explicit instruction only, and never posted under the old title. ★ WHAT WOULD STILL IMPROVE IT is replication rather than access: a site-by-histology cross-tab to remove the one inferred step, the same query without the lymph-node-status requirement, and Table 1 of PMID 32856598 over a different window.
...
⭐ The measurement landed: research/manuscripts/care-delivery/emc-icdo-9231-classification.md now MEASURES the contamination at >=32.1% instead of stating it as an open query. ⛔ STILL NOT AUTHORISED FOR POSTING — CLAUDE.md s3 needs the paper named for the act. Next: adversarial review rounds (`paper-hardening`), and the two-by-two morphology-by-topography display item, which is now a filled figure rather than a mostly-empty one.
