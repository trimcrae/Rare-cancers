...
| **PUB-ATR-PANEL-ASK**<br/>[Reading-frame constraints and retained RG content in NR4A3 fusion model…](../../research/manuscripts/dependency/emc-atr-collaborator-package.md) | ◐ `drafted` | `experimental_proposal` | 1 | — |
...
The reported NR4A3-fusion junctions of extraskeletal myxoid chondrosarcoma can be compiled from primary sources, translated at transcript level, placed on the published retained-RG recruitment axis, and converted into five predictions whose falsifiers, constructs and controls are specified in advance — and the report states inside itself which reported fusions have no sourced transcript-level junction and that no sequence, construct or prediction in it was verified at a bench. ⛔ CORRECTED 2026-09-08: this field read 'Everything a group already running the FET-fusion DSB-recruitment assay would have to derive in order to add EMC as a fourth partner class is pre-built — constructs, controls, predicted outcomes and kill criteria fixed in advance — so the marginal cost of testing the assessment's prediction is the bench time and nothing else.' That sentence describes an experimental programme nobody has performed; the document at document.file is a computational sequence-analysis report with explicit missing sources and no bench verification. The experimental programme it specifies remains unperformed, and BLK-NO-WET-LAB still blocks it.
