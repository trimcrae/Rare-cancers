...
| **PUB-ATR-PANEL-ASK**<br/>[Reading-frame constraints and retained RG content in NR4A3 fusion model…](../../research/manuscripts/dependency/emc-atr-collaborator-package.md) | ◐ `drafted` | `journal_submission` | 1 | — |
...
**◐ `drafted` · aimed at `journal_submission` · [`research/manuscripts/dependency/emc-atr-collaborator-package.md`](../../research/manuscripts/dependency/emc-atr-collaborator-package.md) · ships with **PUB-ATR****

The reported NR4A3-fusion junctions of extraskeletal myxoid chondrosarcoma can be compiled from published sources, including primary reports and reviews, translated at transcript level, placed on the published retained-RG recruitment axis, and converted into five predictions whose falsifiers, constructs and controls are specified in advance — and the report states inside itself which reported fusions have no sourced transcript-level junction, that nothing in it has been verified at a bench or in a patient, and that P5 is UNTESTED because no TCF12::NR4A3 construct exists and the full-length GFP-TCF12 partner-alone control does not test a prediction about a fusion. ⚠ Computational verification HAS occurred: the reading frames, retained-RG counts and in-frame self-checks are computed and re-derivable from committed artifacts. What is absent is bench and patient verification, not verification of any kind. ⛔ CORRECTED 2026-09-08: this field read 'Everything a group already running the FET-fusion DSB-recruitment assay would have to derive in order to add EMC as a fourth partner class is pre-built — constructs, controls, predicted outcomes and kill criteria fixed in advance — so the marginal cost of testing the assessment's prediction is the bench time and nothing else.' That sentence describes an experimental programme nobody has performed; the document at document.file is a computational sequence-analysis report with explicit missing sources and no bench verification. The experimental programme it specifies remains unperformed, and BLK-NO-WET-LAB still blocks it.
