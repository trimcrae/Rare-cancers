...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-26 9:40 PM ET**)
...
| **Step 1 fan-out** (LANE 17/21) — 19 congeneric RBFE edges | **PAST WARMUP — `complex/production@40` (prev `warmup/340`), the first time this unit has crossed into production on any host.** Note the counter RESETS at the phase boundary, so 340 → 40 is a transition and not a regression — the same trap the watchdog's metad→release rule exists for. Earlier tonight the lane's own tick was failing on a **missing PyYAML in the gate step**, so no readout committed and the terminus release could never fire; fixed in both gates, and the **9:12 PM `schedule` run passed**, so the repair holds on cron and not just by hand. LANE 19 separately ruled out mapper exposure here **by measurement**: on all 19 edges 20 s and 300 s return the *same* map in 0.0–1.4 s | the warmup→production crossing is what makes the ΔΔG target readable — it has now landed, so the figure is derivable from the leg log; **still no ETA quoted until it is read**, per Appendix A 19b/19c/19d | ladder **~$36 ($15–80)**; realised **$0.35**. ⚠ **~$31.9 is stale** — the replacement host bills **$0.2247/hr** against the $0.1224 it was built on; ~**$58** if the 18 land in that band, inside the authorised band but well above the $36 point estimate |
