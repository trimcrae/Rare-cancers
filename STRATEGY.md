...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-27 3:45 AM ET**)
...
|---|---|---|---|
| **RUNG 5a-KS** (LANE 16) — the ligand-side causal kill-switch | **BOTH legs are dying every ~15–20 min; NR4A3 was never the healthy control (Appendix A 32).** Both RUNNING, 0 alerts — NR4A1 **recovered by itself to `warmup/256`** (first full 64-iteration segment tonight, same instance, **no fourth rental**), so the fault is **intermittent, not deterministic**. But both phase markers were rewritten within the last half hour (NR4A3 3:19 AM, NR4A1 3:07 AM): **NR4A3 restarts on the same cadence and HIDES it**, because its production interval is `ci=40` where NR4A1's warmup interval is `ci=64` — it crosses a commit boundary before each death and so always looks like progress. *Which leg looks stalled is an artifact of interval vs crash period, not of leg health.* **Measured cost of the churn:** NR4A3 did 40 production iterations in 35 min = **~52 s/iter effective against ~15 s/iter raw in the logs, ≈3.5× overhead paid in restarts** — so every ETA and $/ns for this rung currently understates the true cost by about that factor. The fan-out shakeout has run hours without this, so it is **not** universal to the repo; what differs between the lanes is where to look | no ETA; $0 comparison across both legs' archived `attempts/` logs, **nothing rented** | 5a-KS realised **~$1.5** of ~$12, but the **effective** $/ns is ~3.5× the nominal below. **Nominal $/ns:** NR4A3 $0.00658 · **1.51× basis ⚠**; NR4A1 $0.00562 · 1.29× basis |
