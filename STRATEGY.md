...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-27 4:25 AM ET**)
...
|---|---|---|---|
| **RUNG 5a-KS** (LANE 16) — the ligand-side causal kill-switch | **Both legs alive and progressing; the restart story I built is RETRACTED (Appendix A 33) — the deaths are ordinary spot churn.** NR4A3 **production, 240/2000** at 18.7–19.3 s/iter, current attempt 49 min old; NR4A1 **warmup, 320/1600** at 14.7–15.1 s/iter, attempt 61 min old. A **cross-lane restart census** ($0) measured per-attempt lifetimes from consecutive S3 archive timestamps — nobody had ever measured them: NR4A3 **408/23/84 min** (one attempt ran **6.8 h unbroken** and completed the whole 1600-iteration warmup), NR4A1 **76/103/112/35/143/63 min**. **There is no ~15–20 min period**, and the lane I called the healthy comparator — `calib…probe` — restarted **14 times** with 3-, 6-, 12- and 16-minute attempts the 5a-KS legs never had. **The 5a-KS legs are among the better-behaved units.** Deaths are the **host leaving, not the process faulting**: no `status.json`, no traceback, and crucially **no `[tvast] FAILED at <phase>` line** — which all three pre-equilibration aborts DID write, so the failure path demonstrably works and its silence rules out crash, NaN **and OOM** (a SIGKILL still leaves the EXIT trap to write the record) | NR4A1 warmup ≈**5.3 h** remaining, then production; **no reliable finish ETA while spot churn sets the wall clock** | 5a-KS realised **~$1.5** of ~$12. Restart overhead measured at **≈1.15×**, not the 3.5× I derived from a single window. **$/ns:** NR4A3 $0.00658 · **1.51× basis ⚠**; NR4A1 $0.00562 · 1.29× basis |
