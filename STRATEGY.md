...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-26 11:40 PM ET**)
...
|---|---|---|---|
| **RUNG 5a-KS** (LANE 16) — the ligand-side causal kill-switch | **Both legs RUNNING, 0 alerts, and the identity-map fix is CONFIRMED.** `…nr4a3` (45947117, RTX 4080S) `warmup/832` of 1600, prev 640 — advancing. `…nr4a1` (45947762, RTX 4090) **`warmup/64`, prev 0** — exactly the first checkpoint interval, which is the confirmation the fix took; the earlier `SETUP_STALL` was **premature, not a hang**, and the rebuilt alert had said so in as many words. Root cause settled: at pre-equilibration `ligA` and `ligB` are **the same molecule** (one pose written twice), so LOMAP was asked for an identity map and returned 80/111, then 110/111 at 300 s; fixed by an exact full-molecule substructure match, **111/111** | **NR4A3 ~6:20 AM** — 768 warmup iterations left at a **measured 17.2 s/iter** (192 iters across 3 commit blocks, 9:17→10:12 PM), then 2000 production at the smoke's 7.9 s/iter. **NR4A1 no ETA** — one commit block is not a rate window | 5a-KS realised **~$0.20** + ~$0.34 burned by the earlier NR4A1 loop; the pair remains inside the ladder's **~$12**. **$/ns:** NR4A3 $0.00446 · **1.02× basis**; NR4A1 $0.00442 · **1.02× basis** |
| **Step 1 fan-out** (LANE 17/21) — 19 congeneric RBFE edges | Shakeout advancing in production: **`complex/production@240`, prev @40** — 200 iterations in ~25 min across 5 commit blocks, a **measured 7.5 s/iter** matching the smoke's 7.9. **The 18 remain held by TWO gates**, and the price gate is the one holding: live read **54 offers, only 12 PRICEABLE, mean $0.013774/ns = 3.16× the $0.004359 ladder basis → projected $112.71 against a $80.44 ceiling → HELD** (`first_held_utc 02:04:52Z`). Without it the automatic release would have bought **~$113 against the authorised $15–80**. The board is **thin in BENCHED cards, not in offers** — 42 of 54 are never-benched and ranked last rather than guessed at | **ΔΔG still has NO ETA, deliberately.** The rate is now measured but the lane's **production iteration target remains unread** — it ships with the leg log only when the leg ENDS, and `mode=diag` does not carry it. Borrowing the ternary lane's 2000 would be arithmetic on an assumption (Appendix A 19b). The 18 escalate to trimcrae **~4:05 AM** if the market stays this bad | ladder **~$36 ($15–80)**, realised **$0.35**, **$0 while held**. **$/ns:** shakeout $0.00767 · **1.76× basis ⚠ DRIFT**; market $0.013774 · **3.16× basis ⚠** |
