...
| **valB_mini r1+r2** (4 ternary legs, Vast) — **HELD ON PRICE, not stalled** | ⏸ **HELD, and the hold is trimcrae's decision (2026-07-26: "I'd rather pause until availability opens than pay double per ns"), not a failure.** Live snapshot **7:47 AM ET 2026-07-27** ([`ternary-vast-market-hold.json`](research/modalities/ternary-vast-market-hold.json)): board **48 offers returned · 47 qualifying · 10 priceable · 4 needed**, floors **min $0.133/hr, median $0.182/hr**; best single offer $0.006431/ns but the **4-leg mean is $0.008554/ns = 1.96× basis**, against the **1.5×** line — needs **≤ $0.006539/ns**. Improving slowly (2.03× at 6:10 AM → 1.96× at 7:47 AM). The gate re-checks hourly and **dispatches the legs itself the moment it clears** — no agent in the loop | clears when the board does; **no deadline** — the work is checkpointed and the ladder has none | projected **$17.23** against a **$22.28** ceiling — i.e. **the dollar ceiling is NOT what holds it**; the `$/ns` rate is |
