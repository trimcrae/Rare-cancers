...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-27 1:35 AM ET**)
...
|---|---|---|---|
| **RUNG 5a-KS** (LANE 16) — the ligand-side causal kill-switch | **BOTH legs lost their hosts to capacity reclaim and BOTH are relaunched on fresh 4090s, resuming from checkpoint.** NR4A3 → **45972721** ($0.2071/hr) from **warmup/1600 — warmup COMPLETE**, crossing to production; NR4A1 → **45972724** ($0.1799/hr) from warmup/192. `committed=` already reads 1600 and 192 on the new instances, so no work was lost beyond the last commit. **Diagnosis was structural, not circumstantial:** the engine writes commits **straight to S3** while the log sync is an independent loop, so an uploader fault leaves commits advancing while the log goes stale — here **both froze together**, which only happens if the process is gone. Confirmed by `collect`'s nudge returning `resources_unavailable` (*"machine 18857 has no free GPU and no bid fixes it"*); the host was destroyed and the machine blocked, per the standing rule. **The restart cadence was the tell** — 01:27→03:07→03:41Z is a host repeatedly losing and regaining its GPU, not a hang | **recovery UNCONFIRMED until the counters move**: NR4A3's first production commit lands at 40, NR4A1's next warmup commit at 256. Either sitting at 1600/192 with a stale log is the same bug | 5a-KS realised **~$1.5** of the ladder's ~$12; the stall cost **~$0.87**. **$/ns:** NR4A3 $0.00658 · **1.51× basis ⚠ DRIFT**; NR4A1 $0.00572 · 1.31× basis — rented into the same thin board that is holding valB, and **allowed because the market gate gates FAN-OUTS, not the resume of a unit already part-run** (CLAUDE.md §6) |
| **Step 1 fan-out** (LANE 17/21) — 19 congeneric RBFE edges | Shakeout advancing steadily in production: **`complex/production@760`**, up from @40 at 9:13 PM through @240 at 10:05 PM. Rate **16.4 s/iter**, measured over a **197-minute, 720-iteration window across three readouts** — steady to within 7 % across sub-windows (15.6 / 16.7). *(Supersedes the rate I reported at 10:45 PM — Appendix A 28.)* **The 18 remain held by the price gate:** projected **$112.71** against a **$80.44** ceiling, mean **$0.013774/ns = 3.16× the $0.004359 ladder basis**, `first_held_utc 02:04:52Z`; 42 of 54 offers are never-benched cards ranked last rather than guessed at | **ΔΔG still has NO ETA** — the rate is now solid but the lane's **production iteration target remains unread** (it ships with the leg log only when the leg ENDS; `mode=diag` does not carry it, confirmed by running it). Borrowing the ternary lane's 2000 would be arithmetic on an assumption | ladder **~$36 ($15–80)**, realised **$0.35**, **$0 while held**. **$/ns:** shakeout $0.00767 · **1.76× basis ⚠ DRIFT**; market $0.013774 · **3.16× basis ⚠** |
