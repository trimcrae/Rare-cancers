...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-27 2:05 AM ET**)
...
|---|---|---|---|
| **RUNG 5a-KS** (LANE 16) — the ligand-side causal kill-switch | **Recovery is PARTIAL and the host churn is the story.** NR4A3 (**45972721**, RTX 4090 $0.2071/hr) is up 36 min and still reads `warmup/1600` — which **is its resume point**, so no new commit yet and the `production/40` recovery criterion is **not met**. NR4A1's replacement (45972724) **DIED within 36 minutes** of being rented; the watchdog caught it (`verdict=DIED, instance=None`) and **auto-relaunched with no agent involved** to **45974679** on machine 55559 at $0.1768/hr — session-independent recovery working as designed, attempt 1/8 of a daily cap. **⚠ NINE machines are now blacklisted** as known to refuse starts — `117843, 12697, 140043, 18857, 26910, 28164, 29668, 29770, 46392` — and **140043 is the host rented 36 min earlier**. Tonight's board is not merely expensive, it is **unstable**: hosts are expiring shortly after rent. That is a distinct failure mode from the price signal and it **strengthens the fan-out hold** — 18 units into a board with this churn would pay restart overhead on top of a 3.16× rate | recovery still reads at NR4A3 `production/40`, NR4A1 `warmup/256` | 5a-KS realised **~$1.5** of ~$12. **$/ns:** NR4A3 $0.00658 · **1.51× basis ⚠ DRIFT**; NR4A1's new host $0.1768/hr, card unconfirmed so **$/ns not quoted** rather than assumed |
