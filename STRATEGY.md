...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-27 2:40 AM ET**)
...
|---|---|---|---|
| **RUNG 5a-KS** (LANE 16) — the ligand-side causal kill-switch | **NR4A3 RECOVERY CONFIRMED — `production/120`** on **45972721** (RTX 4090, up 69 min), past the `production/40` criterion: warmup complete at 1600, production running. **NR4A1 is NOT recovered** — still `warmup/192`, which is its resume point, on **45974679** (RTX 4090, up 33 min) with **`stall=1`** and a **19-minute-stale log** where a healthy leg reads 0–2 min. Its next commit is due at 256 (ci=64); one more frozen pass makes it a stall. This unit has now lost **three** hosts tonight, which is host churn rather than a leg fault — nine machines are blacklisted | **The ~6:20 AM ETA is WITHDRAWN (Appendix A 29)** — it used **7.9 s/iter** taken from the smoke leg's **12 production iterations**, far too small to carry an ETA. The live window is ~17 s/iter but spans the warmup→production transition, so it is contaminated; **no ETA until a clean production window exists** | 5a-KS realised **~$1.5** of ~$12. **$/ns:** NR4A3 $0.00658 · **1.51× basis ⚠ DRIFT**; NR4A1 $0.1768/hr on a 4090 → **$0.00562 · 1.29× basis** |
