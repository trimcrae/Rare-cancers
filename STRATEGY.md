...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-26 4:55 PM ET**)
...
|---|---|---|---|
| **RUNG 5a-KS** (LANE 16) — the ligand-side causal kill-switch | **BOTH TERNARY LEGS RUNNING.** Co-fold done and reaped; staging verified against the real CIFs; the `preequil` failure was root-caused (no hydrogens / no terminal OXT, so `createSystem` could not match templates), fixed via the crystal stager's own `_hydrogenate_pdb`, and **proved CONFIRMED on a free CPU runner** (run 30218400450) before anything was re-rented. Smoke then reached its **real success terminus** (`production/12`, leg record, host reaped). Legs: **NR4A3** Vast `45941659` machine 117843 RTX 4090 $0.2348/h; **NR4A1** Vast `45941913` machine 114101 RTX 4090 $0.2237/h — its first host answered `resources_unavailable` and the lane destroyed it and moved on, per the standing rule. Watchdog armed on both, `verify-armed` clean | ~**8 h/leg** off the smoke's measured **7.9 s/iter**, so **S ~1–2 AM ET Mon** | **~$3.6 projected** for the pair at the observed rates — well inside the ladder's **~$12**. Realized so far **$0.11** (co-fold $0.04 + two smokes $0.07) |
| **Step 1 fan-out** (LANE 17) — 19 congeneric RBFE edges | **The lane no longer needs an agent alive.** Shakeout unit `…cw_ev_5cooh…` resumed a 2-day-old wave-1 checkpoint **on a different host** and ran at **~261 iter/h** against the ~265 wave-1 implies — full card rate, so a cross-host resume costs nothing. Preempted 4:31 PM, auto-resumed onto **45938720**. The other 18 are held by a terminus gate that releases them on the first `ddg.json`. **Session-independent cover is asymmetric and worth stating:** the unit is in main's `vast-watch.json` (`step1_fanout`, enabled) and `vast-watchdog` is PROVEN firing on `schedule`, but `step1-fanout-autoscale.yml` (*/20, on main) has so far only ever run on `workflow_dispatch` — **no `schedule` event yet**, so the auto-release of the 18 is not yet proven autonomous | shakeout ΔΔG **~2:00–3:30 AM** — *an estimate, not arithmetic*: the iteration target is unread until the leg log or the warmup→production crossing lands. Fan-out completes ~13.7 GPU-h after release | ladder **~$36 ($15–80)**. The lane's own ledger, at the **$0.1224/hr actually charged**, projects the tranche at **~$31.9** — under plan, and confirmation that wave 1's $0.35–0.39/hr was the retired ×1.9 bid multiplier rather than the market. **Realised so far $0.09** |
