...
|---|---|---|---|
| `ternary-vast-watchdog.yml` (cron) | the **2 RUNG 5a-KS ternary legs** (`nr4a1`, `nr4a3`). ⚠ **Not the 4 RUNG 2b legs any more** — those landed and every one of their entries is `enabled: false`, so naming them here would be coverage of nothing | **YES** — relaunches a DIED leg from its last checkpoint; a STALL alerts but does **not** relaunch, because a relaunch would hang the same way and pay for it again | **All runs success, latest 7:29 AM ET 2026-07-27** (both legs RUNNING and advancing). Cron says `*/15` but GitHub throttles busy repos, so real firings are ~55–65 min apart — i.e. **hourly in practice** |
...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-27 6:40 AM ET**)
...
| **valB_mini r1+r2** (LANE 19) — the 4 legs that turn the calibration verdict from INDETERMINATE into a real one | **Cohort 2 is GONE and cohort 3 is being rented (3:38 PM ET).** The 2:13 PM cohort's four hosts were all reclaimed by their machines: `task=collect` at 3:36 PM nudged every one and Vast answered `resources_unavailable` on all four, so all four were destroyed and machines **68109 · 67860 · 24573 · 144071** published to the SHARED cross-lane blacklist. ⚠ **They had been dead for ~85 min and the cron watchdog said they were fine** — `classify` reads `instance_alive = inst is not None` and never `cur_state`, so two ticks printed *"advancing at warmup/512 … Leaving it alone"* over boxes whose GPUs were already gone; fixed (`ternary_vast_watchdog.stopped_and_billing`, 7 tests). **Work banked, not lost:** binary r1 reached **warmup/512 of 1600**, binary r2 **warmup/192**; both ternary legs committed **nothing** (their hosts never started a container). **SCOPE CONFIRMED AT 4 LEGS, NOT 2** — `per_replicate_ddg_coop` pairs on `set(ternary) & set(binary)`, so a ternary-only set leaves n_paired=1 and returns the SAME INDETERMINATE (argument + 12 tests: `tests/test_edge_reps_seed_independence.py`) | no deadline — every leg checkpointed in S3; a replicate SD exists only after all 4 land and `task=reduce-reps` runs | derived, never typed (`rung_band_usd(4)`): plan **$7.32**, ceiling **$20.74**, 536 ns/leg. Realised on cohort 2 **$0.81** (0.09/0.27/0.18/0.27). **Gate at 3:38 PM: mean $0.005423/ns · 1.59× basis, projected $11.63 — CLEAR on both ceilings** (board 158 offers, 93 priceable). **Cohort 3 BILLED per host, off the rental receipt — 4 hosts for 4 units, `already_live: []`, `failed: []`:** ternary r1 `46035940` **$0.005119/ns · 1.500×**, binary r1 `46036026` **$0.005214/ns · 1.528×**, ternary r2 `46036082` **$0.005119/ns · 1.500×**, binary r2 `46036096` **$0.005369/ns · 1.574×** — **every host under the $0.006539/ns buy line** (1.917× basis), burn **$0.8461/hr** |
