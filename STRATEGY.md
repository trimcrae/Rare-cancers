...
| **The closure triangle** (LANE 9/20) — decides whether valB's miss is fixable at all | **LAUNCHED AND BILLING — 3 of its 4 legs hosted at 7:51 PM ET.** The path that got here, in order: the **smoke landed at 6:56 PM** (`status=done`, dG 44.807 ± 0.582, `NaN=False`, 13.7 s/iter production), the **atom-map gate then PASSED** — *every leg's atom map measured complete at the production budget* — and the price gate cleared on a **180-offer board** (174 qualifying, 97 priceable). Running: `calib_lo_to_lo2__ternary_vhl` (46055583), `calib_lo_to_lo2__binary_vhl` (46055590), `calib_hi_to_lo2__ternary_vhl` (46055595). ⚠ **`calib_hi_to_lo2__binary_vhl` is the 4th and is NOT yet hosted** — the launcher recorded `n_requested: 4, n_rented: 3`, so it is *visibly* unplaced rather than dropped, and a re-dispatch places it without disturbing the three running | no leg ETA yet — all three were at `committed=none/0` on the 7:55 PM board, so none has reached its first checkpoint. `R` exists only once all 4 land and `task=triangle-reduce` runs | projected **$9.86** against this rung's DERIVED ceiling **$15.40** | **1.50× / 1.69× / 1.69× basis** ($0.005119 / $0.005750 / $0.005775 per ns), ⚠ **PAYING** — all three **under** the $0.006539/ns buy line; the gate's own board mean was 1.73×. `rented_any_over_buy_line: false` |
...
(4 legs) and, since 7:51 PM ET, the **closure triangle** (3 of 4 legs hosted). **Two more are held,
