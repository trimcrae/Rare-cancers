...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-26 6:15 PM ET**)
...
|---|---|---|---|
| **RUNG 5a-KS** (LANE 16) — the ligand-side causal kill-switch | **The two legs are now on different tracks.** `…ternary_nr4a3` was **preempted** at warmup while advancing (`Iteration 44/64, 7.742 s/iter`, no failure record — routine) and is **running again** on **45943982**, an **RTX 3090 at $0.0643/hr**: a third the 4090s it kept landing on, and better **$/ns** on this bandwidth-bound workload, not worse. `…ternary_nr4a1` is **HELD, not re-rented** — it was **refused by the engine's own endpoint verification** (**80 mapped / 31 dummy / `graph_identical: false`** against the NR4A3 arm's **111 / 0 / true** on the same construct), cause named as the warhead's indole being **Kekulé on one side and aromatic on the other** — a perception difference, not chemistry. Staging cleared **by measurement** (both `ligands.sdf` from the stage cache the hosts actually read: identical constitution, 21 aromatic atoms, `C44H47N7O13`, indole aromatic), so the corruption is **downstream in the endpoint rebuild**. A **$0 CPU reproducer** (run `30222095681`) discriminates on **MD length** — `PREEQUIL_SMOKE=1`, ~800 steps against production's 275,000: if the abort does NOT reproduce the 0.5 ns relaxation distorted the indole; if it DOES, the `ligB` rebuild loses aromaticity independent of MD and that is the defect to fix | NR4A3 ~**10–13 h** on the 3090; NR4A1 gated on the diagnostic (~30–40 min). **`S` has NO estimate until the cause is named** — three ETAs carried forward without arithmetic are already in Appendix A 19b/19c/19d and this is not becoming the fourth | NR4A3 leg ~**$0.85**; pair still inside the ladder's **~$12**. The two dead attempts bought ~**$0.1** of pre-equilibration. 5a-KS realised **~$0.20** |
