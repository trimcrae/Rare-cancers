...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-27 3:15 AM ET**)
...
|---|---|---|---|
| **RUNG 5a-KS** (LANE 16) — the ligand-side causal kill-switch | **NR4A1 WAS NEVER STUCK AT 192 — the frozen counter was the COMMIT INTERVAL, not the failure point (Appendix A 31).** Its log shows healthy iterations to **240/256 at 14–15 s each**, no NaN, no traceback: it resumes from 192 cleanly, runs ~48 more iterations, then dies **before the 256 commit boundary**, so nothing new is ever recorded. `warmup_ckpt_iters` is **64**, and any host dying inside a 64-iteration segment reports 192 — which is why three independent machines gave the identical number without hardware being involved. **Nothing is lost:** the 192 checkpoint is sound and warmup is not forfeit; the ~$2.6 across three rentals bought no *recorded* progress purely because the interval outlives the process. **Unresolved: WHY it dies ~15–20 min in.** Steady iterations then abrupt silence with no traceback is an **OOM signature**, and NR4A1 solvates to **288,352 atoms against a 32 GB default host** — leading hypothesis, **not** the cause, until the container exit reason is read. That read decides the fix: OOM needs a **bigger host** (and a launcher that filters on RAM, not just GPU and $/ns), while a shorter commit interval alone would make the leg crawl 16 iterations at a time, paying 20-minute image pulls every ~15 min. NR4A3 healthy at **`production/160`** | NR4A1 blocked pending the exit reason; **no fourth rental** | 5a-KS realised **~$1.5** of ~$12. **$/ns:** NR4A3 $0.00658 · **1.51× basis ⚠ DRIFT**; NR4A1 $0.00562 · 1.29× basis |
