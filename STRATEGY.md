...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-27 3:05 AM ET**)
...
|---|---|---|---|
| **RUNG 5a-KS** (LANE 16) — the ligand-side causal kill-switch | **The two legs have diverged decisively, and the earlier "host churn" reading is WRONG for NR4A1 (Appendix A 30).** NR4A3 (**45972721**, RTX 4090) is **healthy at `production/160`**, up 96 min, advancing 120→160 — the first 5a-KS leg to reach production. NR4A1 is **STALLED at `warmup/192` for the THIRD time, on the THIRD independent machine** (45947762 → 45972724 → **45974679**), each freezing at **exactly the same iteration**, log 45 min stale. **Same rung, same image, same code, sibling leg fine** — a failure that reproduces at one specific iteration across three separate machines is **the leg or its checkpoint, not the market**. Under $0 diagnosis, with three independent attempt logs to compare; the leading hypothesis is a **poisoned checkpoint at 192** (every host resumes the same S3 state and dies the same way), testable by resuming from an earlier checkpoint. **NOT relaunching on a fourth host** — three rentals have produced zero iterations past 192 between them | NR4A3 running; **NR4A1 blocked**, and if the checkpoint is poisoned the warmup already done is lost — a real cost to state, not absorb | 5a-KS realised **~$1.5** of ~$12. **$/ns:** NR4A3 $0.00658 · **1.51× basis ⚠ DRIFT**; NR4A1 $0.00562 · 1.29× basis |
