...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-27 4:05 AM ET**)
...
|---|---|---|---|
| **RUNG 5a-KS** (LANE 16) — the ligand-side causal kill-switch | **Both legs RUNNING and healthy; the "~15–20 min death cadence" was measured and does not exist (Appendix A 33).** Every archived attempt of every unit in the lane was censused for $0: the 5a-KS legs restart *less* often than the calibration units held up as the control, and one NR4A3 attempt ran 6.8 h unbroken through the entire 1600-iteration warmup. Restart cost re-measured at **≈1.15×**, not the retracted 3.5×. **NR4A3** is in production, committed through iteration 240 of 2000 at 18.7–19.3 s/iter; **NR4A1** is in warmup, committed through 320 of 1600 at 14.7–15.1 s/iter. No failure record exists for either leg — every post-pre-equilibration attempt ends mid-iteration with no traceback and without the `[tvast] FAILED at <phase>` line the pre-equilibration aborts all carried, which is the signature of the host going away rather than the process faulting. Treated as **ordinary spot churn** per CLAUDE.md §6, not investigated further | NR4A1 warmup 1280 iters remaining ≈5.3 h + production; **no reliable finish ETA while spot churn sets the wall clock** | 5a-KS realised **~$1.5** of the ~$12 rung ([vast-ladder-repricing.json](./research/modalities/vast-ladder-repricing.json)). **$/ns:** NR4A3 $0.00658 · **1.51× basis ⚠**; NR4A1 $0.00562 · 1.29× basis |
| **Step 1 fan-out** (LANE 17/21) — 19 congeneric RBFE edges | Shakeout advancing steadily: **`complex/production@1400`** (from @1080), **18.5 s/iter** over a 99-min window — and **unaffected by the 5a-KS restart fault**, which is what makes that fault lane-specific rather than repo-wide. **The 18 are held by the TERMINUS gate, NOT by price (Appendix A 33).** The tick's own words: *"TERMINUS NOT PROVEN — no unit has a ddg.json … Holding 18 unit(s) … submitting 0 … cost of THIS submission (0 units): plan $0.0"*. The price guard evaluated **once**, at 10:06 PM, and has not run since, because with nothing submitted there is nothing to price — the $112.71-vs-$80.44 snapshot is that single evaluation, not a six-hour hold. Price becomes binding only when the terminus proves, and the guard re-evaluates against the market at that moment | ΔΔG **without arithmetic** — the production target is still unread. The whole remaining tranche prices at **plan $35.67 (band $14.82–80.44)** | ladder **~$36 ($15–80)**, realised **$0.35**, **$0 while held**. **$/ns:** shakeout $0.00767 · **1.76× basis ⚠ DRIFT** |
| ⚠ **GITHUB CRON DELIVERY HAS STOPPED** — affects every watchdog | **No `schedule` event on ANY workflow since 10:04 PM**, and **no Actions run of any kind between 10:41 and 11:37 PM**. All three workflows report `state: active`, so they are not disabled, and **`workflow_dispatch` works fine** — I dispatched both watchdogs plus the fan-out tick at 11:37 PM and all three went green. So this is GitHub's scheduled-event delivery, not the repo. Gaps at this check: vast-watchdog **92 min**, ternary **139 min**, autoscale **144 min**, against the 56–97 min band LANE 18 measured. **Consequence that matters:** the market guard's ~4:05 AM escalation fires *from the tick*, so if cron stays dead it would not fire — the one path meant to reach trimcrae without an agent awake depends on the thing that is failing | manual dispatch each hourly check restores coverage meanwhile | **$0** | — |
