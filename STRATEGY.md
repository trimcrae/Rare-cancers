...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-26 10:20 PM ET**)
...
|---|---|---|---|
| **RUNG 5a-KS** (LANE 16) — the ligand-side causal kill-switch | **The two legs have diverged.** `…ternary_nr4a3` (45947117) is **RUNNING and advancing — `warmup/640`, prev 192, stall 0**, up 157 min. `…ternary_nr4a1` (45947762) is **`SETUP_STALL`: up 146 min with ZERO committed iterations** against a 90-min grace — *"setup is hung, not slow… NOT relaunching — a relaunch would hang the same way."* Under diagnosis, and the open question is whether the box can even see the **exact-substructure identity-map fix**, since it was rented ~6:51 PM, *before* that fix was pushed: if the repo is fetched once per RENTAL rather than per retry it will loop on the old code until destroyed. Root cause of the underlying defect is settled — at pre-equilibration `ligA` and `ligB` are **the same molecule** (this rung writes one pose twice), so LOMAP was asked for an **identity map** and returned 80/111, then 110/111 at 300 s; the fix is an exact full-molecule substructure match, verified **111/111**. LANE 19's audit independently confirms the legs were still at **110/111 even at 300 s** | NR4A3 past warmup shortly; NR4A1 blocked. **`S` has no ETA** | 5a-KS realised **~$0.20** plus ~**$0.34** burned by the stalled NR4A1 box for zero iterations |
| **Step 1 fan-out** (LANE 17/21) — 19 congeneric RBFE edges | Shakeout **past warmup at `complex/production@40`** and advancing. **The 18 are now held by TWO gates, and the second one is holding.** The terminus gate (releases on the first `ddg.json`) is unchanged; added per CLAUDE.md §6 is a **$/ns market guard** that prices the fleet over the N cheapest *benched* offers and refuses a launch projecting past the top of this rung's own band. Live read **10:06 PM ET: 54 offers, only 12 PRICEABLE, mean $0.013774/ns = 3.16× the $0.004359 ladder basis → projected $112.71 against a $80.44 ceiling → HELD.** Without it the automatic release would have bought a **~$113 tranche against the authorised $15–80**. The board is **not thin in offers — it is thin in BENCHED cards**: 42 of 54 are cards never benched, which the repo ranks last rather than guessing at. Every hold is committed to [`step1-fanout-market-hold.json`](research/modalities/step1-fanout-market-hold.json) with its snapshot, and a hold persisting past **6 h** fails the job so GitHub notifies — the guard never buys in on its own, and never idles silently | shakeout ΔΔG **without arithmetic** — the iteration target is still unread. The 18 are **market-dependent**; escalation fires **~4:05 AM ET** if the board stays this bad | ladder **~$36 ($15–80)**, realised **$0.35**, **$0 while held**. ⚠ **~$31.9 is stale** — built on $0.1224/hr; the cheapest priceable host tonight is **$0.1867/hr → $0.00767/ns, already 1.76× basis** |
