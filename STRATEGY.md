...
| **The closure triangle** (LANE 9/20) — decides whether valB's miss is fixable at all | **2 OF 4 LEGS LANDED, 2 STILL RUNNING (as of 6:21 AM ET Jul 30).** Both binary legs are done — `calib_lo_to_lo2__binary_vhl` finished at production/2000 and `calib_hi_to_lo2__binary_vhl` before it. The two ternary legs are at **89.9 %** and **85.5 %** and are the long pole | **`R` DOES NOT EXIST YET AND WILL NOT BE APPROXIMATED.** `valb_triangle_reduce.reduce_triangle` refuses a partial set by construction — *"Five legs cannot close a triangle — an R from a partial cycle is not a smaller-n R, it is a different number"* — so `task=triangle-reduce` runs only once all four land. On measured throughput (~3.1 %/h effective) that is **late morning Jul 30**, not overnight | the 4-leg tranche was priced against its own **$3.85** ceiling per pass | every rental of the night cleared the **$0.006539/ns** buy line; the two live legs are at **1.50× and 1.32× basis**. ⚠ **The slip is host churn, not price and not slow cards** — five host changes across the two ternary legs in ~2 h, each costing a cold start (image pull, stage restore, checkpoint restore, re-minimise), giving a duty cycle near **39 %**: ~8 %/h in-MD against ~3.1 %/h effective. Cheap bid-tier rentals near the market floor are outbid first (valB r2's 0.807×-basis host lasted 13 min; its 1.50× replacement held to completion). Bidding above the floor to buy host stickiness is a pricing-policy change and is trimcrae's call, not taken here |
