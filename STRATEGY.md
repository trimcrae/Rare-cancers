...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-26 5:33 PM ET**)
...
|---|---|---|---|
| **RUNG 5a-KS** (LANE 16) — the ligand-side causal kill-switch | **BOTH LEGS RUNNING.** The pre-equilibration gate returned **CONFIRMED verbatim** (ARM A rc=1 on a force-field template error, ARM B rc=0), corroborated physically by the staged tars going **471,040 → 921,600 B**. The smoke leg then reached a **real success terminus** — `status=done, committed=production/12, NaN=False`, host self-destroyed — not an absence of errors. Legs: `…ternary_nr4a3` on **45941659** (RTX 4090, $0.2348/hr) and `…ternary_nr4a1` on **45941913** ($0.2237/hr); NR4A1's first host answered `resources_unavailable` and the lane destroyed it and moved on by itself rather than queueing. Watchdog armed on both | ~**8 h/leg**, so **`S` ~1–2 AM Mon** | **~$3.6 projected for the pair**, against the ladder's **~$12** allowance — the gap is the smoke leg's measured **7.9 s/iter** vs the calibration ternary's 17.0, which turns an allowance into a projection. Realised 5a-KS to date **$0.11** |
| **Step 1 fan-out** (LANE 17) — 19 congeneric RBFE edges | **The lane no longer needs an agent alive.** Shakeout unit `…cw_ev_5cooh…` resumed a 2-day-old wave-1 checkpoint **on a different host** and ran at **~261 iter/h** against the ~265 wave-1 implies — full card rate, so a cross-host resume costs nothing. Preempted 4:31 PM, auto-resumed onto **45938720**. The other 18 are held by a terminus gate that releases them on the first `ddg.json`. **The auto-release of the 18 is now PROVEN autonomous on two independent triggers (LANE 18).** `step1-fanout-autoscale.yml` fired on `schedule` for the **first time at 5:30 PM ET** (run `30221227033`, **completed success 5:32 PM**) — **70 min** after landing on main, so the earlier "*/20 that never fires" was **first-fire latency, not a defect**: measured first-fire on this repo's own recent scheduled workflows is **56 / 208 / 210 min**. Its steady state will not be */20 either — measured gaps today are **56–97 min** for the two workflows asking */15 and **85–148 min** for the one asking hourly, i.e. GitHub throttles this repo to **~1 run per workflow per hour whatever is requested**, which is well inside the release window. Because a single unproven trigger was carrying the whole night, `vast-watchdog.yml` (12 `schedule` events today) now also carries an independent job that **dispatches** the tick — it duplicates no logic, so the gate and launcher stay single-homed. Verified live at **5:29 PM ET**: a watchdog pass created step1 run `30221167072`, `actor=github-actions[bot]`, which held the 18 correctly and committed its readout. Gate verdict on that tick: *TERMINUS NOT PROVEN — holding 18, resuming the shakeout, submitting 0, $0* | shakeout ΔΔG **~2:00–3:30 AM** — *an estimate, not arithmetic*: the iteration target is unread until the leg log or the warmup→production crossing lands. Fan-out completes ~13.7 GPU-h after release | ladder **~$36 ($15–80)**. The lane's own ledger, at the **$0.1224/hr actually charged**, projects the tranche at **~$31.9** — under plan, and confirmation that wave 1's $0.35–0.39/hr was the retired ×1.9 bid multiplier rather than the market. **Realised so far $0.09** |
