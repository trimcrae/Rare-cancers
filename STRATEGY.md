...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-26 9:05 PM ET**)
...
|---|---|---|---|
| **RUNG 5a-KS** (LANE 16) — the ligand-side causal kill-switch | **Root cause found, and it was not the mapper budget.** At pre-equilibration `ligA` and `ligB` are **the same molecule** — `_load_ligands` reads the two SDF records verbatim and this rung writes **one pose twice**; the endpoint rebuild from SMILES happens later, in the FEP engine. So LOMAP was asked for an **identity map** and returned 80/111, then 110/111 after the 20 s→300 s raise, while the same inputs gave 111/111 elsewhere. **A self-map cannot be incomplete for any chemical reason**, so every shortfall was a search artifact turning identical atoms into dummies — and it explains the Kekulé-vs-aromatic indole as one molecule re-perceived across a transplant it never needed. **Fix:** exact full-molecule substructure match when both endpoints share atom count and canonical SMILES — deterministic, no timeout, host-independent; verified **111/111, exact identity permutation**. The 300 s budget stays for genuinely different endpoints but is no longer load-bearing here. **The failure was HOST-DEPENDENT, not deterministic** — it passed on a CPU runner and failed twice on Vast, which is why running the same input elsewhere caught it and reasoning about it did not. NR4A3 healthy at `Iteration 505/512` of warmup (~14 s/iter, ~2 h wall); NR4A1's stale FAILED record cleared (archived, not deleted) and its next start picks up the identity-map path | NR4A3 well into warmup; NR4A1 restarting. **`S` still without an ETA** until both legs are past warmup | 5a-KS burn **$0.27/hr**, realised **~$0.20**; NR4A1's fail/retry loop cost ~**$0.06 per cycle** and existed *because* the failed-leg retry fix works |
| **Step 1 fan-out** (LANE 17/21) — 19 congeneric RBFE edges | **Recovered and advancing past the resume point.** The 5:51/6:51 PM watchdog reds were REAL: instance **45938720 never ran its container** — `actual_status=loading` 4:39→6:52 PM with docker layer messages, container stdout begins **7:31:48 PM**, first `phase.txt` write 8 s later; the frozen `/260` was its **predecessor's** commit. Measured pull **2.91 GiB / 177 min ≈ 2.4 Mbit/s** against **142.4** advertised, so a bandwidth floor at selection would not have caught it. Machine **28164** excluded; **45951628** (RTX 4080S) started its container in **21 min** and the watchdog reads **260 → 300 → 320, stall 0**. **Six defects fixed and on main**, the two worst being `_lprint(…, flush=True)`, which crashed `mode_launch` on its **first successful submission** — renting a box *before* the ledger write, the watchdog arming and the readout, so it billed while invisible to both spend and monitoring — and the launcher arming `vast-watch.json` on the **fleet branch while the watchdog reads main**, which means **the 18 would have run unwatched** | ΔΔG **without arithmetic** — the warmup/production iteration targets have never been read off a real build, and **no iter/h is quoted**: two commit blocks is not a rate window (Appendix A 19b) | ladder **~$36 ($15–80)**. Realised **$0.35**, of which the stalled window is **$0.159 for zero iterations**. ⚠ **The ~$31.9 projection is STALE and must not be carried forward** — it was built on $0.1224/hr and the replacement host bills **$0.2247/hr**, because excluding the bad machine pushed selection onto a higher floor. One host is not a market; re-derive from the real `_rentals.json` once the fleet is up. If the 18 land in that band the tranche is **~$58** — inside the $15–80 authorised band, but well above the $36 point estimate |
