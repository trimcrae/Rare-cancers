...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-26 6:00 PM ET**)
...
**Two lanes are live: the RUNG 5a-KS ternary legs (LANE 16) and the Step 1 fan-out (LANE 17).** Both LANE-13 paralogue legs and all four RUNG 2b
...
|---|---|---|---|
| **RUNG 5a-KS** (LANE 16) — the ligand-side causal kill-switch | **BOTH LEGS WENT DOWN ON THEIR FIRST ATTEMPT, for two different reasons — and neither was a stall: both were demonstrably advancing when they stopped.** `…ternary_nr4a3` (45941659) reached warmup and was running steadily (`Iteration 44/64, 7.742 s/iter`) when the host went with **no failure record written** — that is a **preemption**, routine, and the pipeline withholds a `leg.json` for host-side aborts precisely so the relaunch picks a different machine. **Relaunching.** `…ternary_nr4a1` (45941913) completed its full 0.5 ns relaxation and then **was REFUSED by the engine's own endpoint verification**: **80 mapped / 31 dummy / `graph_identical: false`**, against the NR4A3 arm's **111 / 0 / true** on the *same construct*. Cause named by the engine and it is **not** the aza-scan — the warhead's indole is **Kekulé on one side and aromatic on the other**, a perception difference. Staging is cleared **by measurement**: both `ligands.sdf` pulled from the stage cache the rented hosts actually read, in the parity image, are identical constitution, 21 aromatic atoms, `C44H47N7O13`, indole aromatic. So the corruption is **downstream, in the endpoint rebuild** where `ligB` is core-transplanted onto the relaxed `ligA`. Next step is a **$0 two-arm CPU diagnostic, deliberately not a re-rental** — the failure is deterministic for this pose, so re-renting reproduces it at full pre-equilibration cost | NR4A3 back to ~8 h from relaunch; NR4A1 gated on the $0 diagnostic, so **`S` slips past ~1–2 AM Mon — no new estimate until the cause is named** | pair still projects **~$3.6** against the ladder's **~$12**; the two dead attempts bought **~$0.1** of pre-equilibration between them. 5a-KS realised **~$0.2** |
| **Step 1 fan-out** (LANE 17) — 19 congeneric RBFE edges | **The lane no longer needs an agent alive.** Shakeout unit `…cw_ev_5cooh…` resumed a 2-day-old wave-1 checkpoint **on a different host** and ran at **~261 iter/h** against the ~265 wave-1 implies — full card rate, so a cross-host resume costs nothing. Preempted 4:31 PM, auto-resumed onto **45938720**. The other 18 are held by a terminus gate that releases them on the first `ddg.json`. **The auto-release of the 18 is PROVEN autonomous (LANE 18).** `step1-fanout-autoscale.yml` fired on `schedule` for the **first time at 5:30 PM ET** (run `30221227033`, **completed success 5:32 PM**) — **70 min** after landing on main, so the earlier "*/20 that never fires" was **first-fire latency, not a defect**: measured first-fire on this repo's own recent scheduled workflows is **56 / 208 / 210 min**. Its steady state will not be */20 either — measured gaps today are **56–97 min** for the two workflows asking */15 and **85–148 min** for the one asking hourly, i.e. GitHub throttles this repo to **~1 run per workflow per hour whatever is requested** — still inside the release window, but the real margin is **~3× thinner than the cron literal reads**, so the release runs on **two independent triggers**: `vast-watchdog.yml` (proven, 12 `schedule` events today) also carries a job that **dispatches** the tick. It duplicates no logic — gate and launcher stay single-homed — so the two paths cannot drift. Verified live twice, most recently at **5:39 PM ET** on the merged SHA `ac6213f5`: watchdog run `30221532247` → step1 run `30221535623`, `actor=github-actions[bot]` | shakeout ΔΔG **~2:00–3:30 AM** — *an estimate, not arithmetic*: the iteration target is unread until the leg log or the warmup→production crossing lands. Fan-out completes ~13.7 GPU-h after release | ladder **~$36 ($15–80)**. The lane's own ledger, at the **$0.1224/hr actually charged**, projects the tranche at **~$31.9** — under plan, and confirmation that wave 1's $0.35–0.39/hr was the retired ×1.9 bid multiplier rather than the market. **Realised so far $0.09** |
