...
**As of 2026-07-27 6:05 PM ET · 7 gates passed · 3 failed (all caught before the spend) · 2 in flight, neither
graded yet · 2 deliverables done · realised spend $23.60 machine-ledgered.**
...
## ⏱️ IN FLIGHT — what is actually running right now (as of **2026-07-27 6:05 PM ET**)
...
|---|---|---|---|---|
| **Step 1 fan-out** (LANE 17/21) — 19 congeneric RBFE edges | **Billing and advancing.** At the 5:44 PM tick: **15 live instances** (9 running · 4 loading · 2 exited), **16 units at `leg-complex-running`**, 1 done, 2 at `leg-complex-FAILED-rc1`. Furthest units are at `complex/production@1640` and `@1480`. Of the two rc1 failures, **`cw_bio_nmethyl_amide` is the permanently blocked one** and `cw_bio_primary_amide` failed at 1:41 PM and has since been re-placed. Host churn this tick: **3 never started** (host fault, machines published) and **4 preempted** — routine, noted lightly per CLAUDE.md §6 | no fleet ETA — width is set by what the market sells at or under the buy line, and the last tick could only place 5 of 5 after 4 submits found **no offer left inside our own exclusion set** (51 machines removed before ranking: *our filter, not the market*) | realised **$20.11**, machine-ledgered, against a DERIVED cap of **$74.91** (`market_ceiling_usd(19)`) → **$54.80 headroom**. Ladder plan for the lane **~$36 ($15–80)** | last wave placed at **$0.005388–$0.006384/ns · 1.58–1.87× basis**, ⚠ **PAYING** — every one **under** the $0.006539/ns buy line. Effective ceiling **binds on the rate line**, not on the dollar ceiling ($0.008594/ns, 2.52×) |
| **valB_mini r1+r2** (LANE 19) — the 4 replicate legs | **Billing, 4 of 4 hosted, 0 done.** ⚠ **This is cohort 4** — the 4:00 PM cohort is gone and the current hosts were rented at **4:51 PM ET**: ternary r1 `46040507`, binary r1 `46040514`, ternary r2 `46040577`, binary r2 `46040659`. **A per-host rate on this lane has a half-life of about two hours**, so every rate below names its cohort ([Appendix A](#appendix-a--superseded-numbers-and-retracted-claims) 38) | no deadline — every leg is checkpointed in S3. A replicate SD exists only once all 4 land and `task=reduce-reps` runs | derived, never typed (`rung_band_usd(4)`): plan **$7.32**, ceiling **$20.74**, 536 ns/leg. Realised is **NOT machine-ledgered on this lane** — the gap is named in [`realised-spend.json`](research/modalities/realised-spend.json) | cohort 4, off the rental receipt: **1.500× · 1.500× · 1.859× · 1.649×** basis ($0.005119 / $0.005119 / $0.006342 / $0.005626 per ns), ⚠ **PAYING** — all four **under** the buy line. *(The 4:00 PM cohort 3 was 1.500/1.528/1.500/1.574× and is gone.)* |
...
| **The closure triangle** (LANE 9/20) — decides whether valB's miss is fixable at all | **NOT LAUNCHED — the atom-map gate refused it at 6:01 PM ET, before any rental.** The price gate had already cleared it (projected **$2.49** against this rung's **$3.85** ceiling, 1.75× basis), and the map gate stopped it anyway: *the launch cannot rent until the map is proven complete.* Working as designed — a refusal before the money, not after | blocked on the map, not on the market | **$0 spent** | ⛔ **REFUSED before rental — $0 spent** (the 1.75× board reading is what we did **not** buy) |
...
**Two lanes are billing** — the **Step 1 fan-out** (19 congeneric RBFE edges) and the **valB_mini replicates**
(4 legs). **Two more are held, deliberately and for stated reasons**: RUNG **5a-KS** behind the relaunch price
gate, and the **restrained binary re-run** behind the closure triangle's `R`. Live state, ETAs, cost and `$/ns`
...
- **The closure triangle** — priced and gate-cleared, but its launch is refused by the atom-map gate until the
  map is proven complete. That is a code fix, not a spend decision.
