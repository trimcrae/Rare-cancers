...

{
  ...
  echo "      lease count to 5 against a \`subagent_width\` of 5, \`continuity.py\` read AT CAPACITY,"
  echo "      returned 0, and this hook said nothing while the row sat untouched for 39 minutes."
  echo "      A worker is never blocked by itself; your own lease is now excluded from that count."
  echo "      ⛔ A LEASE IS NOT A"
